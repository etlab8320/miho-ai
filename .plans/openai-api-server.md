# OpenAI-Compatible API Server for Miho Agent
...
SSE streaming. By exposing this endpoint, miho-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  miho-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.miho/.env`
2. Run `miho gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with miho-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns miho-agent as a model) |
...
Authorization: Bearer miho-api-key-here
...
But miho-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "miho-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured miho-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of miho-agent's core prompt
...
Once implemented, miho-agent works as a drop-in backend for:
