#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Miho Skills Hub.
...
Used by miho_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            miho_meta = metadata.get("miho", {})
            if isinstance(miho_meta, dict):
                tags = miho_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.miho/skills/ during setup.  They are discoverable via the Skills Hub
...
# ---------------------------------------------------------------------------
# Miho centralized index source
# ---------------------------------------------------------------------------

MIHO_INDEX_URL = "https://miho-agent.nousresearch.com/docs/api/skills-index.json"
MIHO_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "miho-index.json"
MIHO_INDEX_TTL = 6 * 3600  # 6 hours


def _load_miho_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for MIHO_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if MIHO_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - MIHO_INDEX_CACHE_FILE.stat().st_mtime
            if age < MIHO_INDEX_TTL:
                return json.loads(MIHO_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(MIHO_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Miho index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Miho index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        MIHO_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        MIHO_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class MihoIndexSource(SkillSource):
    """Skill source backed by the centralized Miho Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "miho-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "miho-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        MihoIndexSource(auth=auth), # Centralized index (search + resolved install paths)
