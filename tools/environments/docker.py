...
from tools.environments.local import _MIHO_PROVIDER_ENV_BLOCKLIST
...
def _load_miho_env_vars() -> dict[str, str]:
    """Load ~/.miho/.env values without failing Docker command execution."""
    try:
        from miho_cli.config import load_env
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``MIHO_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("MIHO_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using MIHO_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image entrypoint drops from root to the 'miho'
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.miho/sandboxes/). Non-persistent
        ...
        container_name = f"miho-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Miho secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _MIHO_PROVIDER_ENV_BLOCKLIST)
        miho_env = _load_miho_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = miho_env.get(key)
