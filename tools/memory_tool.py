...
# (MIHO_HOME env var changes) are always respected.  The old module-level
# constant was cached at import time and could go stale if a profile switch
# happened after the first import.
def get_memory_dir() -> Path:
    """Return the profile-scoped memories directory."""
    return get_miho_home() / "memories"
...
_MEMORY_THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.miho/\.env|\~/\.miho/\.env', "miho_env"),
]
...
class MemoryStore:
    ...
    def __init__(self, memory_char_limit: int = 2200, user_char_limit: int = 2200):
