...
selectable via ``miho tools`` → Image Generation; the active model is
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                ...
                f"  • Pick a different model via `miho tools` → Image Generation."
            ) from exc
        raise
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_nous_tools_enabled():
        lines.append(
            ...
            "gateway enabled (`miho setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `miho tools` "
        "→ Image Generation (run `miho plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping miho with a FAL key configured
