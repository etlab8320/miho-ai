...
it is automatically downloaded from GitHub releases to $MIHO_HOME/bin/tirith.
...
def _get_miho_home() -> str:
    """Return the Miho home directory, respecting MIHO_HOME env var."""
    return str(get_miho_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_miho_home(), ".tirith-install-failed")
...
def _miho_bin_dir() -> str:
    """Return $MIHO_HOME/bin, creating it if needed."""
    d = os.path.join(_get_miho_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $MIHO_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_miho_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $MIHO_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $MIHO_HOME/bin/tirith
    ...
    miho_bin = os.path.join(_miho_bin_dir(), "tirith")
    if os.path.isfile(miho_bin) and os.access(miho_bin, os.X_OK):
        _resolved_path = miho_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return miho_bin
