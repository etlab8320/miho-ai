"""Miho-managed Camofox state helpers.

Provides profile-scoped identity and state directory paths for Camofox
persistent browser profiles.  When managed persistence is enabled, Miho
...
def get_camofox_state_dir() -> Path:
    ...
    return get_miho_home() / CAMOFOX_STATE_DIR_NAME / CAMOFOX_STATE_SUBDIR


def get_camofox_identity(task_id: Optional[str] = None) -> Dict[str, str]:
    """Return the stable Miho-managed Camofox identity for this profile.

    The user identity is profile-scoped (same Miho profile = same userId).
