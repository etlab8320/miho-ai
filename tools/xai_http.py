...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``miho tools`` repaint, tool-registration scans,
    ...
    2. ``~/.miho/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_miho_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.miho/.env`` first, then ``os.environ``.

    Wraps :func:`miho_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from miho_cli.config import get_env_value as _miho_get_env_value

        value = _miho_get_env_value(name)
...
def miho_xai_user_agent() -> str:
    """Return a stable Miho-specific User-Agent for xAI HTTP calls."""
    try:
        from miho_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Miho-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Miho-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``miho_cli.config.get_env_value`` so keys
    stored in ``~/.miho/.env`` (the standard Miho location) are honored —
