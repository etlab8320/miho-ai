...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Miho exit
...
# display_miho_home imported lazily at call site (stale-module safety during miho update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of miho directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "miho-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = os.getenv("MIHO_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("MIHO_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (MIHO_INTERACTIVE=1).
    ...
    try:
        os.environ["MIHO_SPINNER_PAUSE"] = "1"
        time.sleep(0.2)
        ...
        if "MIHO_SPINNER_PAUSE" in os.environ:
            del os.environ["MIHO_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (MIHO_INTERACTIVE=1):
    ...
    # interactive Miho password prompt or the sudo -S password-pipe path.
    ...
    # silently without Miho prompting.
    if not has_configured_password and not sudo_password and _sudo_nopasswd_works():
        return command, None

    if not has_configured_password and not sudo_password and env_var_enabled("MIHO_INTERACTIVE"):
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Miho can track lifecycle and output.
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, MihoSweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.miho/.env or environment variables."
        )
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "Modal credentials/config were found. Log in with `miho model` or "
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so Miho can track the process, then run "
        ...
        if background:
            ...
            try:
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("MIHO_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("MIHO_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("MIHO_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("MIHO_SESSION_USER_ID", "")
                        _gw_user_name = _gse("MIHO_SESSION_USER_NAME", "")
                        _gw_message_id = _gse("MIHO_SESSION_MESSAGE_ID", "")
            ...
            if modal_state["selected_backend"] != "direct":
                if modal_state["managed_mode_blocked"]:
                    logger.error(
                        ...
                        "a paid Nous subscription is required for the Tool Gateway and no direct "
                        "Modal credentials/config were found. Log in with `miho model` "
