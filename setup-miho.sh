...
# 5. Symlinks the 'miho' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Miho Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/miho-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink miho into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up miho command..."

MIHO_BIN="$SCRIPT_DIR/venv/bin/miho"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$MIHO_BIN" "$COMMAND_LINK_DIR/miho"
echo -e "${GREEN}✓${NC} Symlinked miho → $COMMAND_LINK_DISPLAY_DIR/miho"
...
# ============================================================================
# Seed bundled skills into ~/.miho/skills/
# ============================================================================

MIHO_SKILLS_DIR="${MIHO_HOME:-$HOME/.miho}/skills"
mkdir -p "$MIHO_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.miho/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$MIHO_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  miho status        # Check configuration"
if is_termux; then
    echo "  miho gateway       # Run gateway in foreground"
else
    echo "  miho gateway install # Install gateway service (messaging + cron)"
fi
echo "  miho cron list     # View scheduled jobs"
echo "  miho doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m miho_cli.main setup
fi
