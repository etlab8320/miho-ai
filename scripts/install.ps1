# ============================================================================
# Miho Agent Installer for Windows
# ============================================================================
...
$RepoUrlSsh = "git@github.com:etlab8320/miho-ai.git"
$RepoUrlHttps = "https://github.com/etlab8320/miho-ai.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Miho AI Installer                       |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  Miho AI local agent installer.                         |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $MihoHome)) {
        New-Item -ItemType Directory -Force -Path $MihoHome | Out-Null
    }
    $envFile = Join-Path $MihoHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $MihoHome "node"
...
function Install-Git {
    ...
    which Miho uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\miho\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Miho needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Miho copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\miho\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``MIHO_GIT_BASH_PATH`` (User scope) so Miho can find it in a fresh
    ...
    # Download PortableGit into $MihoHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $MihoHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Miho features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Miho needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``MIHO_GIT_BASH_PATH`` (User env scope) so Miho can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $MihoHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $MihoHome\git\usr\bin\bash.exe
    $candidates += "$MihoHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$MihoHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("MIHO_GIT_BASH_PATH", $candidate, "User")
            $env:MIHO_GIT_BASH_PATH = $candidate
            Write-Info "Set MIHO_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Miho may not find Git Bash."
    Write-Info "If needed, set MIHO_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$MihoHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$MihoHome\node;$env:Path"
        Write-Success "Node.js $version found (Miho-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # The portable zip path drops node.exe + npm into $MihoHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $MihoHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\miho-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$MihoHome\node") { Remove-Item -Recurse -Force "$MihoHome\node" }
                Move-Item $extractedDir.FullName "$MihoHome\node"
                ...
                $version = & "$MihoHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $MihoHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running miho processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/etlab8320/miho-ai/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/etlab8320/miho-ai/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/etlab8320/miho-ai/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\miho-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\miho-agent-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `miho.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install miho-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and miho.exe broken with
    ...
    # here, not 30 seconds later when the user runs `miho`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `miho dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `miho dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `miho dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up miho command..."
    
    if ($NoVenv) {
        $mihoBin = "$InstallDir"
    } else {
        $mihoBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so miho is globally available
    # On Windows, the miho.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$mihoBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$mihoBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $mihoBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set MIHO_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\miho instead
    # of the Unix default ~/.miho
    $currentMihoHome = [Environment]::GetEnvironmentVariable("MIHO_HOME", "User")
    if (-not $currentMihoHome -or $currentMihoHome -ne $MihoHome) {
        [Environment]::SetEnvironmentVariable("MIHO_HOME", $MihoHome, "User")
        Write-Success "Set MIHO_HOME=$MihoHome"
    }
    $env:MIHO_HOME = $MihoHome
    
    # Update current session
    $env:Path = "$mihoBin;$env:Path"
    
    Write-Success "miho command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.miho directory structure
    New-Item -ItemType Directory -Force -Path "$MihoHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MihoHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MihoHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MihoHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MihoHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MihoHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MihoHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MihoHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MihoHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$MihoHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.miho/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.miho/.env"
        }
    } else {
        Write-Info "~/.miho/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$MihoHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.miho/config.yaml from template"
        }
    } else {
        Write-Info "~/.miho/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Miho's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.miho/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.miho/"
    
    # Seed bundled skills into ~/.miho/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.miho/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.miho/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$MihoHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.miho/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'miho setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\miho-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\miho-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\miho-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.miho/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'miho setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m miho_cli.main setup
    } else {
        python -m miho_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $mihoCmd = "$InstallDir\venv\Scripts\miho.exe"
    if (-not (Test-Path $mihoCmd)) {
        $mihoCmd = "miho"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$MihoHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'miho whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: miho gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$MihoHome\logs\gateway.log"
            Start-Process -FilePath $mihoCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$MihoHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: miho gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: miho gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$MihoHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$MihoHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$MihoHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$MihoHome\miho-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   miho              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   miho setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   miho config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   miho config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   miho gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   miho update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Miho repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding Miho to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/etlab8320/miho-ai/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
