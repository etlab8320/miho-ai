...
# Bootstrap Open WebUI against Miho Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.miho/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-miho.sh
...
#   MIHO_API_PORT=8642
#   MIHO_API_HOST=127.0.0.1
#   MIHO_API_MODEL_NAME='Miho Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Miho Agent WebUI}"
...
MIHO_ENV_FILE="${MIHO_ENV_FILE:-$HOME/.miho/.env}"
MIHO_API_PORT="${MIHO_API_PORT:-8642}"
MIHO_API_HOST="${MIHO_API_HOST:-127.0.0.1}"
MIHO_API_CONNECT_HOST="${MIHO_API_CONNECT_HOST:-127.0.0.1}"
MIHO_API_MODEL_NAME="${MIHO_API_MODEL_NAME:-Miho Agent}"
MIHO_API_BASE_URL="http://${MIHO_API_CONNECT_HOST}:${MIHO_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-miho.sh"
LOG_DIR="$HOME/.miho/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$MIHO_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$MIHO_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.miho.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.miho</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.miho"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.miho"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-miho.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Miho Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-miho.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.miho/logs/openwebui.log
StandardError=append:%h/.miho/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-miho.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$MIHO_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Miho API server is configured...'
  upsert_env API_SERVER_ENABLED true "$MIHO_ENV_FILE"
  upsert_env API_SERVER_HOST "$MIHO_API_HOST" "$MIHO_ENV_FILE"
  upsert_env API_SERVER_PORT "$MIHO_API_PORT" "$MIHO_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$MIHO_API_MODEL_NAME" "$MIHO_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$MIHO_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Miho gateway so API server settings take effect...'
  miho gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${MIHO_API_CONNECT_HOST}:${MIHO_API_PORT}/health" >/dev/null; then
    log 'Miho API server did not answer on the first check. Trying to start gateway in the background...'
    nohup miho gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${MIHO_API_CONNECT_HOST}:${MIHO_API_PORT}/health" >/dev/null
  ...
  log "Miho API endpoint: ${MIHO_API_BASE_URL}"
