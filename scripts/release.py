...
AUTHOR_MAP = {
    ...
    "274096618+miho-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "miho@nousresearch.com": "NousResearch",
    ...
    "miho@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (MIHO_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (MIHO_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: miho-eval community link)
    ...
    "mihoagent26@gmail.com": "mihoagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+mihoagent26@users.noreply.github.com": "mihoagent26",
    ...
    "vigo@miho": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"miho-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "miho@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/miho-agent",
    ...
    lines.append(f"# Miho Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Miho Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Miho Agent Release Tool")
