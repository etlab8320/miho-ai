#!/usr/bin/env bash
# Canonical test runner for miho-agent. Run this instead of calling
...
#   * MIHO_LOCAL_EMBEDDING=0 (avoid 20 parallel ONNX model loads in tests)
#   * Env vars blanked (conftest.py also does this, but this
#     is belt-and-suspenders for anyone running pytest outside our
#     conftest path — e.g. on a single file)
#   * Proper venv activation (probes .venv, venv, then ~/.miho/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.miho/miho-agent/venv"; do
...
if [ -f "$HOME/.miho/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.miho"
