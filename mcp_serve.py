"""
Miho MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Miho-specific extra)

Usage:
    miho mcp serve
    miho mcp serve --verbose
...
logger = logging.getLogger("miho.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using MIHO_HOME."""
    try:
        from miho_constants import get_miho_home
        return get_miho_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("MIHO_HOME", Path.home() / ".miho")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from miho_constants import get_miho_home
        directory_file = get_miho_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("MIHO_HOME", Path.home() / ".miho")
...
class EventBridge:
    ...
    This is the Miho equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from miho_constants import get_miho_home
            db_file = get_miho_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("MIHO_HOME", Path.home() / ".miho")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Miho MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "miho",
        instructions=(
            "Miho Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Miho MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
