...
There are two ways to add a platform to the Miho gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.miho/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Miho code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `miho gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `miho_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Miho core.
...
And add it to the `miho-gateway` composite:

```python
"miho-gateway": {
    "includes": [..., "miho-your-platform"]
}
```
...
## 12. Status Display (`miho_cli/status.py`)
...
## 13. Gateway Setup Wizard (`miho_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ miho_cli/ toolsets.py \
  --include="*.py" -l | sort -u
