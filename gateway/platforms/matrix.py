...
# Uses get_miho_home() so each profile gets its own Matrix store.
from miho_constants import get_miho_dir as _get_miho_dir

_STORE_DIR = _get_miho_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("MIHO_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("MIHO_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
    ...
    async def connect(self) -> bool:
        ...
        if self._encryption:
            ...
            try:
                ...
                _acct_id = self._user_id or "miho"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Miho Agent" become "Agent".
        """
        if not body:
            return ""
