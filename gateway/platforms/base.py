...
# Audio file extensions Miho recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Miho' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Miho-created Telegram private-chat
...
# Default location: {MIHO_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_miho_dir("cache/images", "image_cache")
...
AUDIO_CACHE_DIR = get_miho_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_miho_dir("cache/videos", "video_cache")
...
def validate_media_delivery_path(
    ...
    existing regular files under Miho-managed media caches, or roots the
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?miho\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+miho[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from miho_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          MIHO_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          MIHO_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          MIHO_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("MIHO_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("MIHO_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("MIHO_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _miho_run_generation is set on
