"""
Email platform adapter for the Miho gateway.

Allows users to interact with Miho by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "miho-agent" "version" "{_miho_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Miho Agent")
        ...
        msg_id = f"<miho-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
