/**
 * Miho Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.miho/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__MIHO_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__MIHO_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "miho-kanban-dot-triage",
    todo: "miho-kanban-dot-todo",
    ready: "miho-kanban-dot-ready",
    running: "miho-kanban-dot-running",
    blocked: "miho-kanban-dot-blocked",
    done: "miho-kanban-dot-done",
    archived: "miho-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-miho-task";
  ...
  const DOCS_URL = "https://miho-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://miho-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "miho.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="miho-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "miho-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "miho-kanban-md",
  ...
  // standard `drop` event and our `miho-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("miho-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("miho-kanban-column--drop");
          if (target) target.classList.add("miho-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("miho-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("miho-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("miho-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        const token = window.__MIHO_SESSION_TOKEN__ || "";
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "miho-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "miho-kanban-attention",
        "miho-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "miho-kanban-attention-bar" },
        h("span", { className: "miho-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "miho-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "miho-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "miho-kanban-attention-row",
                  "miho-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "miho-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "miho-kanban-attention-row-id" }, task.id),
                h("span", { className: "miho-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "miho-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "miho-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "miho-kanban-diag-action-btn",
      action.suggested ? "miho-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " miho-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".miho-kanban-drawer-comment-row input, .miho-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "miho-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("miho-kanban-diag", sevClass) },
      h("div", { className: "miho-kanban-diag-header" },
        h("span", { className: "miho-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "miho-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "miho-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "miho-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "miho-kanban-diag-data-row" },
                  h("span", { className: "miho-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "miho-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "miho-kanban-diag-data-row" },
                h("span", { className: "miho-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "miho-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "miho-kanban-diag-reassign-row" },
            h("span", { className: "miho-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "miho-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "miho-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "miho-kanban-diag-msg",
              msg.ok ? "miho-kanban-diag-msg--ok" : "miho-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "miho-kanban-section" },
      h("div", { className: "miho-kanban-section-head-row" },
        h("span", { className: "miho-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "miho-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "miho-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "miho-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "miho-kanban-docs-link",
      title: "Open Miho Kanban docs in a new tab",
      "aria-label": "Miho Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `miho kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "miho-kanban-msg-ok" : "miho-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "miho-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "miho-kanban-boardswitcher" },
      h("div", { className: "miho-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "miho-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "miho-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "miho-kanban-dialog-title" },
        ...
        h("div", { className: "miho-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Miho profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "miho-kanban-bulk" },
      h("span", { className: "miho-kanban-bulk-count" },
      ...
      h("div", { className: "miho-kanban-bulk-priority",
      ...
      h("div", { className: "miho-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Miho profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "miho-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("miho-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("miho-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "miho-kanban-trash",
        dragOver ? "miho-kanban-trash--drop" : "",
        props.draggingTaskId ? "miho-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "miho-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "miho-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".miho-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "miho-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("miho-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("miho-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "miho-kanban-column",
        dragOver ? "miho-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "miho-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "miho-kanban-col-check",
        ...
        h("span", { className: cn("miho-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "miho-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "miho-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "miho-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "miho-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "miho-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "miho-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "miho-kanban-lane" },
                  h("div", { className: "miho-kanban-lane-head" },
                    h("span", { className: "miho-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "miho-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "miho-kanban-card--stale-red";
    if (age >= tier.amber) return "miho-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".miho-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "miho-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "miho-kanban-card",
        props.selected ? "miho-kanban-card--selected" : "",
        props.failed ? "miho-kanban-card--failed" : "",
        props.draggingSource ? "miho-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "miho-kanban-card-content" },
          h("div", { className: "miho-kanban-card-row" },
            h("label", {
              className: "miho-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "miho-kanban-card-check",
            ...
            h("span", { className: "miho-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or miho kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "miho-kanban-warning-badge",
                    "miho-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "miho-kanban-needs-assignee",
          ...
          h("div", { className: "miho-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "miho-kanban-card-row miho-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "miho-kanban-assignee",
                            title: `Assigned to Miho profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "miho-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "miho-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "miho-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "miho-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Miho profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Miho profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "miho-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "miho-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "miho-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "miho-kanban-drawer-close",
        ...
        data ? h("div", { className: "miho-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "miho-kanban-drawer-body" },
      h("div", { className: "miho-kanban-drawer-title" },
        h("span", { className: cn("miho-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "miho-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "miho-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "miho-kanban-section" },
        h("div", { className: "miho-kanban-section-head" }, tx(i18n, "result", "Result")),
        h(MarkdownBlock, { source: t.result, enabled: props.renderMarkdown }),
      ) : null,
      h("div", { className: "miho-kanban-section" },
        h("div", { className: "miho-kanban-section-head" },
          `${tx(i18n, "comments", "Comments")} (${comments.length})`),
        comments.length === 0
          ? h("div", { className: "text-xs text-muted-foreground" },
              tx(i18n, "noComments", "— no comments —"))
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "miho-kanban-comment" },
                h("div", { className: "miho-kanban-comment-head" },
                  h("span", { className: "miho-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "miho-kanban-comment-ago" },
      ...
      h("div", { className: "miho-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "miho-kanban-event",
              isDiag ? "miho-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "miho-kanban-event-header" },
                  h("span", { className: "miho-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "miho-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "miho-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "miho-kanban-event-header-plain" },
                  h("span", { className: "miho-kanban-event-kind" }, e.kind),
                  h("span", { className: "miho-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "miho-kanban-event-phantom-row" },
                  h("span", { className: "miho-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "miho-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "miho-kanban-event-payload" },
    ...
    return h("div", { className: "miho-kanban-section" },
      h("div", { className: "miho-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "miho-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `miho-kanban-run--${r.outcome || r.status || "ended"}`
          : "miho-kanban-run--active";
        return h("div", { key: r.id, className: cn("miho-kanban-run", outcomeClass) },
          h("div", { className: "miho-kanban-run-head" },
            h("span", { className: "miho-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "miho-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "miho-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "miho-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "miho-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "miho-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "miho-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "miho-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "miho-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "miho-kanban-pre miho-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "miho-kanban-meta-row" },
      h("span", { className: "miho-kanban-meta-label" }, props.label),
      h("span", { className: "miho-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "miho-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "miho-kanban-meta-row" },
        h("span", { className: "miho-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "miho-kanban-meta-value miho-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "miho-kanban-meta-row" },
        h("span", { className: "miho-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "miho-kanban-meta-value miho-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "miho-kanban-section" },
      h("div", { className: "miho-kanban-section-head-row" },
        h("span", { className: "miho-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "miho-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "miho-kanban-section" },
      h("div", { className: "miho-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "miho-kanban-deps-row" },
        h("span", { className: "miho-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "miho-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "miho-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "miho-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "miho-kanban-dep-chip-x",
      ...
      h("div", { className: "miho-kanban-deps-row" },
        h("span", { className: "miho-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "miho-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "miho-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "miho-kanban-section" },
      ...
      h("div", { className: "miho-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "miho-kanban-home-sub miho-kanban-home-sub--on"
              : "miho-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__MIHO_PLUGINS__ && typeof window.__MIHO_PLUGINS__.register === "function") {
    window.__MIHO_PLUGINS__.register("kanban", KanbanPage);
  }
})();
