"""SimpleX Chat platform adapter (Miho plugin).
...
This adapter ships as a Miho platform plugin under
``plugins/platforms/simplex/``. The Miho plugin loader scans the
...
discoverable and `miho setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Miho — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"miho-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``miho setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.miho/.env`` via ``miho_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("miho_cli.config not available; set SIMPLEX_* vars manually in ~/.miho/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `miho gateway status` without
