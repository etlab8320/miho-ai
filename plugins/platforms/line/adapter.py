"""
LINE Messaging API platform adapter for Miho Agent.
...
to Miho Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``MIHO_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            miho_home = Path.home().joinpath(".miho").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``miho status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``miho status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``miho setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.miho/.env`` via ``miho_cli.config``.
    """
    print()
    ...
    try:
        from miho_cli.config import get_env_var, set_env_var
    except ImportError:
        print("miho_cli.config not available; set LINE_* vars manually in ~/.miho/.env")
        return
