...
# ``model_tools`` import time, so ``miho status``, ``miho chat``, etc.
...
# ``miho_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Miho is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a miho MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${MIHO_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _miho_home = _get_miho_home()
        except (ModuleNotFoundError, ImportError):
            _miho_home = _Path.home() / ".miho"
        self._thread_count_store = _ThreadCountStore(
            _miho_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Miho is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("MIHO_HOME", str(_Path.home() / ".miho"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${MIHO_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a miho MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Miho is thinking…' marker message.
        ...
        "Miho is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Miho is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Miho is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Miho is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Miho is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``miho setup``.

    The setup wizard at ``miho_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.miho/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. miho-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.miho/.env")
    print_info("Restart the gateway: miho gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``miho cron`` running as a
    separate process from ``miho gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Miho plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'miho-agent[google_chat]'",
        ...
        # the "Miho is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Miho is thinking…' marker message "
