"""
Microsoft Teams platform adapter for Miho Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Miho"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        miho_action = data.get("miho_action", "")
        session_key = data.get("session_key", "")

        if not miho_action or not session_key:
        ...
        choice = choice_map.get(miho_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="miho_approve",
                    data={**btn_data_base, "miho_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="miho_approve",
                    data={**btn_data_base, "miho_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="miho_approve",
                    data={**btn_data_base, "miho_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="miho_approve",
                    data={**btn_data_base, "miho_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Miho\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.miho/.env")
