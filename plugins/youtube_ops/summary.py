...
def _merge_notes(llm: Any, metadata: VideoMetadata, notes: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    instructions = (
        ...
        "Return JSON keys: short_title, topic, summary_lines, important_points, lessons, "
        "practical_takeaways, tags. Keep important_points concrete and source-grounded."
    )
    payload = {
...
def _heuristic_chunk_note(chunk: str) -> dict[str, Any]:
    ...
    picked = [re.sub(r"^\d{1,2}:\d{2}(?::\d{2})?\s+", "", item) for item in sentences[:5]]
