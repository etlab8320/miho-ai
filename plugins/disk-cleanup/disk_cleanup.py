"""disk_cleanup — ephemeral file cleanup for Miho Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under MIHO_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly MIHO_HOME and /tmp/miho-*
Never touches: ~/.miho/logs/ or any system directory.
"""
...
try:
    ...
    def get_miho_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("MIHO_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".miho").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$MIHO_HOME/logs/``."""
    return get_miho_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$MIHO_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under MIHO_HOME or ``/tmp/miho-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    miho_home = get_miho_home()
    try:
        path.resolve().relative_to(miho_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/miho-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("miho-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside MIHO_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under MIHO_HOME (but leave MIHO_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "miho-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(miho_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == miho_home:
                continue
            try:
                rel_parts = dirpath.relative_to(miho_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    miho_home = get_miho_home()
    try:
        rel = path.resolve().relative_to(miho_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "miho-agent",
        }:
            return None
        ...
        # Path isn't under MIHO_HOME (e.g. /tmp/miho-*) — fall through.
        pass
