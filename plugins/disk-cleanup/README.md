...
Auto-tracks and cleans up ephemeral files created during Miho Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$MIHO_HOME` and `/tmp/miho-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `MIHO_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `MIHO_HOME` or `/tmp/miho-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$MIHO_HOME/disk-cleanup/` is itself excluded
- `$MIHO_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
