# Miho Achievements Implementation Spec (Detailed)
...
- persisted runtime files:
  - `~/.miho/plugins/miho-achievements/state.json` (existing unlock state)
  - `~/.miho/plugins/miho-achievements/scan_snapshot.json` (new)
  - `~/.miho/plugins/miho-achievements/scan_checkpoint.json` (new)
