# Miho Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: miho-achievements plugin
...
### `GET /api/plugins/miho-achievements/achievements`
...
### `POST /api/plugins/miho-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/miho-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/miho-achievements/overview`
...
- Persist plugin data under:
  - `~/.miho/plugins/miho-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.miho/plugins/miho-achievements/scan_checkpoint.json`
...
- `~/.miho/plugins/miho-achievements/`
