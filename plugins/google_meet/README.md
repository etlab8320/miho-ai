...
Let the miho agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where miho runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `miho meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `miho meet` CLI |
...
| `cli.py` | `miho meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$MIHO_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `miho meet node {run,list,approve,remove,status,ping}` |
...
miho meet install                                      # pip + Chromium
miho meet setup                                        # preflight
miho meet auth                                         # optional
miho meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
miho meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.miho/.env
miho meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
miho meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
miho meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.miho/.env
miho meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, miho will **not** switch your system audio input automatically — the
...
miho plugins enable google_meet
miho meet node run --display-name my-mac --host 0.0.0.0 --port 18789
