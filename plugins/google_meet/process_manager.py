...
session-scoped state file under ``$MIHO_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $MIHO_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_miho_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this miho install, leave it first —
    ...
    env = os.environ.copy()
    env["MIHO_MEET_URL"] = url
    env["MIHO_MEET_OUT_DIR"] = str(out)
    env["MIHO_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["MIHO_MEET_HEADED"] = "1"
    if auth_state:
        env["MIHO_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["MIHO_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if MIHO_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["MIHO_MEET_MODE"] = mode
    if realtime_model:
        env["MIHO_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["MIHO_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["MIHO_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["MIHO_MEET_REALTIME_KEY"] = realtime_api_key
