        ...
        if grade is not None and grade <= 3:
            ...
            if not ok_gap:
                ...
                errors.append(
                    ...
                    "분야로 만들지 마라. 분야가 적어도 세특 근거가 분명한 게 낫다(교과뿐 아니라 창체=동아리·진로·자율도 "
                    "세특 근거가 있으면 분야가 된다). 개수는 강제하지 않는다 — 전공에 진짜 닿는 만큼만. "
                    ...
                    "steps(탐구 단계 3개 이상, 각 단계는 서로 다른 내용으로 '무엇을·어떤 방법으로·어떤 산출물/평가축 "
                    "연결까지' 120~180자로 깊이 있게 — 단계마다 데이터·방법·해석이 구체적이어야 한다). 예를 들면 "
                    "'체력측정 5종목을 재측정해 4주 전후 변화량을 종목별 그래프로 분석하고, 약점 종목 원인을 운동역학 "
                    "관점에서 해석한 개선 보고서로 정리한다' 수준의 밀도를 단계마다 유지한다. "
            ...
            "properties": {
                ...
                "content": {
                    ...
                    "description": (
                        ...
                        "steps=탐구 단계 3개+(각 단계 서로 다른 내용으로 120~180자 깊이 있게 — 분야 한 페이지 본문이 500자 이상 꽉 차게, 100자대 휑한 디벨롭은 반려) · "
