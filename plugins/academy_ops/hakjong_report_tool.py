...
def _grounding_errors(
    ...
    if subjects:
        ...
        if grade is not None and grade <= 3:
            ...
            # 고3 1학기는 창체(자율·자치·동아리)를 새로 설계·실행할 시간이 없다(편성 종료·9월 원서).
            # gap_plan은 교과 세특만 담고, 창체는 강점·면접에서 기존 활동으로 다룬다(사장님 2026-06-13). 고1·2만 창체 설계.
            if grade == 3 and isinstance(gap_subjects, list):
                _cre = [
                    r for r in gap_subjects
                    if isinstance(r, dict)
                    and any(k in str(r.get("field") or "") for k in ("창체", "동아리", "자율", "자치", "진로", "봉사", "활동"))
                ]
                if _cre:
                    errors.append(
                        "고3 1학기는 창체(자율·자치·동아리)를 새로 설계·실행할 시간이 없다(동아리 편성 종료, 9월 수시 원서) — "
                        "gap_plan에서 창체 분야를 빼고 교과 세특 설계만 담아라. 창체는 strategy_section의 강점·면접에서 "
                        "'기존 활동을 어떻게 면접·서류에 쓸지'로 다뤄라(신규 창체 설계 금지)."
                    )
            ...
            "properties": {
                ...
                "content": {
                    ...
                    "description": (
                        ...
                        "'어떤 분야를 어떻게 할지'까지 신규 설계가 핵심이다 / 고3 1학기는 창체를 새로 설계·실행할 시간이 없으니 "
                        "gap_plan에 창체(자율·자치·동아리·진로) 분야를 넣지 말고 교과 세특 설계만 담아라 — 창체는 강점·면접에서 "
                        "기존 활동을 어떻게 쓸지로만 다룬다(교과 세특도 1학기 실현 가능한 것만) / N수생은 기존 재해석 + 면접. "
