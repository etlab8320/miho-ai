        ...
        if grade is not None and grade <= 3:
            ...
            if not ok_gap:
                ...
                errors.append(
                    ...
                    "★과세특의 핵심은 학종 평가관이 보는 행동특성이다 — 이건 어느 학과·어느 학교나 중요하다. "
                    "한 분야 설계 전체(steps+expected_effect)에 다음 흐름이 자연히 드러나야 한다: "
                    "①탐구 동기(왜 이 탐구를 시작했나) ②과정·방법(그래서 어떻게 했나) "
                    "③문제해결(과정에서 막힌 점을 어떻게 개선했나) ④성찰(탐구하며 느낀 점) "
                    "⑤학습·성장(무엇을 배워 어디로 확장했나) ⑥공동체(친구와 어떻게 협업·공유·토론했나 = 공동체역량 근거). "
                    "'측정한다/분석한다'로만 끝내지 말고 동기→과정→개선→성찰→공동체가 한 분야 안에 흐르게 써라. 예: "
                    "'체력측정 5종목을 재측정해 4주 전후 변화량을 그래프로 분석하던 중 측정 오차를 발견해 측정 프로토콜을 "
                    "표준화했고(문제해결), 친구와 교차측정해 신뢰도를 높이며(공동체) 데이터 해석의 중요성을 배웠다(성찰·학습)' "
                    "수준의 서사 밀도. "
