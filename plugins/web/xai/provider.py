...
``{title, url, description, position}`` rows every other Miho web
provider produces.
...
prefers Miho-managed xAI Grok OAuth (via ``miho auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.miho/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``miho tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `miho auth` to sign in with "
