...
def _load_supermemory_config(miho_home: str) -> dict:
    config = _default_config()
    config_path = Path(miho_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, miho_home: str) -> None:
    config_path = Path(miho_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `miho memory setup`.
        # All other options are documented for $MIHO_HOME/supermemory.json
    ...
    def save_config(self, values, miho_home):
        ...
        _save_supermemory_config(sanitized, miho_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from miho_constants import get_miho_home
        self._miho_home = kwargs.get("miho_home") or str(get_miho_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._miho_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "miho", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "miho_memory", "target": target, "type": "explicit_memory"},
