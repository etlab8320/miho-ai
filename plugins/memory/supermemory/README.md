...
Config file: `$MIHO_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `miho` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `miho-{identity}` → `miho-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Miho profile:

```json
{
  "container_tag": "miho-{identity}"
}
```

For a profile named `coder`, this resolves to `miho-coder`. The default profile resolves to `miho-default`. Without `{identity}`, all profiles share the same container.
