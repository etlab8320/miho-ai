...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: miho)
...
Or via $MIHO_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Miho can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $MIHO_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_miho_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "miho": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "miho"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Miho config."""
    profile = config.get("profile", "miho")
    return str(profile or "miho")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Miho profile name (from agent_identity)
      {workspace} — Miho workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``miho-{user}`` with no user becomes ``miho``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Miho Agent and the User"
    ...
    def save_config(self, values, miho_home):
        """Write config to $MIHO_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(miho_home) / "hindsight"
    ...
    def post_setup(self, miho_home: str, config: dict) -> None:
        ...
        from miho_cli.memory_setup import _curses_select
        ...
        provider_config.setdefault("bank_id", "miho")
        ...
        self.save_config(provider_config, miho_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(miho_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(miho_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "miho"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: miho-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Miho Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "miho"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "miho"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "miho", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "miho")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Miho Agent and the User")
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Miho created/used that client on the shared async loop,
