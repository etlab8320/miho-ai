...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/miho>
...
```bash
miho honcho setup    # full interactive wizard (cloud or local)
miho memory setup    # generic picker, also works
```

Or manually:
```bash
miho config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.miho/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$MIHO_HOME/honcho.json` | Profile-local (isolated Miho instances) |
| 2 | `~/.miho/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Miho profile: `miho` (default) or `miho.<profile>`.
...
| 4 | `per-session` strategy | Miho session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`miho-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`miho`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-miho-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening miho in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Miho session ID (timestamp + hex). Every `miho` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Miho profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`miho`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Miho profile: `miho` (default) or `miho.<profile>` (e.g. `miho -p coder` → host key `miho.coder`).
...
| `MIHO_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `miho honcho setup` | Full interactive setup wizard |
| `miho honcho status` | Show resolved config for active profile |
| `miho honcho enable` / `disable` | Toggle Honcho for active profile |
| `miho honcho mode <mode>` | Change recall or observation mode |
| `miho honcho peer --user <name>` | Update user peer name |
| `miho honcho peer --ai <name>` | Update AI peer name |
| `miho honcho tokens --context <N>` | Set context token budget |
| `miho honcho tokens --dialectic <N>` | Set dialectic max chars |
| `miho honcho map <name>` | Map current directory to a session name |
| `miho honcho sync` | Create host blocks for all Miho profiles |
