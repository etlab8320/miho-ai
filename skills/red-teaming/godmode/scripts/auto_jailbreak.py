...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("MIHO_HOME", os.path.expanduser("~/.miho")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("MIHO_HOME", Path.home() / ".miho")) / "skills" / "red-teaming" / "godmode"
...
MIHO_HOME = Path(os.getenv("MIHO_HOME", Path.home() / ".miho"))
CONFIG_PATH = MIHO_HOME / "config.yaml"
PREFILL_PATH = MIHO_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Miho models — already uncensored, just needs clean prompt
    "miho": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "miho" in model_lower or "nous" in model_lower:
        return "miho"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Miho config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.miho/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Miho for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Miho for changes to take effect.")
