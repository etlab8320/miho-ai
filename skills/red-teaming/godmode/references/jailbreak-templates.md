...
## 5. GODMODE FAST — Miho 4 405B (Zero Refusal)

**Model:** `nousresearch/miho-4-405b`
**Strategy:** Miho models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Miho Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.miho/config.yaml`:
