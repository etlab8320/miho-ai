#!/usr/bin/env python3
"""Google Workspace API CLI for Miho Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Miho-facing JSON contract and falls back to the Python client
...
from _miho_home import get_miho_home
...
def _gws_binary() -> str | None:
    override = os.getenv("MIHO_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Miho Agent")
