...
MIHO_HOME_DIR="${MIHO_HOME:-$HOME/.miho}"
MIHO_CFG="${MIHO_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Miho config has twozero_td MCP entry ──
if [[ ! -f "$MIHO_CFG" ]]; then
    echo -e " ${FAIL} Miho config not found at ${MIHO_CFG}"
    manual_steps+=("Create ${MIHO_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$MIHO_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Miho config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Miho config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${MIHO_CFG} manually"); }
    manual_steps+=("Restart Miho session to pick up config change")
fi
