...
## 5. Miho Configuration

### Config location

`$MIHO_HOME/config.yaml` (defaults to `~/.miho/config.yaml` when `MIHO_HOME` is unset)
...
Restart the Miho session for changes to take effect. The MCP connection is
