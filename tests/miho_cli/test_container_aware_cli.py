...
def container_env(tmp_path, monkeypatch):
    """Set up a fake MIHO_HOME with .container-mode file."""
    miho_home = tmp_path / ".miho"
    miho_home.mkdir()
    monkeypatch.setenv("MIHO_HOME", str(miho_home))
    monkeypatch.delenv("MIHO_DEV", raising=False)

    container_mode = miho_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=miho-agent\n"
        "exec_user=miho\n"
        "miho_bin=/data/current-package/bin/miho\n"
    )
    return miho_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("miho_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "miho-agent"
    assert info["exec_user"] == "miho"
    assert info["miho_bin"] == "/data/current-package/bin/miho"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("miho_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_miho_dev(container_env, monkeypatch):
    """Returns None when MIHO_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("MIHO_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_miho_dev_zero(container_env, monkeypatch):
    """MIHO_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("MIHO_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        miho_home = Path(tmpdir) / ".miho"
        miho_home.mkdir()
        (miho_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("miho_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_miho_home": lambda: miho_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("MIHO_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=miho-custom\n"
        "exec_user=myuser\n"
        "miho_bin=/opt/miho/bin/miho\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "miho-custom"
    assert info["exec_user"] == "myuser"
    assert info["miho_bin"] == "/opt/miho/bin/miho"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from miho_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "miho"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "miho-agent" in cmd
    assert "/data/current-package/bin/miho" in cmd
    assert "chat" in cmd
