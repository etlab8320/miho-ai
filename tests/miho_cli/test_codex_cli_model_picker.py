...
Covers:
 - Normal path (tokens already in Miho auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Miho
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `miho auth openai-codex`. The old
...
@pytest.fixture()
def miho_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Miho auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(miho_auth_only_env):
    """openai-codex appears when tokens are already in Miho auth store."""
...
def test_codex_picker_uses_live_codex_catalog(miho_auth_only_env, tmp_path, monkeypatch):
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Miho
    auth store."""
    miho_home = tmp_path / ".miho"
    miho_home.mkdir()
