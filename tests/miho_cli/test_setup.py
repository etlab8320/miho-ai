...
from miho_cli.setup import setup_model_provider
...
def _stub_tts(monkeypatch):
    ...
    monkeypatch.setattr("miho_cli.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("miho_cli.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_delegates_to_select_provider_and_model(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("miho_cli.main.select_provider_and_model", fake_select)
...
def test_setup_gateway_in_container_shows_docker_guidance(monkeypatch, capsys):
    ...
    monkeypatch.setattr(miho_constants, "is_container", lambda: True)
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("miho_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("miho_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "miho_cli.main._model_flow_named_custom",
...
def test_select_provider_and_model_accepts_named_provider_from_providers_section(
    ...
    monkeypatch.setattr(
        "miho_cli.main._prompt_provider_choice",
...
def test_modal_setup_can_use_nous_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("miho_cli.setup.managed_nous_tools_enabled", lambda: True)
    monkeypatch.setenv("MIHO_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr("miho_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("miho_cli.setup.prompt", fake_prompt)
    monkeypatch.setattr("miho_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "miho_cli.setup.get_nous_subscription_features",
    ...
    from miho_cli.setup import setup_terminal_backend
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("miho_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("miho_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("miho_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "miho_cli.setup.get_nous_subscription_features",
        lambda config: type("Features", (), {"nous_auth_present": True})(),
