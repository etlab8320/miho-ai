"""Behavior tests for miho_cli.inventory.
...
from miho_cli.inventory import (
    ConfigContext,
...
def test_build_models_payload_does_not_call_provider_model_ids():
    ...
    """
    rows = [{"slug": "nous", "name": "Nous", "models": ["miho-4-405b"],
             "total_models": 1, "is_current": False, "is_user_defined": False,
             "source": "built-in"}]
    ctx = _empty_ctx()
    with _list_auth_returning(rows), \
         patch("miho_cli.models.provider_model_ids") as mock_pm:
...
def test_picker_hints_adds_warning_to_skeleton_rows():
    ...
    for row in skeleton_rows:
        ...
        assert (
            ...
            or row["warning"].startswith("run `miho model`")
        )
