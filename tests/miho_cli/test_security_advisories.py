"""Tests for miho_cli.security_advisories.
...
import miho_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect MIHO_HOME so banner cache and config writes are sandboxed."""
    home = tmp_path / ".miho"
    home.mkdir()
    (home / "cache").mkdir()
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full miho_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "miho_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "miho_cli.config.save_config",
