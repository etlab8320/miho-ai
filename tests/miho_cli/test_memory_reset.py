"""Tests for the `miho memory reset` CLI command.
...
- Profile-scoped reset (uses MIHO_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake MIHO_HOME with memory files."""
    miho_home = tmp_path / ".miho"
    memories = miho_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("MIHO_HOME", str(miho_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nMiho repo is at ~/.miho/miho-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `miho memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        miho_home = tmp_path / ".miho"
        (miho_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("MIHO_HOME", str(miho_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's MIHO_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        miho_home = tmp_path / ".miho"
        miho_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("MIHO_HOME", str(miho_home))

        # The memories dir won't exist; get_miho_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
