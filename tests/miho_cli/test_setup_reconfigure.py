...
- Bare `miho setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `miho setup --quick` runs the narrower "fill in missing items" flow.
- `miho setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("miho_cli.setup.ensure_miho_home", {}),
        ("miho_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("miho_cli.config.is_managed", {"return_value": False}),
        ("miho_cli.setup.load_config", {"return_value": {}}),
        ("miho_cli.setup.save_config", {}),
        ("miho_cli.setup.get_env_value", {"return_value": None}),
        ("miho_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("miho_cli.setup._print_setup_summary", {}),
        ("miho_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("miho_cli.setup.load_config", {"return_value": {}}),
        ("miho_cli.setup.save_config", {}),
        ("miho_cli.auth.get_active_provider", {"return_value": None}),
        ("miho_cli.setup.get_env_value", {"return_value": None}),
        ("miho_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `miho setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="miho_cli.setup.prompt_choice",
                quick="miho_cli.setup._run_quick_setup",
                model="miho_cli.setup.setup_model_provider",
                terminal="miho_cli.setup.setup_terminal_backend",
                agent="miho_cli.setup.setup_agent_settings",
                gateway="miho_cli.setup.setup_gateway",
                tools="miho_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`miho setup --reconfigure` behaves the same as bare `miho setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("miho_cli.setup.prompt_choice", {"return_value": 0}),
                first="miho_cli.setup._run_first_time_quick_setup",
            )
            from miho_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "miho_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["miho", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["miho", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["miho", "setup"])
        try:
            main()
        except SystemExit:
            pass
