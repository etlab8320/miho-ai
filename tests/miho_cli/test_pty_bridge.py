"""Unit tests for miho_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf miho-ok"])
        try:
            output = _read_until(bridge, b"miho-ok")
            assert b"miho-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$MIHO_PTY_TEST\""],
            env={**os.environ, "MIHO_PTY_TEST": "pty-env-works"},
        )
        try:
