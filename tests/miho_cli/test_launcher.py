"""Tests for the top-level `./miho` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./miho` should use `miho_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "miho"
    called = []

    fake_main_module = types.ModuleType("miho_cli.main")

    def fake_main():
        called.append("miho_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "miho_cli.main", fake_main_module)
