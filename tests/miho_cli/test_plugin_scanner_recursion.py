...
from miho_cli.plugins import PluginManager, PluginManifest
...
def _enable(miho_home: Path, name: str) -> None:
    """Append ``name`` to ``plugins.enabled`` in ``<miho_home>/config.yaml``."""
...
class TestCategoryNamespaceRecursion:
    def test_category_namespace_discovered(self, tmp_path, monkeypatch):
        ...
        import os
        miho_home = Path(os.environ["MIHO_HOME"])  # set by hermetic conftest fixture
        user_plugins = miho_home / "plugins"

        _write_plugin(user_plugins, ["image_gen", "openai"])
        _enable(miho_home, "image_gen/openai")
...
class TestKindField:
    def test_default_kind_is_standalone(self, tmp_path, monkeypatch):
        import os
        miho_home = Path(os.environ["MIHO_HOME"])  # set by hermetic conftest fixture
        _write_plugin(miho_home / "plugins", ["p1"])
        _enable(miho_home, "p1")
...
class TestBackendGate:
    ...
    def test_user_backend_loads_when_enabled(self, tmp_path, monkeypatch):
        ...
        _enable(miho_home, "image_gen/fancy")
    ...
    def test_exclusive_kind_skipped(self, tmp_path, monkeypatch):
        ...
        _enable(miho_home, "some-backend")
...
class TestRegisterImageGenProvider:
    def test_accepts_valid_provider(self, tmp_path, monkeypatch):
        ...
        _enable(miho_home, "my-img-plugin")
    ...
    def test_rejects_non_provider(self, tmp_path, monkeypatch, caplog):
        ...
        _enable(miho_home, "bad-img-plugin")
