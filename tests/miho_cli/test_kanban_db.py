"""Tests for the Kanban DB layer (miho_cli.kanban_db)."""
...
from miho_cli import kanban_db as kb


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated MIHO_HOME with an empty kanban DB."""
    home = tmp_path / ".miho"
    home.mkdir()
...
def test_connect_rejects_tls_record_in_sqlite_header(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("MIHO_HOME", str(home))
    monkeypatch.delenv("MIHO_KANBAN_DB", raising=False)
    monkeypatch.delenv("MIHO_KANBAN_HOME", raising=False)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("MIHO_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import miho_cli.kanban_db as _kb
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Miho profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="miho-task", assignee="daily")
...
def test_scratch_workspace_created_under_miho_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
# spawned with `miho -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's MIHO_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's MIHO_HOME."""

    def _set_home(self, monkeypatch, tmp_path, miho_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MIHO_HOME", str(miho_home))
        monkeypatch.delenv("MIHO_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_miho(
        self, tmp_path, monkeypatch
    ):
        # Standard install: MIHO_HOME == ~/.miho, no profile active.
        default_home = tmp_path / ".miho"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.miho/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.miho/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.miho/kanban.db.
        default_home = tmp_path / ".miho"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # MIHO_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `miho -p coder`).
        monkeypatch.setenv("MIHO_HOME", str(profile_home))
    ...
    def test_docker_custom_miho_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: MIHO_HOME points outside ~/.miho.
        # `get_default_miho_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".miho"`.
        custom_root = tmp_path / "opt" / "miho"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: MIHO_HOME=/opt/miho/profiles/coder;
        # `get_default_miho_root()` walks up to /opt/miho because
    ...
    def test_explicit_override_via_miho_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: MIHO_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".miho"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MIHO_HOME", str(profile_home))
        monkeypatch.setenv("MIHO_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MIHO_HOME", str(default_home))
        monkeypatch.setenv("MIHO_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # MIHO_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile MIHO_HOME and reads.
        monkeypatch.setenv("MIHO_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_miho_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # MIHO_KANBAN_DB pins the file path directly and beats both
        # MIHO_KANBAN_HOME and the `get_default_miho_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MIHO_HOME", str(default_home))
        monkeypatch.setenv("MIHO_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("MIHO_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows MIHO_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_miho_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # MIHO_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".miho"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("MIHO_HOME", str(default_home))
        monkeypatch.setenv("MIHO_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("MIHO_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows MIHO_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MIHO_HOME", str(default_home))
        monkeypatch.setenv("MIHO_KANBAN_DB", "   ")
        monkeypatch.setenv("MIHO_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject MIHO_KANBAN_DB
        # and MIHO_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites MIHO_HOME.
        default_home = tmp_path / ".miho"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["MIHO_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["MIHO_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["MIHO_KANBAN_TASK"] == "t_dispatch_env"
        assert env["MIHO_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see miho_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    with _patch("miho_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="miho_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'miho kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_miho_argv()
#
# Workers spawned by the dispatcher must use a `miho` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["miho", ...]`
...
def test_resolve_miho_argv_prefers_path_shim(monkeypatch):
    """When `miho` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import miho_cli.kanban_db as kb

    monkeypatch.delenv("MIHO_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/miho")
    argv = kb._resolve_miho_argv()
    assert argv == ["/usr/local/bin/miho"]


def test_resolve_miho_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import miho_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("MIHO_BIN", ".\\miho.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_miho_argv() == [os.path.abspath(".\\miho.exe")]


def test_resolve_miho_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "miho.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("MIHO_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_miho_argv() == [sys.executable, "-m", "miho_cli.main"]


def test_resolve_miho_argv_honors_miho_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like MIHO_BIN lets service managers pin the executable."""
    import shutil
    import miho_cli.kanban_db as kb

    shim = tmp_path / "bin" / "miho"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("MIHO_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_miho_argv() == [str(shim)]


def test_resolve_miho_argv_miho_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare MIHO_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import miho_cli.kanban_db as kb

    cwd_miho = tmp_path / "miho"
    cwd_miho.write_text("wrong\n", encoding="utf-8")
    cwd_miho.chmod(cwd_miho.stat().st_mode | stat.S_IXUSR)
    path_miho = tmp_path / "bin" / "miho"
    path_miho.parent.mkdir()
    path_miho.write_text("right\n", encoding="utf-8")
    path_miho.chmod(path_miho.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_miho.parent))
    monkeypatch.setenv("MIHO_BIN", "miho")

    assert kb._resolve_miho_argv() == [str(path_miho)]


def test_resolve_miho_argv_miho_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare MIHO_BIN does not accept current-directory shadow executables."""
    import sys
    import miho_cli.kanban_db as kb

    (tmp_path / "miho.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_miho_argv_miho_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved MIHO_BIN batch shim is not used as worker argv[0]."""
    import sys
    import miho_cli.kanban_db as kb
...
def test_resolve_miho_argv_miho_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved MIHO_BIN command names do not delegate cwd search to Popen."""
    import sys
    import miho_cli.kanban_db as kb
...
def test_resolve_miho_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m miho_cli.main`.

    Pins the correct module name (NOT `miho` — there is no top-level
    `miho` package). Regression for #23198: the original PR shipped
    `python -m miho` which fails with `No module named miho` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_miho_argv()
    assert argv == [sys.executable, "-m", "miho_cli.main"]


def test_resolve_miho_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `miho_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m miho_cli.main --version`
    ...
    assert "Miho Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="miho_cli.kanban_db"):
        with kb.connect() as conn:
