"""Tests for IRC gateway configuration via `miho setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Miho install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Miho install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#miho")
            monkeypatch.setenv("IRC_NICKNAME", "miho-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `miho setup gateway` experience with IRC present."""
