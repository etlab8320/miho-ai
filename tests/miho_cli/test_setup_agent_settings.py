...
from miho_cli.setup import setup_agent_settings
...
def test_setup_agent_settings_uses_displayed_max_iterations_value(tmp_path, monkeypatch, capsys):
    ...
    .env `MIHO_MAX_ITERATIONS` fallback was removed because it was
    ...
    monkeypatch.setattr("miho_cli.setup.prompt", lambda *args, **kwargs: next(prompt_answers))
    monkeypatch.setattr("miho_cli.setup.prompt_choice", lambda *args, **kwargs: 4)
    monkeypatch.setattr("miho_cli.setup.save_env_value", lambda *args, **kwargs: None)
    monkeypatch.setattr("miho_cli.setup.remove_env_value", lambda *args, **kwargs: None)
    monkeypatch.setattr("miho_cli.setup.save_config", lambda *args, **kwargs: None)
...
def test_setup_agent_settings_prefers_config_over_stale_env(tmp_path, monkeypatch, capsys):
    ...
    Regression guard for the bug where `.env MIHO_MAX_ITERATIONS=60`
    from an old `miho setup` run shadowed `agent.max_turns: 500` in
    ...
    monkeypatch.setattr(
        "miho_cli.setup.get_env_value",
        lambda key: "60" if key == "MIHO_MAX_ITERATIONS" else "",
    )
    monkeypatch.setattr("miho_cli.setup.prompt", lambda *args, **kwargs: next(prompt_answers))
    monkeypatch.setattr("miho_cli.setup.prompt_choice", lambda *args, **kwargs: 4)
    monkeypatch.setattr("miho_cli.setup.save_env_value", lambda *args, **kwargs: None)
    ...
    monkeypatch.setattr(
        "miho_cli.setup.remove_env_value",
    ...
    assert "MIHO_MAX_ITERATIONS" in removed_keys
