...
``miho_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``miho --help``, ``miho version``,
``miho logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``miho --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from miho_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["miho", "-w", "--tui"], None),
        (["miho", "version"], "version"),
        (["miho", "--tui", "chat"], "chat"),
        (["miho", "-w", "logs"], "logs"),
        (["miho", "chat", "hello world"], "chat"),
        (["miho", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["miho", "-m", "gpt5", "chat"], "chat"),
        (["miho", "--model", "gpt5", "chat", "hi"], "chat"),
        (["miho", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["miho", "-z", "hello world"], None),
        (["miho", "-z", "hello", "chat"], "chat"),
        (["miho", "--model=gpt5", "chat"], "chat"),     # inline form
        (["miho", "--", "chat"], "chat"),               # -- terminator
        (["miho", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["miho", "some-plugin-cmd"], "some-plugin-cmd"),
        (["miho", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["miho"],                          # bare → chat
        ["miho", "--help"],                # top-level help
        ["miho", "-h"],
        ["miho", "version"],               # known built-in
        ["miho", "logs"],
        ["miho", "gateway", "run"],
        ["miho", "--tui"],
        ["miho", "-w", "--tui"],
        ["miho", "chat", "hi"],
        ["miho", "help"],                  # accepted built-in-ish
        ["miho", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["miho", "meet", "join"],          # potential google_meet plugin
        ["miho", "honcho", "status"],      # potential memory plugin
        ["miho", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``miho help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"miho_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
