...
When ``MIHO_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.miho/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.miho            (has its own auth.json fixture)
    * Profile     -> tmp_path/.miho/profiles/coder   (active, MIHO_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (MIHO_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.miho as the "real home" — which is NOT equal
    # to the MIHO_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    miho_home = tmp_path / "classic"
    miho_home.mkdir()
    monkeypatch.setenv("MIHO_HOME", str(miho_home))

    _write(miho_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from miho_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: MIHO_HOME is set to a custom path that is NOT under
    # ~/.miho/profiles/ — get_default_miho_root() returns MIHO_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from miho_cli.auth import read_credential_pool, write_credential_pool
