...
async def test_notifier_does_not_call_init_db(kanban_home):
    ...
    with patch("gateway.run.asyncio.sleep", side_effect=_fast_sleep), \
         patch("miho_cli.kanban_db.init_db", side_effect=_spy_init_db):
        await asyncio.wait_for(
            runner._kanban_notifier_watcher(interval=1),
            timeout=10.0,
        )
...
async def test_notifier_uploads_artifacts_on_completion(kanban_home, tmp_path, monkeypatch):
    ...
    # via ``MIHO_MEDIA_ALLOW_DIRS``. Test fixtures live under ``tmp_path``,
    ...
    import os
    os.environ["MIHO_KANBAN_TASK"] = tid
    try:
        ...
        os.environ.pop("MIHO_KANBAN_TASK", None)
