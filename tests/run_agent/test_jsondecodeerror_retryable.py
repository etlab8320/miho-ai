...
as a local validation error by API-error retry handling.

`json.JSONDecodeError` inherits from `ValueError`. The API-error retry
classifier treats bare `ValueError` / `TypeError` as local programming bugs
and skips retry. Without an explicit carve-out, a transient provider hiccup
(malformed response body, truncated stream, routing-layer corruption) that
surfaces as a JSONDecodeError would bypass the retry path and fail the turn
immediately.
...
def _mirror_agent_predicate(err: BaseException) -> bool:
    """Exact shape of the API retry is_local_validation_error check.
...
class TestAgentLoopSourceStillHasCarveOut:
    ...
    def test_run_agent_excludes_jsondecodeerror_from_local_validation(self):
        import inspect
        from agent import conversation_error_retry

        src = inspect.getsource(conversation_error_retry)
        ...
        assert "JSONDecodeError" in src, (
            "agent/conversation_error_retry.py must carve out json.JSONDecodeError "
