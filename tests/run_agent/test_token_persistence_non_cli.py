...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    miho_state = ModuleType("miho_state")
    miho_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "miho_state", miho_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Miho"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Miho"
