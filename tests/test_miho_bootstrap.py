"""Tests for miho_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Miho entry point
(miho, miho-agent, miho-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Miho entry point imports miho_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported miho_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("miho_bootstrap", None)
    import miho_bootstrap  # noqa: WPS433
    return miho_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Miho CLI
...
class TestEntryPointsImportBootstrap:
    """Every Miho entry point must import miho_bootstrap as its
    ...
    # Entry points that invoke Miho as a process.  Each one must
    # import miho_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "miho_cli/main.py",   # miho CLI (console_script)
        "miho_cli.py",          # miho CLI alias (console_script)
        "run_agent.py",          # miho-agent (console_script)
        "acp_adapter/entry.py",  # miho-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import miho_bootstrap' and that
        ...
        half-finished ``miho update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``miho_bootstrap`` as a top-level module) leaves miho
        ...
        """
        # Resolve relative to the miho-agent repo root.  Tests live
        # at tests/test_miho_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # miho start even when miho_bootstrap hasn't been
        ...
        assert first_import_name == "miho_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'miho_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import miho_bootstrap' line to be the first import."
        )
