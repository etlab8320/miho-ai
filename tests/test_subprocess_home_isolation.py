...
See: https://github.com/NousResearch/miho-agent/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for miho_constants.get_subprocess_home()."""

    def test_returns_none_when_miho_home_unset(self, monkeypatch):
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MIHO_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("MIHO_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        from miho_constants import (
            get_miho_home,
            reset_miho_home_override,
            set_miho_home_override,
        )
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_miho_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_miho_home_override(profile)
        try:
            assert get_miho_home() == profile
            release.set()
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(miho_home / "home")
    ...
    def test_no_injection_when_miho_home_unset(self, monkeypatch):
        monkeypatch.delenv("MIHO_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["MIHO_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from miho_cli.profiles import _PROFILE_DIRS
