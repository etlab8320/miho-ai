...
from miho_cli.profiles import _get_default_miho_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "miho"
        assert config.workspace_id == "miho"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.miho/honcho.json
        monkeypatch.setenv("MIHO_HOME", str(tmp_path / "isolated"))
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"miho": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="miho-agent"
        ):
            result = config.resolve_session_name("/home/user/miho-agent/subdir")
        assert result == "miho-agent"
...
class TestResolveConfigPath:
    def test_prefers_miho_home_when_exists(self, tmp_path):
        miho_home = tmp_path / "miho"
        miho_home.mkdir()
        local_cfg = miho_home / "honcho.json"
        local_cfg.write_text('{"apiKey": "local"}')

        with patch.dict(os.environ, {"MIHO_HOME": str(miho_home)}):
            result = resolve_config_path()
        assert result == local_cfg

    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: MIHO_HOME points at ~/.miho/profiles/<name>, so
        # _get_default_miho_home() must resolve back to ~/.miho — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".miho"
        ...
        assert _get_default_miho_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_miho_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".miho".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".miho"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_default_returns_miho(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("MIHO_HONCHO_HOST", None)
            os.environ.pop("MIHO_HOME", None)
            assert resolve_active_host() == "miho"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"MIHO_HONCHO_HOST": "miho.coder"}):
            assert resolve_active_host() == "miho.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("MIHO_HONCHO_HOST", None)
            with patch("miho_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "miho.coder"

    def test_default_profile_returns_miho(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("MIHO_HONCHO_HOST", None)
            with patch("miho_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "miho"

    def test_custom_profile_returns_miho(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("MIHO_HONCHO_HOST", None)
            with patch("miho_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "miho"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("MIHO_HONCHO_HOST", None)
            # Temporarily remove miho_cli.profiles to simulate import failure
            saved = sys.modules.get("miho_cli.profiles")
            sys.modules["miho_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "miho"
            finally:
                if saved is not None:
                    sys.modules["miho_cli.profiles"] = saved
                else:
                    sys.modules.pop("miho_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="miho.coder")
        assert config.host == "miho.coder"
        assert config.workspace_id == "miho"  # shared workspace
        assert config.ai_peer == "miho.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="miho")
        assert config.host == "miho"
        assert config.workspace_id == "miho"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "miho": {"aiPeer": "miho", "peerName": "alice"},
                "miho.coder": {
                    "aiPeer": "miho.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="miho.coder", config_path=config_file,
        )
        assert config.host == "miho.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "miho.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="miho.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "miho.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"miho": {"enabled": True, "aiPeer": "miho"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"miho": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_miho_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="miho",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("miho_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("miho_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_miho_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="miho",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("miho_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
