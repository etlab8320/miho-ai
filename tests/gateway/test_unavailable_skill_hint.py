...
* ``disabled`` membership is checked by the declared name, because that
  is what :func:`miho_cli.skills_config.save_disabled_skills` stores.
"""
...
def tmp_skills(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolated skills dir + MIHO_HOME so the real user config is untouched."""
    home = tmp_path / ".miho"
    home.mkdir()
    (home / "skills").mkdir()
...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    # Config disables by declared name (matches what `miho skills config` writes).
    monkeypatch.setattr(
    ...
    assert "miho skills config" in msg
...
def test_optional_skill_uses_frontmatter_slug(
    ...
    monkeypatch.setattr(
        "miho_constants.get_optional_skills_dir",
