...
@pytest.mark.asyncio
async def test_streaming_delivery_routes_markdown_image_to_batch_sender():
    event = _event(thread_id="topic-1")
    adapter = SimpleNamespace(
        name="test",
        extract_media=BasePlatformAdapter.extract_media,
    ...
    await GatewayRunner._deliver_media_from_response(
        _fake_runner({"thread_id": "topic-1"}),
        "완성했어.\n![generated image](https://fal.media/generated.png)",
        event,
        adapter,
    )
    ...
    adapter.send_multiple_images.assert_awaited_once_with(
        chat_id="chat-1",
        images=[("https://fal.media/generated.png", "generated image")],
        metadata={"thread_id": "topic-1"},
    )
    adapter.send_document.assert_not_awaited()
