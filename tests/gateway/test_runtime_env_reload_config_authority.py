...
MIHO_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env MIHO_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_keeps_env_max_iterations_when_config_omits_key(
    ...
    miho_home = tmp_path / ".miho"
    miho_home.mkdir()
    (miho_home / "config.yaml").write_text(yaml.safe_dump({"agent": {}}), encoding="utf-8")
    (miho_home / ".env").write_text("MIHO_MAX_ITERATIONS=123\n", encoding="utf-8")

    monkeypatch.setattr(gateway_run, "_miho_home", miho_home)
    monkeypatch.delenv("MIHO_MAX_ITERATIONS", raising=False)

    gateway_run._reload_runtime_env_preserving_config_authority()

    assert os.environ["MIHO_MAX_ITERATIONS"] == "123"
