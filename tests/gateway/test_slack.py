...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.miho."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /miho, /btw, /stop,
        ...
        for expected in ("/miho", "/btw", "/stop", "/model", "/help"):
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@miho`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@miho"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@miho")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /miho subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /miho btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_miho_prefix_still_works(self, adapter):
        """Backward compat: /miho btw foo must still route to /btw foo.

        Old workspace manifests only declared /miho as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_miho_freeform_question(self, adapter):
        """/miho <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/miho",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_miho_slash_stashes_context(self, adapter):
        """Legacy /miho <subcommand> also stashes context."""
        command = {
            "command": "/miho",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_miho_question_does_not_stash_context(self, adapter):
        """Free-form /miho <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/miho",
