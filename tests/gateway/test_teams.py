...
class TestTeamsInteractiveSetup:
    def test_interactive_setup_persists_credentials(self, tmp_path, monkeypatch):
        ...
        from miho_cli.cli_output (not miho_cli.config) and persist
        credentials to .env without crashing.
        """
        miho_home = tmp_path / "miho"
        monkeypatch.setenv("MIHO_HOME", str(miho_home))

        import miho_cli.cli_output as cli_output_mod
        ...
        env_text = (miho_home / ".env").read_text(encoding="utf-8")
...
class TestTeamsMessageHandling:
    ...
    async def test_bot_mention_stripped_from_text(self):
        ...
        activity = self._make_activity(
            text="<at>Miho</at> what is the weather?",
            from_id="user-id",
        )
        await adapter._on_message(self._make_ctx(activity))
