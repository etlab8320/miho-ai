...
def gateway_home(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("MIHO_PREFILL_MESSAGES_FILE", raising=False)
    monkeypatch.delenv("MIHO_EPHEMERAL_SYSTEM_PROMPT", raising=False)
    monkeypatch.delenv("MIHO_GATEWAY_BUSY_INPUT_MODE", raising=False)
    monkeypatch.delenv("MIHO_RESTART_DRAIN_TIMEOUT", raising=False)
    monkeypatch.delenv("MIHO_BACKGROUND_NOTIFICATIONS", raising=False)
    return tmp_path
