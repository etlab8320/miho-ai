...
substrings like 'foo@miho_bot.example'.
...
def _make_adapter():
    ...
    adapter._bot = SimpleNamespace(id=999, username="miho_bot")
    return adapter


def _mention_entity(text, mention="@miho_bot"):
...
class TestRealMentionsAreDetected:
    ...
    def test_mention_mid_sentence(self):
        adapter = _make_adapter()
        text = "hey @miho_bot, can you help?"
        msg = _message(text=text, entities=[_mention_entity(text)])
        assert adapter._message_mentions_bot(msg) is True

    def test_mention_at_end_of_message(self):
        adapter = _make_adapter()
        text = "thanks for looking @miho_bot"
        msg = _message(text=text, entities=[_mention_entity(text)])
        assert adapter._message_mentions_bot(msg) is True

    def test_mention_in_caption(self):
        adapter = _make_adapter()
        caption = "photo for @miho_bot"
...
class TestSubstringFalsePositivesAreRejected:
    ...
    def test_email_like_substring(self):
        """bug #12545 exact repro: 'foo@miho_bot.example'."""
        adapter = _make_adapter()
        msg = _message(text="email me at foo@miho_bot.example")
        assert adapter._message_mentions_bot(msg) is False

    def test_hostname_substring(self):
        adapter = _make_adapter()
        msg = _message(text="contact user@miho_bot.domain.com")
        assert adapter._message_mentions_bot(msg) is False

    def test_superstring_username(self):
        """`@miho_botx` is a different username; Telegram would emit a mention
        entity for `@miho_botx`, not `@miho_bot`."""
        adapter = _make_adapter()
        msg = _message(text="@miho_botx hello")
        assert adapter._message_mentions_bot(msg) is False

    def test_underscore_suffix_substring(self):
        adapter = _make_adapter()
        msg = _message(text="see @miho_bot_admin for help")
        assert adapter._message_mentions_bot(msg) is False

    def test_substring_inside_url_without_entity(self):
        """@handle inside a URL produces a URL entity, not a MENTION entity."""
        adapter = _make_adapter()
        msg = _message(text="see https://example.com/@miho_bot for details")
        assert adapter._message_mentions_bot(msg) is False

    def test_substring_inside_code_block_without_entity(self):
        """Telegram doesn't emit mention entities inside code/pre entities."""
        adapter = _make_adapter()
        msg = _message(text="use the string `@miho_bot` in config")
        assert adapter._message_mentions_bot(msg) is False
    ...
    def test_email_substring_in_caption(self):
        adapter = _make_adapter()
        msg = _message(caption="foo@miho_bot.example")
        assert adapter._message_mentions_bot(msg) is False
...
class TestCaseInsensitivity:
    ...
    def test_uppercase_mention(self):
        adapter = _make_adapter()
        text = "hi @MIHO_BOT"
        msg = _message(text=text, entities=[_mention_entity(text, mention="@MIHO_BOT")])
        assert adapter._message_mentions_bot(msg) is True
