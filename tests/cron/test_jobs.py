...
class TestComputeNextRun:
    ...
    def test_once_recent_past_within_grace_returns_time(self, monkeypatch):
        ...
        monkeypatch.setattr("cron.jobs._miho_now", lambda: now)
...
class TestAdvanceNextRun:
    ...
    def test_advances_interval_job(self, tmp_cron_dir):
        ...
        updated = get_job(job["id"])
        from cron.jobs import _ensure_aware, _miho_now
        new_next_dt = _ensure_aware(datetime.fromisoformat(updated["next_run_at"]))
        assert new_next_dt > _miho_now(), "next_run_at should be in the future after advance"
    ...
    def test_already_future_stays_future(self, tmp_cron_dir):
        ...
        from cron.jobs import _ensure_aware, _miho_now
        new_next_dt = _ensure_aware(datetime.fromisoformat(updated["next_run_at"]))
        assert new_next_dt > _miho_now(), "next_run_at should remain in the future"
