...
- Path resolution (absolute, relative to MIHO_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp MIHO_HOME."""
    miho_home = tmp_path / ".miho"
    miho_home.mkdir()
    (miho_home / "cron").mkdir()
    (miho_home / "cron" / "output").mkdir()
    (miho_home / "scripts").mkdir()
    monkeypatch.setenv("MIHO_HOME", str(miho_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "MIHO_DIR", miho_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", miho_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", miho_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", miho_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.miho/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("MIHO_SESSION_PLATFORM") is None
        assert os.environ.get("MIHO_SESSION_CHAT_ID") is None
        assert os.environ.get("MIHO_SESSION_CHAT_NAME") is None
