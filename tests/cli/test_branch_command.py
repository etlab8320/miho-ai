...
def session_db(tmp_path):
    ...
    os.environ["MIHO_HOME"] = str(tmp_path / ".miho")
    os.makedirs(tmp_path / ".miho", exist_ok=True)
    from miho_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".miho" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal MihoCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_creates_new_session(self, cli_instance, session_db):
        ...
        MihoCLI._handle_branch_command(cli_instance, "/branch")
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        MihoCLI._handle_branch_command(cli_instance, "/branch refactor approach")
    ...
    def test_branch_rotates_miho_session_id_env_and_context(self, cli_instance, session_db):
        """Branching must update process-local session-id readers too."""
        from cli import MihoCLI
        from gateway.session_context import _UNSET, _VAR_MAP, get_session_env

        old_session_id = cli_instance.session_id
        os.environ["MIHO_SESSION_ID"] = old_session_id
        _VAR_MAP["MIHO_SESSION_ID"].set(old_session_id)

        try:
            MihoCLI._handle_branch_command(cli_instance, "/branch")

            assert cli_instance.session_id != old_session_id
            assert os.environ["MIHO_SESSION_ID"] == cli_instance.session_id
            assert get_session_env("MIHO_SESSION_ID") == cli_instance.session_id
        finally:
