...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("miho/miho-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned miho/* and pr-* branches."""

    def test_prunes_orphaned_miho_branch(self, git_repo):
        """miho/miho-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "miho/miho-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "miho/miho-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "miho/miho-deadbeef" in result.stdout
        ...
        assert "miho/miho-deadbeef" in orphaned
        ...
        assert "miho/miho-deadbeef" not in result.stdout
