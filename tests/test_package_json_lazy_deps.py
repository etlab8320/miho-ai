...
The root ``package.json`` is installed by ``miho update`` on every user,
...
* ``@askjo/camofox-browser`` is NOT eager. It is an explicit opt-in
  ...
  ``miho tools`` → Browser Automation → Camofox, and only used at
  runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
  Firefox-fork binary, which silently blocked ``miho update`` for
...
guidance in the ``miho-agent-dev`` skill before changing the
expectations.
"""
...
def test_camofox_is_not_in_root_dependencies() -> None:
    ...
    assert "@askjo/camofox-browser" not in deps, (
        ...
        "Camofox post_setup handler in miho_cli/tools_config.py so it "
        "only installs when the user explicitly selects Camofox via "
        "`miho tools` → Browser Automation → Camofox."
    )
...
def test_agent_browser_stays_eager() -> None:
    ...
    assert "agent-browser" in deps, (
        ...
        "after `miho setup` / `miho update` without an explicit "
        "post_setup step."
    )
