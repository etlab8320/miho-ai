"""Tests for acp_adapter.server — MihoACPAgent ACP server."""
...
from acp_adapter.server import MihoACPAgent, MIHO_VERSION
...
def agent(mock_manager):
    """MihoACPAgent backed by a mock session manager."""
    return MihoACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "miho-agent"
        assert resp.agent_info.version == MIHO_VERSION
        ...
        assert payloads == [
            {
                "args": ["--setup"],
                "description": (
                    "Open Miho' interactive model/provider setup in a terminal. "
                    "Use this when Miho has not been configured on this machine yet."
                ),
                "id": TERMINAL_SETUP_AUTH_METHOD_ID,
                "name": "Configure Miho provider",
                "type": "terminal",
            }
        ]
...
class TestSessionOps:
    ...
    async def test_new_session_returns_model_state(self):
        ...
        acp_agent = MihoACPAgent(session_manager=manager)

        with patch(
            "miho_cli.models.curated_models_for_provider",
    ...
    async def test_load_session_replays_persisted_history_to_client(self, agent):
        ...
        state.history = [
            ...
            {"role": "assistant", "content": "MihoACPAgent._ADVERTISED_COMMANDS controls them."},
            {
                "role": "assistant",
                "content": "",
                "tool_calls": [
        ...
        assert replay_calls[1].kwargs["update"].content.text.startswith("MihoACPAgent")
...
class TestSessionConfiguration:
    ...
    async def test_set_session_model_accepts_provider_prefixed_choice(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("miho_cli.config.load_config", lambda: {
        ...
        monkeypatch.setattr(
            "miho_cli.models.parse_model_input",
...
class TestPrompt:
    ...
    @pytest.mark.asyncio
    async def test_prompt_propagates_miho_session_id_env(self, agent, monkeypatch):
        """ACP must propagate the originating session id to the agent loop
        via ``MIHO_SESSION_ID`` so tools that want to stamp side-effects
        ...
        def mock_run(user_message, conversation_history=None, task_id=None, **kwargs):
            ...
            captured["env"] = os.environ.get("MIHO_SESSION_ID")
        ...
        assert captured["env"] == new_resp.session_id, (
            "MIHO_SESSION_ID must be set to the originating ACP session id "
        ...
        assert os.environ.get("MIHO_SESSION_ID") is None, (
            "MIHO_SESSION_ID must be restored after the agent call so "
            "a re-used executor thread doesn't leak the id into the next "
            "session's tools"
        )

    @pytest.mark.asyncio
    async def test_prompt_restores_prior_miho_session_id(self, agent, monkeypatch):
        """If the env already had MIHO_SESSION_ID set (e.g. nested
        agent loops), the prior value must be restored after the inner
        prompt completes — not popped, not left at the inner id."""
        monkeypatch.setenv("MIHO_SESSION_ID", "outer-sess")
        ...
        def mock_run(*args, **kwargs):
            captured["inner"] = os.environ.get("MIHO_SESSION_ID")
            return {"final_response": "ok", "messages": []}
        ...
        assert os.environ.get("MIHO_SESSION_ID") == "outer-sess"
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["miho-acp"]
    ...
    async def test_refreshes_agent_tool_surface(self, agent, mock_manager):
        ...
        mock_defs.assert_called_once_with(
            enabled_toolsets=["miho-acp", "mcp-srv"],
            disabled_toolsets=None,
            quiet_mode=True,
        )
        assert state.agent.enabled_toolsets == ["miho-acp", "mcp-srv"]
