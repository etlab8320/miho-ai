"""Shared fixtures for the miho-agent test suite.
...
2. **Isolated MIHO_HOME.** MIHO_HOME points to a per-test tempdir so
   code reading ``~/.miho/*`` via ``get_miho_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".miho"`` instead of the canonical
   ``get_miho_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No MIHO_SESSION_* inheritance** — the agent's current gateway
...
# MIHO_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_MIHO_BEHAVIORAL_VARS = frozenset({
    "MIHO_YOLO_MODE",
    "MIHO_INTERACTIVE",
    "MIHO_QUIET",
    "MIHO_TOOL_PROGRESS",
    ...
    "MIHO_SESSION_CHAT_ID",
    "MIHO_SESSION_CHAT_NAME",
    "MIHO_SESSION_THREAD_ID",
    "MIHO_SESSION_SOURCE",
    "MIHO_SESSION_KEY",
    "MIHO_GATEWAY_SESSION",
    "MIHO_PLATFORM",
    "MIHO_MODEL",
    "MIHO_INFERENCE_MODEL",
    "MIHO_INFERENCE_PROVIDER",
    "MIHO_TUI_PROVIDER",
    "MIHO_MANAGED",
    "MIHO_DEV",
    "MIHO_CONTAINER",
    ...
    "MIHO_TIMEZONE",
    "MIHO_REDACT_SECRETS",
    "MIHO_BACKGROUND_NOTIFICATIONS",
    "MIHO_EXEC_ASK",
    "MIHO_HOME_MODE",
    "MIHO_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.miho/kanban.db instead of the per-test MIHO_HOME.
    "MIHO_KANBAN_DB",
    "MIHO_KANBAN_BOARD",
    "MIHO_KANBAN_HOME",
    "MIHO_KANBAN_WORKSPACES_ROOT",
    "MIHO_KANBAN_LOGS_ROOT",
    "MIHO_KANBAN_TASK",
    "MIHO_KANBAN_WORKSPACE",
    "MIHO_KANBAN_RUN_ID",
    "MIHO_KANBAN_CLAIM_LOCK",
    "MIHO_KANBAN_DISPATCH_IN_GATEWAY",
    "MIHO_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and MIHO_HOME to per-test tempdirs so code that
    reads ``~/.miho/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral MIHO_* vars that could change test semantics.
    for name in _MIHO_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect MIHO_HOME to a per-test tempdir. Code that reads
    #    ``~/.miho/*`` via ``get_miho_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.miho/*`` via
    #    ``Path.home() / ".miho"`` instead of ``get_miho_home()``
    #    is a bug to fix at the callsite.
    fake_miho_home = tmp_path / "miho_test"
    fake_miho_home.mkdir()
    (fake_miho_home / "sessions").mkdir()
    (fake_miho_home / "cron").mkdir()
    (fake_miho_home / "memories").mkdir()
    (fake_miho_home / "skills").mkdir()
    monkeypatch.setenv("MIHO_HOME", str(fake_miho_home))
    ...
    #    ~/.miho/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import miho_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_miho_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal miho config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``miho-gateway`` process
...
# correlated with a ``tests/miho_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> miho-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart miho-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting miho/python patterns are also blocked.
    """
    ...
    def _matches_miho_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _MIHO_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_miho_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch miho-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m miho_cli.main".
                if (
                    "miho" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live miho-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting miho/python could hit the live gateway. "
