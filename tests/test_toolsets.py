...
class TestToolsetConsistency:
    ...
    def test_miho_platforms_share_core_tools(self):
        """All miho-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on miho-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["miho-cli", "miho-telegram", "miho-discord", "miho-whatsapp", "miho-slack", "miho-signal", "miho-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_miho_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("miho-whatsapp")

    def test_miho_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("miho-api-server")
