...
class TestOpenVikingSummaryUriNormalization:
    def test_normalize_summary_uri_maps_pseudo_files_to_parent_directory(self):
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/miho/.overview.md") == "viking://user/miho"
        ...
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/miho/memories/profile.md") == "viking://user/miho/memories/profile.md"
...
class TestOpenVikingRead:
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    (("uri", "viking://user/miho"),),
                ): {"result": {"content": "overview text"}},
            }
        )

        result = json.loads(provider._tool_read({"uri": "viking://user/miho/.overview.md", "level": "overview"}))

        assert result["uri"] == "viking://user/miho/.overview.md"
        assert result["resolved_uri"] == "viking://user/miho"
        ...
        provider._client = FakeVikingClient(
            {
                (
                    "/api/v1/content/read",
                    (("uri", "viking://user/miho/memories/profile.md"),),
                ): {"result": "full text"},
            }
        )

        result = json.loads(provider._tool_read({"uri": "viking://user/miho/memories/profile.md", "level": "full"}))

        assert result["uri"] == "viking://user/miho/memories/profile.md"
        assert result["resolved_uri"] == "viking://user/miho/memories/profile.md"
    ...
    def test_overview_file_uri_routes_straight_to_content_read_via_stat_probe(self):
        ...
        provider = OpenVikingMemoryProvider()
        file_uri = "viking://user/miho/memories/entities/mem_abc.md"
        provider._client = FakeVikingClient(
            {
                (
                    "/api/v1/fs/stat",
    ...
    def test_overview_dir_uri_skips_stat_when_pseudo_summary(self):
        ...
        assert provider._client.calls == [
            ("/api/v1/content/overview", {"uri": "viking://user/miho"}),
        ]
    ...
    def test_overview_directory_uri_uses_stat_probe_then_overview(self):
        ...
        provider = OpenVikingMemoryProvider()
        dir_uri = "viking://user/miho/memories"
        provider._client = FakeVikingClient(
            {
                (
                    "/api/v1/fs/stat",
    ...
    def test_summary_uri_error_does_not_fallback_and_raises(self):
        ...
        try:
            provider._tool_read({"uri": "viking://user/miho/.overview.md", "level": "overview"})
...
class TestOpenVikingBrowse:
    def test_list_browse_unwraps_and_normalizes_entry_shapes(self):
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    "result": {
                        "entries": [
                            {"name": "memories", "uri": "viking://user/miho/memories", "type": "dir"},
                            {"rel_path": "profile.md", "uri": "viking://user/miho/memories/profile.md", "isDir": False, "abstract": "Profile"},
                        ]
                    }
                },
            }
        ...
        result = json.loads(provider._tool_browse({"action": "list", "path": "viking://user/miho"}))

        assert result["path"] == "viking://user/miho"
        assert result["entries"] == [
            {"name": "memories", "uri": "viking://user/miho/memories", "type": "dir", "abstract": ""},
            {"name": "profile.md", "uri": "viking://user/miho/memories/profile.md", "type": "file", "abstract": "Profile"},
        ]
        assert provider._client.calls == [(
            "/api/v1/fs/ls",
            {"uri": "viking://user/miho"},
