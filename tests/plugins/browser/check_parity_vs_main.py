...
Run from the PR worktree:
    ...
    cd ~/.miho/miho-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.miho/miho-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.miho/miho-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a miho-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a miho-agent checkout"
)
...
# Isolated MIHO_HOME for the config write.
