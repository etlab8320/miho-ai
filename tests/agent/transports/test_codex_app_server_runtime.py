...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Miho passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("MIHO_HOME", "/users/alice/.miho/profiles/backend-worker")
        monkeypatch.setenv("MIHO_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "MIHO_KANBAN_DB",
            "/users/alice/.miho/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.miho/kanban/boards/smoke"]'
            in cmd
        )
