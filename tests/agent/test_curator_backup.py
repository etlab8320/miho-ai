...
def backup_env(monkeypatch, tmp_path):
    """Isolate MIHO_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".miho"
    home.mkdir()
    (home / "skills").mkdir()
    ...
    # Reload so get_miho_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd miho_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under MIHO_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level MIHO_DIR picks up the tmp HOME."""
    import miho_constants
    importlib.reload(miho_constants)
...
def test_snapshot_without_cron_jobs_file_still_succeeds(backup_env):
    ...
    cb = backup_env["cb"]
    _write_skill(backup_env["skills"], "alpha")
    # Deliberately do not create ~/.miho/cron/jobs.json
