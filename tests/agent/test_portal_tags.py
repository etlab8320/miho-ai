...
def test_miho_client_tag_includes_current_version():
    """The client tag must reflect miho_cli.__version__ verbatim."""
    from miho_cli import __version__
    from agent.portal_tags import miho_client_tag

    assert miho_client_tag() == f"client=miho-client-v{__version__}"


def test_miho_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import miho_client_tag

    tag = miho_client_tag()
    assert tag.startswith("client=miho-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import miho_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=miho-agent" in tags
    assert miho_client_tag() in tags
    assert len(tags) == 2
