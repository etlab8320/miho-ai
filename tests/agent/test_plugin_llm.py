...
""",
        ...
        from miho_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        miho_home = tmp_path / ".miho"
        miho_home.mkdir()
        (miho_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("MIHO_HOME", str(miho_home))
        from miho_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from miho_cli.plugins import PluginContext, PluginManifest, PluginManager
