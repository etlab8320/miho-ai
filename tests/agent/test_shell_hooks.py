...
class TestCallbackSubprocess:
    ...
    def test_block_translation_end_to_end(self, tmp_path):
        ...
        must translate it to the canonical Miho block shape so that
    ...
    def test_block_aggregation_through_plugin_manager(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("MIHO_ACCEPT_HOOKS", "1")
...
class TestAllowlistConcurrency:
    ...
    def test_save_allowlist_failure_logs_actionable_warning(
        ...
        re-prompt consequence so "miho keeps asking" is debuggable."""
        import logging
        monkeypatch.setenv("MIHO_HOME", str(tmp_path / "home"))
        monkeypatch.setattr(
    ...
    def test_command_script_path_resolution(self):
        ...
        ``miho hooks doctor``'s executability check and silently
