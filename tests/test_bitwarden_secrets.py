...
binary download are exercised manually by `miho secrets bitwarden
...
def miho_home(tmp_path, monkeypatch):
    """Point Miho at an isolated home directory."""
    home = tmp_path / ".miho"
    home.mkdir()
    monkeypatch.setenv("MIHO_HOME", str(home))
    # Some modules cache get_miho_home; clear if needed.
    import miho_constants
    if hasattr(miho_constants, "_MIHO_HOME_CACHE"):
        miho_constants._MIHO_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(miho_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(miho_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(miho_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from miho_cli.env_loader import _apply_external_secret_sources
