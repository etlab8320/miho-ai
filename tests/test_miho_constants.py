"""Tests for miho_constants module."""
...
class TestGetDefaultMihoRoot:
    """Tests for get_default_miho_root() — Docker/custom deployment awareness."""

    def test_no_miho_home_returns_native(self, tmp_path, monkeypatch):
        """When MIHO_HOME is not set, returns ~/.miho."""
        monkeypatch.delenv("MIHO_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_miho_root() == tmp_path / ".miho"

    def test_miho_home_is_native(self, tmp_path, monkeypatch):
        """When MIHO_HOME = ~/.miho, returns ~/.miho."""
        native = tmp_path / ".miho"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MIHO_HOME", str(native))
        assert get_default_miho_root() == native

    def test_miho_home_is_profile(self, tmp_path, monkeypatch):
        """When MIHO_HOME is a profile under ~/.miho, returns ~/.miho."""
        native = tmp_path / ".miho"
    ...
    def test_miho_home_is_docker(self, tmp_path, monkeypatch):
        """When MIHO_HOME points outside ~/.miho (Docker), returns MIHO_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MIHO_HOME", str(docker_home))
        assert get_default_miho_root() == docker_home

    def test_miho_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any MIHO_HOME outside ~/.miho is treated as the root."""
        custom = tmp_path / "my-miho-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MIHO_HOME", str(custom))
        assert get_default_miho_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (MIHO_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MIHO_HOME", str(profile))
        assert get_default_miho_root() == docker_root
    ...
    def test_miho_runtime_defaults_to_miho_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MIHO_RUNTIME", "1")
        monkeypatch.delenv("MIHO_HOME", raising=False)
        monkeypatch.delenv("MIHO_HOME", raising=False)
    ...
    def test_miho_home_wins_over_miho_home(self, tmp_path, monkeypatch):
        """Miho must not accidentally reuse a user's Miho install."""
        ...
        assert get_miho_home() == miho_home
        assert get_default_miho_root() == miho_home
        assert display_miho_home() == "~/miho-data"
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(miho_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(miho_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".miho"
