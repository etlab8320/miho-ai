...
class TestMihoConstantsFallback:
    """Tests for _miho_home.py fallback when miho_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_miho_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _miho_home.py with miho_constants blocked."""
        monkeypatch.setitem(sys.modules, "miho_constants", None)
        spec = importlib.util.spec_from_file_location("_miho_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_miho_home_env_var(self, monkeypatch, tmp_path):
        """When miho_constants is missing, MIHO_HOME comes from env var."""
        monkeypatch.setenv("MIHO_HOME", str(tmp_path / "custom-miho"))
        module = self._load_helper(monkeypatch)
        assert module.get_miho_home() == tmp_path / "custom-miho"

    def test_fallback_defaults_to_dot_miho(self, monkeypatch):
        """When miho_constants is missing and MIHO_HOME unset, default to ~/.miho."""
        monkeypatch.delenv("MIHO_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_miho_home() == Path.home() / ".miho"

    def test_fallback_ignores_empty_miho_home(self, monkeypatch):
        """Empty/whitespace MIHO_HOME is treated as unset."""
        monkeypatch.setenv("MIHO_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_miho_home() == Path.home() / ".miho"

    def test_fallback_display_miho_home_shortens_path(self, monkeypatch):
        """Fallback display_miho_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("MIHO_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_miho_home() == "~/.miho"

    def test_fallback_display_miho_home_profile_path(self, monkeypatch):
        """Fallback display_miho_home() handles profile paths under ~/."""
        monkeypatch.setenv("MIHO_HOME", str(Path.home() / ".miho/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_miho_home() == "~/.miho/profiles/coder"

    def test_fallback_display_miho_home_custom_path(self, monkeypatch):
        """Fallback display_miho_home() returns full path for non-home locations."""
        monkeypatch.setenv("MIHO_HOME", "/opt/miho-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_miho_home() == "/opt/miho-custom"

    def test_delegates_to_miho_constants_when_available(self):
        """When miho_constants IS importable, _miho_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_miho_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import miho_constants
        assert module.get_miho_home is miho_constants.get_miho_home
        assert module.display_miho_home is miho_constants.display_miho_home
