...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        miho_home.mkdir()
        (miho_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(miho_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.miho/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        miho_home = tmp_path / ".miho"
        miho_home.mkdir()
        (miho_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        miho_home = tmp_path / ".miho"
        miho_home.mkdir()
        (miho_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        miho_home = tmp_path / ".miho"
        miho_home.mkdir()
        (miho_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.miho/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        miho_home = tmp_path / ".miho"
        (miho_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"MIHO_HOME": str(miho_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.miho")

        assert mounts[0]["container_path"] == "/home/user/.miho/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.miho/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.miho/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape MIHO_HOME."""
        monkeypatch.setenv("MIHO_HOME", str(tmp_path / ".miho"))
        (tmp_path / ".miho").mkdir()

        # Create a sensitive file one level above miho_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside miho_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside MIHO_HOME must still be registered."""
        miho_home = tmp_path / ".miho"
        miho_home.mkdir()
        monkeypatch.setenv("MIHO_HOME", str(miho_home))
        (miho_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_miho_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of MIHO_HOME must be allowed."""
        miho_home = tmp_path / ".miho"
        miho_home.mkdir()
        subdir = miho_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside MIHO_HOME pointing outside must be rejected."""
        miho_home = tmp_path / ".miho"
        miho_home.mkdir()
        monkeypatch.setenv("MIHO_HOME", str(miho_home))

        # Create a sensitive file outside miho_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside miho_home pointing outside
        symlink = miho_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes MIHO_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, miho_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape MIHO_HOME."""
        miho_home = tmp_path / ".miho"
        miho_home.mkdir()
        monkeypatch.setenv("MIHO_HOME", str(miho_home))
        ...
        sensitive.write_text("{}")
        self._write_config(miho_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(miho_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside MIHO_HOME via config must still mount."""
        miho_home = tmp_path / ".miho"
        miho_home.mkdir()
        monkeypatch.setenv("MIHO_HOME", str(miho_home))

        (miho_home / "oauth.json").write_text("{}")
        self._write_config(miho_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        miho_home = tmp_path / ".miho"
        miho_home.mkdir()
        (miho_home / "cache" / "documents").mkdir(parents=True)
        (miho_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("MIHO_HOME", str(miho_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.miho/cache/documents" in paths
        assert "/root/.miho/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.miho/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        miho_home = tmp_path / ".miho"
        miho_home.mkdir()
        # Use legacy dir name — get_miho_dir prefers old if it exists
        (miho_home / "document_cache").mkdir()
        (miho_home / "image_cache").mkdir()
        monkeypatch.setenv("MIHO_HOME", str(miho_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(miho_home / "document_cache") in host_paths
        assert str(miho_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.miho/cache/documents" in container_paths
        assert "/root/.miho/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        miho_home = tmp_path / ".miho"
        doc_dir = miho_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        miho_home = tmp_path / ".miho"
        ss_dir = miho_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.miho/cache/screenshots/session_abc/screen1.png"
