...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._miho_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_miho_bin(self):
        """After _INSTALL_FAILED, manual install in MIHO_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        miho_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(miho_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(miho_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._miho_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == miho_bin
            assert _tirith_mod._resolved_path == miho_bin
            mock_clear.assert_called_once()
...
class TestMihoHomeIsolation:
    def test_miho_bin_dir_respects_miho_home(self):
        """_miho_bin_dir must use MIHO_HOME, not hardcoded ~/.miho."""
        from tools.tirith_security import _miho_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"MIHO_HOME": tmpdir}):
            result = _miho_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_miho_home(self):
        """_failure_marker_path must use MIHO_HOME, not hardcoded ~/.miho."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"MIHO_HOME": "/custom/miho"}):
            result = _failure_marker_path()
        assert result == "/custom/miho/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets MIHO_HOME; verify it's active."""
        miho_home = os.getenv("MIHO_HOME")
        assert miho_home is not None, "MIHO_HOME should be set by conftest"
        assert "miho_test" in miho_home, "Should point to test temp dir"

    def test_get_miho_home_fallback(self):
        """Without MIHO_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_miho_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove MIHO_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".miho")
            result = _get_miho_home()
