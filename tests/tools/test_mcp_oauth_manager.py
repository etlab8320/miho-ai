...
def test_miho_provider_subclass_exists():
    """MihoMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _MIHO_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _MIHO_PROVIDER_CLS is not None
    assert issubclass(_MIHO_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_miho_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns MihoMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _MIHO_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("MIHO_HOME", str(tmp_path))
    ...
    assert _MIHO_PROVIDER_CLS is not None
    assert isinstance(provider, _MIHO_PROVIDER_CLS)
    assert provider._miho_server_name == "srv"
