...
def _make_execute_only_env(forward_env=None):
    ...
    env._snapshot_path = "/tmp/miho-snap-test123.sh"
    env._cwd_file = "/tmp/miho-cwd-test123.txt"
    env._cwd_marker = "__MIHO_CWD_test123__"
...
def test_init_env_args_uses_miho_dotenv_for_allowlisted_env(monkeypatch):
    """_build_init_env_args picks up forwarded env vars from .env file at init time."""
    # Use a var that is NOT in _MIHO_PROVIDER_ENV_BLOCKLIST (GITHUB_TOKEN
    # is in the copilot provider's api_key_env_vars and gets stripped).
    env = _make_execute_only_env(["DATABASE_URL"])

    monkeypatch.delenv("DATABASE_URL", raising=False)
    monkeypatch.setattr(docker_env, "_load_miho_env_vars", lambda: {"DATABASE_URL": "value_from_dotenv"})
...
def test_init_env_args_prefers_shell_env_over_miho_dotenv(monkeypatch):
...
def test_forward_env_overrides_docker_env_in_init_args(monkeypatch):
    ...
    monkeypatch.setattr(docker_env, "_load_miho_env_vars", lambda: {})
...
def test_security_args_include_setuid_setgid_for_gosu_drop(monkeypatch):
    ...
    ``error: failed switching to 'miho': operation not permitted``
