...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because miho uses an internal scheduler, not system crontab.
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("MIHO_INTERACTIVE", raising=False)
        monkeypatch.delenv("MIHO_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("MIHO_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("MIHO_INTERACTIVE", false_like_value)
        monkeypatch.delenv("MIHO_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("MIHO_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["MIHO_INTERACTIVE", "MIHO_GATEWAY_SESSION", "MIHO_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("MIHO_INTERACTIVE", "MIHO_GATEWAY_SESSION", "MIHO_EXEC_ASK"):
