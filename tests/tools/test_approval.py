...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("miho_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("miho_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"MIHO_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_miho_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.miho/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_custom_miho_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $MIHO_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_miho_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$MIHO_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_miho_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $MIHO_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.miho/miho-agent && source venv/bin/activate && python -m miho_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m miho_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m miho_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "miho_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart miho-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_pkill_miho_detected(self):
        """pkill targeting miho/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_miho_detected(self):
        cmd = "killall miho"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep miho) bypasses the pkill/killall name-matching
    ...
    def test_pkill_miho_still_detected(self):
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("MIHO_GATEWAY_SESSION", "MIHO_YOLO_MODE",
                      "MIHO_SESSION_KEY", "MIHO_INTERACTIVE")
        }
        os.environ.pop("MIHO_YOLO_MODE", None)
        os.environ.pop("MIHO_INTERACTIVE", None)
        os.environ["MIHO_GATEWAY_SESSION"] = "1"
        os.environ["MIHO_SESSION_KEY"] = self.SESSION_KEY
