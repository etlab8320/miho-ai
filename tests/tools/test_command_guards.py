...
def _clean_state():
    ...
    saved = {}
    for k in ("MIHO_INTERACTIVE", "MIHO_GATEWAY_SESSION", "MIHO_EXEC_ASK", "MIHO_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["MIHO_INTERACTIVE"] = "1"
        session_key = os.getenv("MIHO_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No MIHO_INTERACTIVE or MIHO_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
