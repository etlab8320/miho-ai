...
def _skill_dir(tmp_path):
    ...
    only the temp directory — not the real ~/.miho/skills/."""
...
class TestSecurityScanGate:
    ...
    def test_guard_flag_reads_config_default_false(self):
        ...
        with patch("miho_cli.config.load_config", return_value={"skills": {}}):
    ...
    def test_guard_flag_reads_config_when_set(self):
        ...
        with patch("miho_cli.config.load_config",
...
class TestExternalSkillMutations:
    ...
    caused agents to create duplicate copies in ~/.miho/skills/ as a
    workaround.
    """
...
# come up. The user unpins via `miho curator unpin <name>` to delete.
# ---------------------------------------------------------------------------
...
class TestPinnedGuard:
    ...
    def test_delete_refuses_pinned(self, tmp_path):
        ...
        assert "miho curator unpin my-skill" in result["error"]
        # Skill still exists
        assert (tmp_path / "my-skill" / "SKILL.md").exists()
