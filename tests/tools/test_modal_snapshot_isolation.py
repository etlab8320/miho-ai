...
def _restore_tool_modules():
    original_miho_home = os.environ.get("MIHO_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_miho_home is None:
            os.environ.pop("MIHO_HOME", None)
        else:
            os.environ["MIHO_HOME"] = original_miho_home
        _reset_modules(("tools", "miho_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    miho_cli = types.ModuleType("miho_cli")
    miho_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["miho_cli"] = miho_cli
    miho_home = tmp_path / "miho-home"
    os.environ["MIHO_HOME"] = str(miho_home)
    sys.modules["miho_cli.config"] = types.SimpleNamespace(
        get_miho_home=lambda: miho_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="miho-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": miho_home / "modal_snapshots.json",
