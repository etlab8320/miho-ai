...
class TestGlobalAllowPrivateUrls:
    ...
    def test_default_is_false(self, monkeypatch):
        ...
        monkeypatch.delenv("MIHO_ALLOW_PRIVATE_URLS", raising=False)
        with patch("miho_cli.config.read_raw_config", side_effect=Exception("no config")):
            assert _global_allow_private_urls() is False

    def test_env_var_true(self, monkeypatch):
        """MIHO_ALLOW_PRIVATE_URLS=true enables the toggle."""
        monkeypatch.setenv("MIHO_ALLOW_PRIVATE_URLS", "true")
        assert _global_allow_private_urls() is True

    def test_env_var_1(self, monkeypatch):
        """MIHO_ALLOW_PRIVATE_URLS=1 enables the toggle."""
        monkeypatch.setenv("MIHO_ALLOW_PRIVATE_URLS", "1")
        assert _global_allow_private_urls() is True

    def test_env_var_yes(self, monkeypatch):
        """MIHO_ALLOW_PRIVATE_URLS=yes enables the toggle."""
        monkeypatch.setenv("MIHO_ALLOW_PRIVATE_URLS", "yes")
        assert _global_allow_private_urls() is True

    def test_env_var_false(self, monkeypatch):
        """MIHO_ALLOW_PRIVATE_URLS=false keeps it disabled."""
        monkeypatch.setenv("MIHO_ALLOW_PRIVATE_URLS", "false")
        assert _global_allow_private_urls() is False
    ...
    def test_config_security_section(self, monkeypatch):
        ...
        with patch("miho_cli.config.read_raw_config", return_value=cfg):
            assert _global_allow_private_urls() is True
