...
class TestWriteDenyExactPaths:
    ...
    def test_miho_env(self):
        # ``.env`` under the active MIHO_HOME (profile-aware, not just
        # ``~/.miho``) must be write-denied. The hermetic test conftest
        # points MIHO_HOME at a tempdir — resolve via get_miho_home()
        # to match the denylist.
        from miho_constants import get_miho_home
        path = str(get_miho_home() / ".env")
        assert _is_write_denied(path) is True

    def test_miho_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.miho/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "miho_root"
        ...
        # Sanity check: MIHO_HOME does point to the profile dir, not the root.
        from miho_constants import get_miho_home, get_default_miho_root
        assert get_miho_home() == profile_home
        assert get_default_miho_root() == root
...
class TestWriteAllowed:
    ...
    def test_miho_config_not_env(self):
        path = os.path.join(str(Path.home()), ".miho", "config.yaml")
