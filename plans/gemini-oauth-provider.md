...
## Token Lifecycle
- Store at `~/.miho/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `miho_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `miho_cli/models.py` — Add Gemini model catalog
4. `miho_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `miho_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `miho_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
- OR: accept user-provided client_id via MIHO_GEMINI_CLIENT_ID env var
...
- miho-agent Copilot OAuth: `miho_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
