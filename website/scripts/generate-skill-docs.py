...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/miho-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    miho_meta = (fm.get("metadata") or {}).get("miho") or {}
    tags = miho_meta.get("tags") or []
    related = miho_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `miho skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Miho Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Miho ships with a large built-in skill library copied into `~/.miho/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Miho also syncs bundled skills on `miho update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.miho/skills/` tree, it is still shipped with Miho; restore it with `miho skills reset <name> --restore`.",
        "",
