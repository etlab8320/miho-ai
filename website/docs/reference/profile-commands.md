...
This page covers all commands related to [Miho profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `miho profile` without a subcommand shows help.
...
```bash
miho profile use <name>
```

Sets `<name>` as the active profile. All subsequent `miho` commands (without `-p`) will use this profile.
...
miho profile use default
```

## `miho profile create`

```bash
miho profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `miho profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-skills` marker into the profile so future `miho update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. |
...
miho profile create mybot

# Clone config only from current profile
miho profile create work --clone

# Clone everything from current profile
miho profile create backup --clone-all

# Clone config from a specific profile
miho profile create work2 --clone --clone-from work
```

## `miho profile describe`

```bash
miho profile describe [<name>] [options]
```
...
miho profile describe researcher

# Set it explicitly
miho profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
miho profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
miho profile describe --all --auto
```

## `miho profile delete`

```bash
miho profile delete <name> [options]
```
...
```bash
miho profile delete mybot
miho profile delete mybot --yes
```
...
```bash
miho profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Miho home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ miho profile show work
Profile: work
Path:    ~/.miho/profiles/work
...
```bash
miho profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Miho installation.
...
```bash
miho profile alias work
# Creates/updates ~/.local/bin/work

miho profile alias work --name mywork
# Creates ~/.local/bin/mywork

miho profile alias work --remove
...
```bash
miho profile rename <old-name> <new-name>
```
...
```bash
miho profile rename mybot assistant
# ~/.miho/profiles/mybot → ~/.miho/profiles/assistant
...
```bash
miho profile export <name> [options]
```
...
```bash
miho profile export work
# Creates work.tar.gz in the current directory

miho profile export work -o ./work-2026-03-29.tar.gz
```

## `miho profile import`

```bash
miho profile import <archive> [options]
```
...
```bash
miho profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

miho profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`miho profile export` / `import` are still the right commands for
...
### `miho profile install`

```bash
miho profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `miho -p telemetry`). |
...
miho profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
miho profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
miho profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
miho profile install ./telemetry/
```

### `miho profile update`

```bash
miho profile update <name> [--force-config] [--yes]
```
...
```bash
miho profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Miho version, author, env var requirements, the source URL/path, and
...
`miho profile list` also shows the distribution name and version in a
`Distribution` column, and `miho profile show <name>` / `delete <name>`
...
miho profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
miho profile install https://github.com/your-org/internal-assistant.git
```
...
`miho_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Miho version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Miho can clone from.
3. Tell recipients to run `miho profile install <your-repo-url>`.
...
```bash
miho -p <name> <command> [options]
miho --profile <name> <command> [options]
```

Global flag to run any Miho command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
miho -p work chat -q "Check the server status"
miho --profile dev gateway start
miho -p personal skills list
miho -p work config edit
```

## `miho completion`

```bash
miho completion <shell>
```
...
- `miho profile <TAB>` — subcommands (list, use, create, etc.)
- `miho profile use <TAB>` — profile names
- `miho -p <TAB>` — profile names
