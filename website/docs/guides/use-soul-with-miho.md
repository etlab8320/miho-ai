---
sidebar_position: 7
title: "Use SOUL.md with Miho"
description: "How to use SOUL.md to shape Miho Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Miho

`SOUL.md` is the **primary identity** for your Miho instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Miho to feel like the same assistant every time you talk to it — or if you want to replace the Miho persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Miho should be
- what Miho should avoid stylistically
- how Miho should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Miho is and how Miho speaks
...
Miho now uses only the global SOUL file for the current instance:

```text
~/.miho/SOUL.md
```

If you run Miho with a custom home directory, it becomes:
...
Miho automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Miho does not overwrite it
- if the file exists but is empty, Miho adds nothing from it to the prompt

## How Miho uses it

When Miho starts a session, it reads `SOUL.md` from `MIHO_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Miho feels.
...
Miho already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Miho should sound.

# Avoid
What Miho should not do.

# Defaults
How Miho should behave when ambiguity appears.
```
...
Then restart Miho or start a new session.
...
### I edited SOUL.md but Miho still sounds the same

Check:
- you edited `~/.miho/SOUL.md` or `$MIHO_HOME/SOUL.md`
...
### Miho is ignoring parts of my SOUL.md
