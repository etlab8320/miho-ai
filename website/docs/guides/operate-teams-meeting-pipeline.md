...
```bash
miho teams-pipeline token-health
miho teams-pipeline token-health --force-refresh
```
...
```bash
miho teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
miho teams-pipeline maintain-subscriptions
miho teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Miho cron (recommended if you already run the Miho gateway)

Miho ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.miho/scripts/`. First create the script:

```bash
mkdir -p ~/.miho/scripts
cat > ~/.miho/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec miho teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.miho/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
miho cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/miho-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Miho Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=miho
EnvironmentFile=/etc/miho/env
ExecStart=/usr/local/bin/miho teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/miho-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Miho Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now miho-teams-pipeline-maintain.timer
systemctl list-timers miho-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/miho teams-pipeline maintain-subscriptions >> /var/log/miho/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.miho/.env` at the top of a wrapper script that crontab calls.
...
```bash
miho teams-pipeline subscriptions   # should show expirationDateTime advanced
miho teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
miho teams-pipeline list
miho teams-pipeline list --status failed
miho teams-pipeline show <job-id>
```

### Replay a stored job

```bash
miho teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
miho teams-pipeline fetch --meeting-id <meeting-id>
miho teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `miho teams-pipeline maintain-subscriptions --dry-run`
- inspect `miho teams-pipeline list --status failed`
...
- run `miho teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `miho teams-pipeline run`
...
- [ ] `miho teams-pipeline validate` returns an OK snapshot
- [ ] `miho teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Miho cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
