---
sidebar_position: 6
title: "Use MCP with Miho"
description: "A practical guide to connecting MCP servers to Miho Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Miho

This guide shows how to actually use MCP with Miho Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Miho tool
- you want Miho to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Miho to internal APIs, databases, or company systems without modifying Miho core

Do not use MCP when:
- a built-in Miho tool already solves the job well
...
- Miho remains the agent
- MCP servers contribute tools
- Miho discovers those tools at startup or reload time
...
If you installed Miho with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Miho banner/status should show MCP integration when configured
- ask Miho what tools it has available
...
## WSL2: bridge Miho in WSL to Windows Chrome

This is the practical setup when:

- Miho runs inside WSL2
...
In this setup, Miho does **not** connect to Chrome directly. Instead:

- Miho runs in WSL
- Miho starts a local stdio MCP server
...
```text
Miho (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Miho stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Miho core browser transport
...
```bash
miho mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
miho mcp test chrome-devtools-win
```

Then start a fresh Miho session or run:
...
Once loaded, Miho can use the MCP-prefixed browser tools directly. For example:
...
If Miho runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Miho from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Miho from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Miho:
...
So Miho will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Miho to reason over a bounded workspace.
...
Now Miho can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Miho core.
...
Because Miho now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
