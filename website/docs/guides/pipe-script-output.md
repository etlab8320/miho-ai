...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `miho send`."
---

# Pipe Script Output to Messaging Platforms

`miho send` is a small, scriptable CLI that pushes a message to any
messaging platform Miho is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | miho send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `miho
...
miho send --to telegram "deploy finished"

# Pipe in stdout from anything
echo "RAM 92%" | miho send --to telegram:-1001234567890

# Send a file
miho send --to discord:#ops --file /tmp/report.md

# Attach a subject/header line
miho send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
miho send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
miho send --list

# Filter by platform
miho send --list telegram
```
...
Any platform Miho ships adapters for works as a target:
...
`miho send` resolves the message body in this order:

1. **Positional argument** — `miho send --to telegram "hi"`
2. **`--file PATH`** — `miho send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | miho send --to telegram`

When stdin is a TTY (no pipe), Miho does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  miho send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `miho send` reuses your Miho config, the same script works on
any host where Miho is installed — no need to export bot tokens into
...
`miho send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  miho send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | miho send \
...
```bash
./train.py --epochs 200 && \
  miho send --to telegram "training done" || \
  miho send --to telegram "training failed (exit $?)"
```
...
miho send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(miho send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `miho send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `miho send` calls
the platform's REST endpoint directly using credentials from
`~/.miho/.env` and `~/.miho/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `miho gateway start` and retry.
...
The listing is built from `~/.miho/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`miho gateway
...
| Approach | Multi-platform | Reuses Miho creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `miho send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`miho send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `miho send`.
...
- [Gateway Internals](/docs/developer-guide/gateway-internals) —
  the delivery router that `miho send` shares with cron delivery.
