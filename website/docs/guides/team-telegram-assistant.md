...
This tutorial walks you through setting up a Telegram bot powered by Miho Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Miho Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.miho/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Miho itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Miho Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_miho_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Miho Agent. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u miho-gateway -f
...
miho gateway start
miho gateway stop
tail -f ~/.miho/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `miho gateway install` to update the plist.
:::
...
```bash
miho gateway stop && miho gateway start
```
...
miho pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.miho/.env`:
...
Control how much detail the bot shows when using tools. In `~/.miho/config.yaml`:
...
Customize how the bot communicates by editing `~/.miho/SOUL.md`:

For a full guide, see [Use SOUL.md with Miho](/docs/guides/use-soul-with-miho).
...
```bash
# From the CLI
miho cron list          # View all scheduled jobs
miho cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u miho-gateway` (Linux) or `~/.miho/logs/gateway.log` (macOS) |
| Cron job output | `~/.miho/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.miho/cron/jobs.json` |
| Pairing data | `~/.miho/pairing/` |
| Session history | `~/.miho/sessions/` |
