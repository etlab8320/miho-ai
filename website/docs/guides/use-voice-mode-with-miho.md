---
sidebar_position: 8
title: "Use Voice Mode with Miho"
description: "A practical guide to setting up and using Miho voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Miho
...
- you want Miho sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Miho.
...
## Step 1: make sure normal Miho works first
...
Miho supports both local and cloud speech stacks.
...
### If you use `miho setup`

If you choose NeuTTS in the setup wizard, Miho checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Miho transcribes and responds
...
- asking Miho to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Miho loop.

## Tuning CLI behavior

### Silence threshold

If Miho starts/stops too aggressively, tune:
...
Miho stays a normal chat bot, but can speak replies.
...
- you want Miho to function like a portable research or ops assistant
...
Miho joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Miho detects speech boundaries
- transcripts are posted in the associated text channel
- Miho responds in text and audio
...
1. get text Miho working
2. install `miho-agent[voice]`
