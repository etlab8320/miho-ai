...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Miho Agent — no API key required"
---

# MiniMax OAuth

Miho Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Miho automatically refreshes your session.
...
# → Miho opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.miho/auth.json` and are refreshed automatically before each session.
...
```bash
miho auth add minimax-cn --type oauth   # if OAuth is supported on your CN account
# or simpler:
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.miho/.env
```
...
```bash
miho auth add minimax-oauth --no-browser
```

Miho will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Miho implements a PKCE device-code flow against the MiniMax OAuth endpoints:

1. Miho generates a PKCE verifier / challenge pair and a random state value.
...
4. Miho polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.miho/auth.json` under the `minimax-oauth` key.
...
```bash
miho config set model MiniMax-M2.7
miho config set provider minimax-oauth
```

## Configuration Reference

After login, `~/.miho/config.yaml` will contain entries similar to:
...
```bash
miho --provider minimax-oauth    # canonical
miho --provider minimax-portal   # alias
miho --provider minimax-global   # alias
miho --provider minimax_oauth    # alias (underscore form)
```
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `miho setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:

```bash
miho --provider minimax-oauth
```
...
Miho refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Miho marks the session as requiring re-login.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Miho marks the refresh token as dead and quarantines it locally so it doesn't keep replaying the doomed exchange. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `miho auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Miho raises a timeout error.

**Fix:** re-run `miho auth add minimax-oauth` (or `miho model`). The flow starts fresh.

### State mismatch (possible CSRF)

Miho detected that the `state` value returned by the authorization server does not match what it sent.
...
If `miho` cannot open a browser window, use `--no-browser`:

```bash
miho auth add minimax-oauth --no-browser
```

Miho prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `miho model` and select MiniMax (OAuth), or run `miho auth add minimax-oauth`.
...
```bash
miho auth remove minimax-oauth
```
...
- [miho doctor](../reference/cli-commands.md)
