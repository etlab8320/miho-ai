...
This page covers setting up inference providers for Miho Agent — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use Miho.

## Inference Providers

You need at least one way to connect to an LLM. Use `miho model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Nous Portal** | `miho model` (OAuth, subscription-based) |
| **OpenAI Codex** | `miho model` (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `miho model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `miho model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `miho model` (Claude Max + extra usage credits via OAuth; also supports Anthropic API key or manual setup-token — see note below) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.miho/.env` |
| **NovitaAI** | `NOVITA_API_KEY` in `~/.miho/.env` (provider: `novita`, 200+ models, Model API, Agent Sandbox, GPU Cloud) |
| **AI Gateway** | `AI_GATEWAY_API_KEY` in `~/.miho/.env` (provider: `ai-gateway`) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.miho/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.miho/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `~/.miho/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `~/.miho/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **GMI Cloud** | `GMI_API_KEY` in `~/.miho/.env` (provider: `gmi`; aliases: `gmi-cloud`, `gmicloud`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.miho/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.miho/.env` (provider: `minimax-cn`) |
| **xAI (Grok) — Responses API** | `XAI_API_KEY` in `~/.miho/.env` (provider: `xai`) |
| **xAI Grok OAuth (SuperGrok)** | `miho model` → "xAI Grok OAuth (SuperGrok Subscription)" — browser login, no API key. See [guide](../guides/xai-grok-oauth.md) |
| **Qwen Cloud (Alibaba DashScope)** | `DASHSCOPE_API_KEY` in `~/.miho/.env` (provider: `alibaba`) |
...
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.miho/.env` (provider: `kilocode`) |
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `~/.miho/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **Tencent TokenHub** | `TOKENHUB_API_KEY` in `~/.miho/.env` (provider: `tencent-tokenhub`, aliases: `tencent`, `tokenhub`, `tencentmaas`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.miho/.env` (provider: `opencode-zen`) |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.miho/.env` (provider: `opencode-go`) |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `~/.miho/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `~/.miho/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.miho/.env` (provider: `gemini`) |
| **Google Gemini (OAuth)** | `miho model` → "Google Gemini (OAuth)" (provider: `google-gemini-cli`, free tier supported, browser PKCE login) |
| **LM Studio** | `miho model` → "LM Studio" (provider: `lmstudio`, optional `LM_API_KEY`) |
| **Custom Endpoint** | `miho model` → choose "Custom endpoint" (saved in `config.yaml`) |
...
# → done — Miho auto-provisions your free tier on first request
```

Miho ships Google's **public** `gemini-cli` desktop OAuth client by default —
...
- Tokens stored at `~/.miho/auth/google_oauth.json` (chmod 0600, atomic write, cross-process `fcntl` lock)
- Automatic refresh 60 s before expiry
- Headless environments (SSH, `MIHO_HEADLESS=1`) → paste-mode fallback
...
- Responses translated back to OpenAI shape so the rest of Miho works unchanged
...
| Workspace / Standard / Enterprise account | Set `MIHO_GEMINI_PROJECT_ID` or `GOOGLE_CLOUD_PROJECT` to your GCP project ID |
| VPC-SC-protected org | Miho detects `SECURITY_POLICY_VIOLATED` and forces `standard-tier` automatically |
...
experience, use your own API key via the `gemini` provider instead. Miho shows
...
```bash
MIHO_GEMINI_CLIENT_ID=your-client.apps.googleusercontent.com
MIHO_GEMINI_CLIENT_SECRET=...   # optional for Desktop clients
```
...
:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). Miho stores the resulting credentials in its own auth store under `~/.miho/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.

If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Miho marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `miho auth add codex-oauth` (or `miho model` → OpenAI Codex) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::

:::warning
Even when using Nous Portal, Codex, or a custom endpoint, some tools (vision, web summarization, MoA) use a separate "auxiliary" model. By default (`auxiliary.*.provider: "auto"`), Miho routes these tasks to your **main chat model** — the same model you picked in `miho model`. You can override each task individually to route it to a cheaper/faster model (e.g. Gemini Flash on OpenRouter) — see [Auxiliary Models](/docs/user-guide/configuration#auxiliary-models).
:::

:::tip Nous Tool Gateway
Paid Nous Portal subscribers also get access to the **[Tool Gateway](/docs/user-guide/features/tool-gateway)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. On a fresh install, `miho setup --portal` logs you in, sets Nous as your provider, and turns the gateway on in one command. Existing users can enable it from `miho model` or per-tool from `miho tools`. Inspect routing at any time with `miho portal status`.
:::

### Two Commands for Model Management

Miho has **two** model commands that serve different purposes:

| Command | Where to run | What it does |
|---------|-------------|--------------|
| **`miho model`** | Your terminal (outside any session) | Full setup wizard — add providers, run OAuth, enter API keys, configure endpoints |
| **`/model`** | Inside a Miho chat session | Quick switch between **already-configured** providers and models |

If you're trying to switch to a provider you haven't set up yet (e.g. you only have OpenRouter configured and want to use Anthropic), you need `miho model`, not `/model`. Exit your session first (`Ctrl+C` or `/quit`), run `miho model`, complete the provider setup, then start a new session.

### Nous Portal

[Nous Portal](https://portal.nousresearch.com) is Nous Research's unified subscription gateway and **the recommended way to use Miho Agent**. One OAuth login replaces the juggling act of separate accounts, API keys, and billing relationships across every model lab, search API, image generator, and browser provider you'd otherwise need to wire up by hand.
...
**The Nous Tool Gateway.** The same subscription unlocks the [Tool Gateway](/docs/user-guide/features/tool-gateway), which routes Miho Agent's tool calls through Nous-managed infrastructure:
...
- **Modal sandbox** (optional add-on) — serverless terminal sandboxes for code execution, available through the same subscription via `miho setup terminal`.
...
**No credentials in your dotfiles.** Because everything routes through one OAuth-authenticated portal session, you don't accumulate a `.env` file with a dozen long-lived API keys. The refresh token at `~/.miho/auth.json` is the only credential on disk, and Miho mints short-lived JWTs from it per request (see [Token handling](#token-handling) below).

#### A note on Miho 4

Nous Research's own **Miho 4** family (Miho-4-70B, Miho-4-405B) is available through the portal at heavily discounted rates. These are **frontier hybrid-reasoning chat models** — strong at math, science, instruction following, schema adherence, roleplay, and long-form writing.

They are **not recommended for use inside Miho Agent**, however. Miho 4 is tuned for chat and reasoning, not the rapid-fire tool-calling loop the agent relies on. Use them for [Nous Chat](https://chat.nousresearch.com), for research workflows, or via the [subscription proxy](/docs/user-guide/features/subscription-proxy) from other tooling — but for agent work, pick a frontier agentic model from the catalog instead:
...
The portal's own [model info page](https://portal.nousresearch.com/info) carries the same warning, so this isn't a Miho-side opinion — it's the official guidance from Nous Research.
...
Opens your browser to portal.nousresearch.com, runs the OAuth flow, sets Nous as your inference provider in `config.yaml`, and turns on the Tool Gateway in the same step. After this you're ready to `miho chat`. If you don't have a subscription yet, grab one at [portal.nousresearch.com/manage-subscription](https://portal.nousresearch.com/manage-subscription) before running the command.
...
This adds Nous Portal as a provider you can switch to with `/model` or `miho model` at any time. Your existing providers (OpenRouter, Anthropic, OpenAI, etc.) stay configured — you can switch back whenever you want. To enable just specific gateway tools (e.g. web search but not image generation), use `miho tools` and pick **Nous Subscription** per backend.

Either path stores a long-lived refresh token at `~/.miho/auth.json`. The token is also written into a shared token store, so signing in on one Miho [profile](/docs/user-guide/profiles) carries over to all of them automatically — no need to repeat the OAuth flow per profile.
...
```bash
miho portal status     # login status, subscription info, gateway routing summary
miho model             # switch the active model within the portal catalog
miho auth add nous     # re-authenticate after a token revocation
```
...
Miho mints a short-lived JWT from your stored Nous refresh token on each inference call rather than reusing a long-lived API key. The token lifecycle is fully automatic — refresh, mint, retry on transient 401 — and you never see it.

If the portal invalidates the refresh token (password change, manual revoke, session expiry), the invalid refresh token is quarantined locally so Miho stops replaying it and you don't see a stream of identical 401s. The next call surfaces a clear "re-authentication required" message. Run `miho auth add nous` to log in again; the quarantine clears on the next successful login.
...
When you authenticate via `miho model` → Anthropic OAuth (or via `miho auth add anthropic --type oauth`), Miho routes as Claude Code against your Anthropic account. **It only works if you're on a Claude Max plan and have purchased extra usage credits.** The base Max plan allowance (the usage included in Claude Code by default) is not consumed by Miho — only the extra/overage credits you've added on top are. Claude Pro subscribers cannot use this path.
...
miho chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `miho model`
# Miho will use Claude Code's credential store directly when available
miho model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
miho chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
miho chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `miho model`, Miho prefers Claude Code's own credential store over copying the token into `~/.miho/.env`. That keeps refreshable Claude credentials refreshable.
...
Miho supports GitHub Copilot as a first-class provider with two modes:
...
```bash
miho chat --provider copilot --model gpt-5.4
```
...
If no token is found, `miho model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `miho model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `miho model` to authenticate via OAuth instead.
:::

:::info Copilot auth behavior in Miho
Miho sends a supported GitHub token (`gho_*`, `github_pat_*`, or `ghu_*`) directly to `api.githubcopilot.com` and includes Copilot-specific headers (`Editor-Version`, `Copilot-Integration-Id`, `Openai-Intent`, `x-initiator`).

On HTTP 401, Miho now performs a one-shot credential recovery before fallback:
...
Some older community proxies use `api.github.com/copilot_internal/v2/token` exchange flows. That endpoint can be unavailable for some account types (returns 404). Miho therefore keeps direct-token auth as the primary path and relies on runtime credential refresh + retry for robustness.
:::
...
```bash
miho chat --provider copilot-acp --model copilot-acp
...
| `MIHO_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `MIHO_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
```bash
# NovitaAI Model API
miho chat --provider novita --model moonshotai/kimi-k2.5
# Requires: NOVITA_API_KEY in ~/.miho/.env

# z.ai / ZhipuAI GLM
miho chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.miho/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
miho chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.miho/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
miho chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ~/.miho/.env

# MiniMax (global endpoint)
miho chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.miho/.env

# MiniMax (China endpoint)
miho chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.miho/.env

# Qwen Cloud / DashScope (Qwen models)
miho chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.miho/.env

# Xiaomi MiMo
miho chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ~/.miho/.env

# Tencent TokenHub (Hy3 Preview)
miho chat --provider tencent-tokenhub --model hy3-preview
# Requires: TOKENHUB_API_KEY in ~/.miho/.env

# Arcee AI (Trinity models)
miho chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ~/.miho/.env

# GMI Cloud
# Use the exact model ID returned by GMI's /v1/models endpoint.
miho chat --provider gmi --model zai-org/GLM-5.1-FP8
# Requires: GMI_API_KEY in ~/.miho/.env
```
...
When using the Z.AI / GLM provider, Miho automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::

### xAI (Grok) — Responses API + Prompt Caching

xAI is wired through the Responses API (`codex_responses` transport) for automatic reasoning support on Grok 4 models — no `reasoning_effort` parameter needed, the server reasons by default. Set `XAI_API_KEY` in `~/.miho/.env` and pick xAI in `miho model`, or drop `grok` as a shortcut into `/model grok-4-1-fast-reasoning`.

SuperGrok and X Premium+ subscribers can sign in with browser OAuth instead of using an API key — pick **xAI Grok OAuth (SuperGrok Subscription)** in `miho model`, or run `miho auth add xai-oauth`. The same OAuth bearer token is automatically reused by direct-to-xAI tools (TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md) for the full flow — and if Miho runs on a remote host, also see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md) for the required `ssh -L` tunnel.

When using xAI as a provider (any base URL containing `x.ai`), Miho automatically enables prompt caching by sending the `x-grok-conv-id` header with every API request. This routes requests to the same server within a conversation session, allowing xAI's infrastructure to reuse cached system prompts and conversation history.
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `miho tools` → Voice & TTS, or see the [Voice & TTS](../user-guide/features/tts.md#text-to-speech) page for config.
...
# Short alias
miho chat --provider novita-ai --model deepseek/deepseek-v3-0324
```
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `miho model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and Miho auto-discovers the available models.
...
miho chat --provider bedrock --model us.anthropic.claude-sonnet-4-6

# Or with explicit env vars
AWS_PROFILE=myprofile AWS_REGION=us-east-1 miho chat --provider bedrock --model us.anthropic.claude-sonnet-4-6
```
...
Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `miho model`, sign in through the browser, and Miho persists the refresh token.

```bash
miho model
# → pick "Qwen OAuth (Portal)"
# → browser opens; sign in with your Alibaba account
# → confirm — credentials are saved to ~/.miho/auth.json

miho chat   # uses portal.qwen.ai/v1 endpoint
```
...
Set `MIHO_QWEN_BASE_URL` only if the portal endpoint relocates (default: `https://portal.qwen.ai/v1`).
...
If you're subscribed to Alibaba's **Coding Plan** (a pricing SKU separate from standard DashScope API access), Miho exposes it as its own first-class provider: `alibaba-coding-plan`. Endpoint: `https://coding-intl.dashscope.aliyuncs.com/v1`. It's OpenAI-compatible like the regular `alibaba` provider but with a different base URL and billing surface.
...
```bash
miho chat --provider alibaba_coding --model qwen3-coder-plus
```
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `miho model`, sign in through the browser, and Miho persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
miho chat   # uses api.minimax.io/anthropic endpoint
```
...
miho chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
# Requires: NVIDIA_API_KEY in ~/.miho/.env

# Local NIM endpoint — override base URL
NVIDIA_BASE_URL=http://localhost:8000/v1 miho chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
```
...
Miho automatically attaches the NIM billing-origin header on every request to `build.nvidia.com` — no configuration needed. This routes consumption against the correct origin in NVIDIA's billing dashboard.
...
```bash
# GMI Cloud
miho chat --provider gmi --model deepseek-ai/DeepSeek-R1
# Requires: GMI_API_KEY in ~/.miho/.env
```
...
```bash
# StepFun
miho chat --provider stepfun --model step-3-mini
# Requires: STEPFUN_API_KEY in ~/.miho/.env
```
...
```bash
# Use any available model
miho chat --provider huggingface --model Qwen/Qwen3-235B-A22B-Thinking-2507
# Requires: HF_TOKEN in ~/.miho/.env

# Short alias
miho chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
Miho Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Miho at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`OPENAI_BASE_URL` and `LLM_MODEL` in `.env` are **removed**. Neither is read by any part of Miho — `config.yaml` is the single source of truth for model and endpoint configuration. If you have stale entries in your `.env`, they are automatically cleared on the next `miho setup` or config migration. Use `miho model` or edit `config.yaml` directly.
:::
...
:::warning miho model vs /model
**`miho model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.

**`/model`** (typed inside an active Miho chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys. If you've only configured one provider (e.g. OpenRouter), `/model` will only show models for that provider.

**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `miho model`, set up the new provider, then start a new session.
:::
...
When switching providers, Miho persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like Miho.
:::
...
| Flag | Purpose |
|------|---------|
| `--enable-auto-tool-choice` | Required for `tool_choice: "auto"` (the default in Miho) |
| `--tool-call-parser <name>` | Parser for the model's tool call format |

Supported parsers: `miho` (Qwen 2.5, Miho 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.
...
Then configure Miho to point at it:
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but Miho won't recognize it as a tool call — you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.

Native tool calling support (best performance): Llama 3.x, Qwen 2.5 (including Coder), Miho 2/3, Mistral, DeepSeek, Functionary. All other models use a generic handler that works but may be less efficient. See the [llama.cpp function calling docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) for the full list.
...
Miho will automatically load a LM Studio model with 64K context length
...
**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, Miho) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.

---

### WSL2 Networking (Windows Users)

Since Miho Agent requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Use that IP in your Miho config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your Miho config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with Miho.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running Miho inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser miho` |
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. Miho's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what Miho thinks the context is
...
Miho auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), Miho uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure Miho with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/docs/user-guide/configuration#context-compression) in Miho.
...
Then configure Miho with `miho model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure Miho with `miho model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `miho model` → Custom endpoint, or in `config.yaml`:
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). Miho uses this to decide when to compress history and to validate API requests.
...
Miho uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
`miho model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key omitted — Miho uses "no-key-required" for keyless local servers
  - name: work
    base_url: https://gpu-server.internal.corp/v1
    key_env: CORP_API_KEY
    api_mode: chat_completions   # set explicitly by `miho model` → Custom Endpoint wizard; auto-detection still happens as a fallback
...
Some OpenAI-compatible endpoints need provider-specific request body fields. Add an `extra_body` map to the matching custom provider and Miho will merge it into each chat-completions request for that endpoint:
...
The `miho model` → Custom Endpoint wizard now prompts for `api_mode` explicitly and persists your answer to `config.yaml`. URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.
...
You can also select named custom providers from the interactive `miho model` menu.

---

### Cookbook: Together AI, Groq, Perplexity

The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under `custom_providers:`. Three worked recipes follow. Each drops into `~/.miho/config.yaml` and the matching API key goes in `~/.miho/.env`.
...
Together's `/v1/models` endpoint works, so `miho model` can auto-discover available models.
...
:::tip Troubleshooting
- `miho doctor` should print no `Unknown provider` warnings for any of these names after the CLI validator fixes in #15083.
- If a provider's `/v1/models` endpoint is unreachable (Perplexity is the common one), `miho model` will persist the model with a warning rather than hard-reject — see #15136.
...
:::tip
You can switch between providers at any time with `miho model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point Miho at your instance (no API key needed):
   ```bash
   miho config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.miho/config.yaml`:
...
OpenRouter ships an experimental coding-model router at `openrouter/pareto-code` that auto-routes requests to the cheapest model meeting a coding-quality bar (ranked by [Artificial Analysis](https://artificialanalysis.ai/)). Pick this model and tune the `min_coding_score` knob in `~/.miho/config.yaml`:
...
Configure a chain of backup providers Miho tries in order when the primary model fails (rate limits, server errors, auth failures). The canonical format is a top-level `fallback_providers:` list:
...
:::tip
Fallback is configured exclusively through `config.yaml` — or interactively via `miho fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](/docs/user-guide/features/fallback-providers).
:::
