...
description: "Run Miho Agent directly on an Android phone with Termux"
---

# Miho on Android with Termux

This is the tested path for running Miho Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Miho from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Miho now ships a Termux-aware installer path:
...
- links `miho` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `miho` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/miho" "$PREFIX/bin/miho"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `miho` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.miho/.env`.
...
### `miho doctor` says ripgrep or Node is missing
