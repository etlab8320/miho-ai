...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Miho Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Miho Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip
Miho also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `miho --tui` — see the [TUI](tui.md) guide.
:::
...
miho chat --model "anthropic/claude-sonnet-4"

# With a specific provider
miho chat --provider nous        # Use Nous Portal
miho chat --provider openrouter  # Force OpenRouter

# With specific toolsets
miho chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
miho -s miho-agent-dev,github-auth
miho chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
miho --continue             # Resume the most recent CLI session (-c)
miho --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
miho chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
miho -w                         # Interactive mode in worktree
miho -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the Miho CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Miho CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `MIHO_YOLO_MODE` is on (either `miho --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`miho -c` or `miho --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Miho to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Miho supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Miho replies |
...
```bash
miho -s miho-agent-dev,github-auth
miho chat -s github-pr-workflow -s github-auth
```

Miho loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.miho/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.miho/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Miho..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Miho recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Miho — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Miho is working, Miho prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Miho to the background — just like any terminal process. The shell prints a confirmation:

```
Miho Agent has been suspended. Run `fg` to bring Miho Agent back.
```
...
Resume this session with:
  miho --resume 20260225_143052_a1b2c3
...
```bash
miho --continue                          # Resume the most recent CLI session
miho -c                                  # Short form
miho -c "my project"                     # Resume a named session (latest in lineage)
miho --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
miho --resume "refactoring auth"         # Resume by title
miho -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `miho sessions rename <id> <title>` from the command line. Use `miho sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Miho's SQLite state database under `~/.miho/state.db`. The database keeps:
...
Miho immediately confirms the task and gives you back the prompt:
