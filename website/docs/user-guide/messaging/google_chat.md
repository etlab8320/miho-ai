...
sidebar_position: 12
title: "Google Chat"
description: "Set up Miho Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Miho Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Miho
...
download the file. Save it somewhere only Miho can read (e.g.,
`~/.miho/google-chat-sa.json`, `chmod 600`).
...
- Topic ID: `miho-chat-events`
- Leave the defaults for everything else.

After creation, the topic's detail page has a **Subscriptions** tab. Create one:

- Subscription ID: `miho-chat-events-sub`
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a miho restart)
...
- Principal: `miho-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Miho calls
...
- **App name**: whatever you want users to see ("Miho" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/miho-chat-events`.
...
`ADDED_TO_SPACE` event that Miho uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Miho

Add the Google Chat section to `~/.miho/.env`:

```bash
# Required
GOOGLE_CHAT_PROJECT_ID=my-chat-bot-123
GOOGLE_CHAT_SUBSCRIPTION_NAME=projects/my-chat-bot-123/subscriptions/miho-chat-events-sub
GOOGLE_CHAT_SERVICE_ACCOUNT_JSON=/home/you/.miho/google-chat-sa.json
...
Install the dependencies the Google Chat adapter needs (no Miho extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Miho is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Miho detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Miho session.
...
3. Download the JSON. Move it onto the host that runs Miho.
4. On the host, register the client with Miho:
...
That writes `~/.miho/google_chat_user_client_secret.json`. This is shared
...
The token lands at `~/.miho/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.miho/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Miho isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `miho gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Miho will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.miho/google_chat_user_tokens/<sanitized_email>.json` (filesystem
