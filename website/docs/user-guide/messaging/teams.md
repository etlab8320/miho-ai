...
description: "Set up Miho Agent as a Microsoft Teams bot"
---

# Microsoft Teams Setup

Connect Miho Agent to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint — either a dev tunnel (local dev) or a real domain (production).
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which Miho strips automatically before processing.
...
```bash
# devtunnel (Microsoft)
devtunnel create miho-bot --allow-anonymous
devtunnel port create miho-bot -p 3978 --protocol https  # replace 3978 with TEAMS_PORT if changed
devtunnel host miho-bot
...
```bash
MIHO_UID=$(id -u) MIHO_GID=$(id -g) docker compose up -d gateway
```
...
Alternatively, configure via `~/.miho/config.yaml`:
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.miho/.env` |
...
| Tunnel URL changes on restart | devtunnel URLs are persistent if you use a named tunnel (`devtunnel create miho-bot`). ngrok and cloudflared generate a new URL each run unless you have a paid plan — update the bot endpoint with `teams app update` when it changes |
| Teams shows "This bot is not responding" | The webhook returned an error. Check `docker logs miho` for tracebacks |
...
- Store credentials in `~/.miho/.env` with permissions `600` (`chmod 600 ~/.miho/.env`)
