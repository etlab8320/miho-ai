...
Connect Miho to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Miho supports two WeCom integration modes:
...
3. Miho decrypts the message, queues it for the agent
...
(Use `miho gateway start` only after `miho gateway install` has registered the systemd/launchd service.)
