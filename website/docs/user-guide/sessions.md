...
Miho Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.miho/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Miho stores session history so it can resume conversations, but it does not
...
Miho explicitly injects for that turn.
...
For example, if a user sends an image and asks Miho to make a meme from it,
Miho may inspect that image once with vision and run an image-processing
...
`miho sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`miho` or `miho chat`) |
...
miho -c "my project"   # → resumes "my project #3"
```
...
miho --resume 20250305_091523_a1b2c3d4
miho -r 20250305_091523_a1b2c3d4

# Resume by title
miho --resume "refactoring auth"

# Or with the chat subcommand
miho chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `miho sessions list`.

### Conversation Recap on Resume

When you resume a session, Miho displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Miho session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.miho/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `miho -r "<title>"` from the shell) and pick up where the platform left off.
...
Miho automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `miho sessions list` or `miho sessions browse`.
...
```bash
miho sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Miho creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`miho -c "my project"`), it automatically picks the most recent session in the lineage.
...
Miho provides a full set of session management commands via `miho sessions`:
...
# Filter by platform
miho sessions list --source telegram

# Show more sessions
miho sessions list --limit 50
```
...
miho sessions export backup.jsonl

# Export sessions from a specific platform
miho sessions export telegram-history.jsonl --source telegram

# Export a single session
miho sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
miho sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
miho sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
miho sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
miho sessions prune --older-than 30

# Only prune sessions from a specific platform
miho sessions prune --source telegram --older-than 60

# Skip confirmation
miho sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`miho insights`](/docs/reference/cli-commands#miho-insights).
...
When Miho cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Miho uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Miho in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.miho/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.miho/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.miho/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |
...
`*.jsonl` files in `~/.miho/sessions/`. They are no longer written or
read by Miho. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Miho process in the same `MIHO_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.miho/config.yaml`:
...
miho sessions delete <session_id>
