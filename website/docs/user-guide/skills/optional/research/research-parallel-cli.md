...
| | |
|---|---|
| Source | Optional — install with `miho skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Miho core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Miho native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Miho native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Miho native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Miho usage patterns
...
- Do not silently choose Parallel when Miho native tools are already sufficient.
