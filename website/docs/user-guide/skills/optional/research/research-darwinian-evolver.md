...
| | |
|---|---|
| Source | Optional — install with `miho skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Miho Agent |
| License | MIT |
...
import upstream classes into Miho itself.
...
```bash
mkdir -p ~/.miho/cache/darwinian-evolver && cd ~/.miho/cache/darwinian-evolver
...
```bash
cd ~/.miho/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.miho/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.miho/skills/research/darwinian-evolver
DE_DIR=~/.miho/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Miho core.
   Custom driver scripts under `~/.miho/skills/...` are user-side and fine.
