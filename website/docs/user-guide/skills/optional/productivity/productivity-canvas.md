...
| | |
|---|---|
| Source | Optional — install with `miho skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Miho Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.miho/.env`:
...
```bash
CANVAS="python $MIHO_HOME/skills/productivity/canvas/scripts/canvas_api.py"
