...
| | |
|---|---|
| Source | Optional — install with `miho skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${MIHO_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${MIHO_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${MIHO_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client miho
```
...
```bash
PUBLISH="${MIHO_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client miho
```
...
```bash
DRIVE="${MIHO_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `miho`)    |
