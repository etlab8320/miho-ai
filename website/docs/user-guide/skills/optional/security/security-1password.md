...
| | |
|---|---|
| Source | Optional — install with `miho skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Miho Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Miho terminal calls (desktop app flow only)
...
### Service Account (recommended for Miho)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.miho/.env` (the skill will prompt for this on first load).
...
## Miho Execution Pattern (desktop app flow)

Miho terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/miho-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/miho-op.sock"
