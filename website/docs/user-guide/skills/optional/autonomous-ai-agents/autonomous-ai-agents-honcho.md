...
description: "Configure and use Honcho memory with Miho -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Miho -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `miho skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Miho

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Miho profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/miho#running-honcho-locally-with-miho

### Verify

```bash
miho honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Miho profile's AI peer
...
Honcho models conversations as interactions between **peers**. Miho creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Miho instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Miho run |
| `global` | Single session across all directories |

Manual override: `miho honcho map my-project-name`
...
Each Miho profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block miho.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `miho.coder` host block in `honcho.json`
...
```bash
miho honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Miho when Honcho memory is active.
...
Config file: `$MIHO_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `miho honcho setup`. Ensure `memory.provider: honcho` is in `~/.miho/config.yaml`.

### Memory not persisting across sessions
Check `miho honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `miho profile create <name> --clone`. For existing profiles: `miho honcho sync`.
...
| Command | Description |
|---------|-------------|
| `miho honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `miho honcho status` | Show resolved config, connection test, peer info for active profile |
| `miho honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `miho honcho disable` | Disable Honcho for the active profile |
| `miho honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `miho honcho peers` | Show peer identities across all profiles |
| `miho honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `miho honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `miho honcho sessions` | List known directory-to-session-name mappings |
| `miho honcho map <name>` | Map current working directory to a Honcho session name |
| `miho honcho identity` | Seed AI peer identity or show both peer representations |
| `miho honcho sync` | Create host blocks for all Miho profiles that don't have one yet |
| `miho honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Miho + Honcho |
| `miho memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `miho memory status` | Show active memory provider and config |
| `miho memory off` | Disable external memory provider |
