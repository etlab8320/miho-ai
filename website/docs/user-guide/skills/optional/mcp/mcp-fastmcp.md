...
| | |
|---|---|
| Source | Optional — install with `miho skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Miho or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Miho. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.miho/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.miho/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Miho integration, either:

- configure the server in `~/.miho/config.yaml` using the `native-mcp` skill, or
...
### Miho cannot see the deployed server

The server-building part may be correct while the Miho config is not. Load the `native-mcp` skill and configure the server in `~/.miho/config.yaml`, then restart Miho.
