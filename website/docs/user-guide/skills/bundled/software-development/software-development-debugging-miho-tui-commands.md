---
title: "Debugging Miho Tui Commands — Debug Miho TUI slash commands: Python, gateway, Ink UI"
sidebar_label: "Debugging Miho Tui Commands"
description: "Debug Miho TUI slash commands: Python, gateway, Ink UI"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Debugging Miho Tui Commands

Debug Miho TUI slash commands: Python, gateway, Ink UI.
...
| Path | `skills/software-development/debugging-miho-tui-commands` |
| Version | `1.0.0` |
| Author | Miho Agent |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `debugging`, `miho-agent`, `tui`, `slash-commands`, `typescript`, `python` |
...
# Debugging Miho TUI Slash Commands
...
Use this skill when you encounter issues with slash commands in the Miho TUI, particularly when commands aren't showing in autocomplete, aren't working properly in the TUI, or need to be added/updated.
...
<!-- ascii-guard-ignore -->
```
Python backend (miho_cli/commands.py)     <- canonical COMMAND_REGISTRY
       │
       ▼
...
3. **Check if the command exists in the Python backend:**
   ```bash
   search_files --pattern "CommandDef" --file_glob "*.py" --path miho_cli/
   search_files --pattern "commandname" --path miho_cli/commands.py --context 3
   ```
...
4. If the command runs server-side, add a handler in `MihoCLI.process_command()` in `cli.py`:
   ```python
...
1. **Command shows in TUI but not in autocomplete.** The command is defined in the TUI codebase but missing from `COMMAND_REGISTRY` in `miho_cli/commands.py`. Autocomplete data ships from Python.
...
1. Rebuild the TUI:
   ```bash
   cd /home/bb/miho-agent && npm --prefix ui-tui run build
   ```
...
4. Execute the command and confirm:
   ...
   - Any persisted config updates correctly (`read_file ~/.miho/config.yaml`)
