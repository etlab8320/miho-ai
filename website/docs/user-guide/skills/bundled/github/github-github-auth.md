...
- Give it a name like "miho-agent"
- Select scopes:
...
- Give it a title like "miho-agent-&lt;machine-name>"
...
elif [ -f ~/.miho/.env ] && grep -q "^GITHUB_TOKEN=" ~/.miho/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.miho/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
