...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Miho Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Miho Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Miho](/docs/guides/use-voice-mode-with-miho).

## Prerequisites

Before using voice features, make sure you have:

1. **Miho Agent installed** — `pip install miho-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `miho model` or set your preferred provider credentials in `~/.miho/.env`
3. **A working base setup** — run `miho` to verify the agent responds to text before enabling voice

:::tip
The `~/.miho/` directory and default `config.yaml` are created automatically the first time you run `miho`. You only need to create `~/.miho/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/docs/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `miho setup --portal` wires both up at once.
:::
...
pip install "miho-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "miho-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "miho-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "miho-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`miho chat`) and the **TUI** (`miho --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.miho/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
miho                # Start the interactive CLI
```
...
1. Start the CLI with `miho` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.miho/config.yaml` (default: `ctrl+b`).
:::
...
```bash
miho gateway        # Start the gateway (connects to configured platforms)
miho gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@mihobyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.miho/.env`:
```bash
...
```bash
miho gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.miho/.env`
...
- Review gateway logs: `tail -f ~/.miho/logs/gateway.log`
