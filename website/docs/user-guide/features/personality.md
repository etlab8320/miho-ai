...
description: "Customize Miho Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Miho Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `MIHO_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Miho is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Miho now seeds a default `SOUL.md` automatically in:

```text
~/.miho/SOUL.md
```

More precisely, it uses the current instance's `MIHO_HOME`, so if you run Miho with a custom home directory, it will use:
...
- Miho creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Miho loads `SOUL.md` only from `MIHO_HOME`
- Miho does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Miho falls back to a built-in default identity
...
If Miho loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `MIHO_HOME`, the personality belongs to the Miho instance itself.

That also makes it easier to teach users:
- "Edit `~/.miho/SOUL.md` to change Miho' default personality."
...
- how Miho should handle uncertainty, disagreement, or ambiguity
...
## What Miho injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Miho falls back to a built-in default identity ("You are Miho Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Miho ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Miho, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Miho its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.miho/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.miho/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-miho)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Miho speaks
- `display.skin` and `/skin` affect how Miho looks in the terminal
