...
When Miho Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
custom instructions in `~/.miho/config.yaml` under `agent.custom_instructions`.
...
If you use Miho' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.miho/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Miho Agent's deliverable
