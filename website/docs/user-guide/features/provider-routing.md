...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Miho Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.miho/config.yaml`:
...
- **CLI mode** — configured in `~/.miho/config.yaml`, loaded at startup
