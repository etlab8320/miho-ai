...
chat completions — use your Miho-managed provider subscription as their
...
| Use case | "Use Miho as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Miho stores
the refresh token in `~/.miho/auth.json` — the same place all Miho
...
```
Starting Miho proxy for Nous Portal
...
Model:      Miho-4-70B    # or Miho-4.3-36B, Miho-4-405B
```
...
```
Miho proxy upstream adapters

  [nous    ] Nous Portal — ready (bearer expires 2026-05-15T06:43:21Z)
```

If you see `not logged in`, run `miho login nous`. If you see
...
INFERENCE_TEXT_MODEL=Miho-4-70B
```
...
```bash
miho proxy start --host 0.0.0.0 --port 8645
```
...
`miho_cli/proxy/adapters/<provider>.py` and registering it in
