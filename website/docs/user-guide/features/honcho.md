...
**Multi-agent profiles**: When multiple Miho instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
miho memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$MIHO_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `miho` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Miho syncs them back at session init.
...
The `miho honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `miho memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
miho honcho status          # Connection status, config, and key settings
miho honcho setup           # Redirects to `miho memory setup`
miho honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
miho honcho peer            # Show or update peer names + dialectic reasoning level
miho honcho mode            # Show or set recall mode (hybrid/context/tools)
miho honcho tokens          # Show or set token budget for context and dialectic
miho honcho identity        # Seed or show the AI peer's Honcho identity
miho honcho sync            # Sync Honcho config to all existing profiles
miho honcho peers           # Show peer identities across all profiles
miho honcho sessions        # List known Honcho session mappings
miho honcho map             # Map current directory to a Honcho session name
miho honcho enable          # Enable Honcho for the active profile
miho honcho disable         # Disable Honcho for the active profile
miho honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `miho honcho`

If you previously used the standalone `miho honcho setup`:
...
No re-login or re-setup needed. Run `miho memory setup` and select "honcho" — the wizard detects your existing config.
