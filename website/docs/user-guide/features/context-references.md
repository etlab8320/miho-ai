...
Type `@` followed by a reference to inject content directly into your message. Miho expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Miho env: `$MIHO_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$MIHO_HOME/skills/.hub/`
