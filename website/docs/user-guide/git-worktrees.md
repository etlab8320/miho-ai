...
title: "Git Worktrees"
description: "Run multiple Miho agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Miho Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Miho so each session has a clean, isolated working directory.

## Why Use Worktrees with Miho?

Miho treats the **current working directory** as the project root:

- CLI: the directory where you run `miho` or `miho chat`
...
git worktree add ../repo-feature feature/miho-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/miho-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Miho there:

```bash
cd ../repo-feature

# Start Miho in the worktree
miho
```
...
git worktree add ../repo-experiment-a feature/miho-a
git worktree add ../repo-experiment-b feature/miho-b
```
...
- Works on its own branch (`feature/miho-a` vs `feature/miho-b`).
...
- Miho checkpoint data under `~/.miho/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Miho experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/miho-checkpoints-docs`, `feature/miho-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Miho from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `miho -w` (Automatic Worktree Mode)

Miho has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `miho/miho-<hash>`).
...
```bash
miho -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `miho -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Miho session its own clean checkout.
