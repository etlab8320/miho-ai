...
Miho Agent uses a SQLite database (`~/.miho/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `miho_state.py`


## Architecture Overview

```
~/.miho/state.db (SQLite, WAL mode)
...
Multiple miho processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.miho/state.db
...
Default path: `~/.miho/state.db`

This is derived from `miho_constants.get_miho_home()` which resolves to
`~/.miho/` by default, or the value of `MIHO_HOME` environment variable.
