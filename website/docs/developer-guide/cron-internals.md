...
description: "How Miho stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `miho_cli/cron.py` | CLI `miho cron` subcommands |
...
Jobs are stored in `~/.miho/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `miho cron` commands are run or during active CLI sessions.
...
```python
# ~/.miho/scripts/check_competitors.py
...
2. **Environment variable** — `MIHO_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.miho/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `miho cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `miho cron` CLI provides direct job management:

```bash
miho cron list                    # Show all jobs
miho cron create                  # Interactive job creation (alias: add)
miho cron edit <job_id>           # Edit job configuration
miho cron pause <job_id>          # Pause a running job
miho cron resume <job_id>         # Resume a paused job
miho cron run <job_id>            # Trigger immediate execution
miho cron remove <job_id>         # Delete a job
```
