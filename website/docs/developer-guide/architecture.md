...
sidebar_position: 1
title: "Architecture"
description: "Miho Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Miho Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ miho_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # MihoCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── miho_state.py           # SQLite session/state database with FTS5
├── miho_constants.py       # MIHO_HOME, profile-aware paths
...
│
├── miho_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `miho` subcommands (large file)
...
│   ├── skills_config.py      # miho skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # miho tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # miho gateway start/stop
│
...
```text
User input → MihoCLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .miho.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.miho/plugins/` (user), `.miho/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `miho plugins` or `config.yaml`.

→ [Plugin Guide](/docs/guides/build-a-miho-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Miho as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`miho -p <name>`) gets its own MIHO_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
