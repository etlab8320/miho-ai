...
[PR #12550](https://github.com/NousResearch/miho-agent/pull/12550) proposed a
...
(`miho-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Miho `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `miho-acp`, `miho-api-server`, core toolsets (gated on CDP reachability)
...
- `miho_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
