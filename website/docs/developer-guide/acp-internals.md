...
The ACP adapter wraps Miho' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
miho acp / miho-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.miho/.env
  -> configure stderr logging
  -> construct MihoACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'miho-agent[acp]==<version>' miho-acp`, pointed at the `miho-agent` PyPI release.
...
- `allow_once` -> Miho `once`
- `allow_always` -> Miho `always`
- reject options -> Miho `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Miho tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["miho-acp"])
...
Instead it reuses Miho' runtime resolver:

- `acp_adapter/auth.py`
- `miho_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Miho provider/credentials. It also always advertises a terminal setup auth method (`miho-setup`, args `--setup`) so first-run registry clients can open Miho' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.miho/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `miho-acp` toolset definition
- `miho_cli/main.py` — `miho acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `miho-acp` script
