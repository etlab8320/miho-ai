...
description: "Build wrapper CLIs that extend the Miho TUI with custom widgets, keybindings, and layout changes"
---

# Extending the CLI

Miho exposes protected extension hooks on `MihoCLI` so wrapper CLIs can add widgets, keybindings, and layout customizations without overriding the 1000+ line `run()` method. This keeps your extension decoupled from internal changes.
...
```python
#!/usr/bin/env python3
"""my_cli.py — Example wrapper CLI that extends Miho."""
...
Called after Miho registers its own keybindings and before the layout is built. Add your keybindings to `kb`.
