...
Pure logic: walks a Miho config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `miho doctor` and a future `miho migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Miho config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Miho config and return retirement issues.
