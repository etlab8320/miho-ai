"""
Status command for miho CLI.

Shows the status of all Miho Agent components.
"""
...
from miho_cli.auth import AuthError, resolve_provider
from miho_cli.colors import Colors, color
from miho_cli.config import get_env_path, get_env_value, get_miho_home, load_config
from miho_cli.models import provider_label
from miho_cli.nous_subscription import get_nous_subscription_features
from miho_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``miho config``'s
...
def show_status(args):
    """Show status of all Miho Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Miho Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: miho auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: miho model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: miho auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: miho auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: miho model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from miho_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'miho-agent[vercel]')"
    ...
    try:
        from miho_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_miho_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_miho_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'miho doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'miho setup' to configure", Colors.DIM))
    print()
