"""Shell completion script generation for miho CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_miho_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Miho Agent bash completion
# Add to ~/.bashrc:
#   eval "$(miho completion bash)"

_miho_profiles() {{
    local profiles_dir="$HOME/.miho/profiles"
...
_miho_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_miho_profiles)" -- "$cur"))
        return
    fi
...
complete -F _miho_completion miho
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef miho
# Miho Agent zsh completion
# Add to ~/.zshrc:
...
_miho_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.miho/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.miho/profiles 2>/dev/null)}}")
    fi
...
_miho() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_miho_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'miho command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Miho Agent fish completion",
        "# Add to your config:",
        "#   miho completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __miho_profiles",
        "    echo default",
        "    if test -d $HOME/.miho/profiles",
        "        ls $HOME/.miho/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c miho -f -s p -l profile"
        " -d 'Profile name' -xa '(__miho_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__miho_profiles)' -d 'Profile name'"
                )
