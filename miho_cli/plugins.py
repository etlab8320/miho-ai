...
_PLUGINS_DEBUG = os.getenv("MIHO_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When MIHO_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Miho loggers.
    """
    ...
    logger.debug(
        "MIHO_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
class PluginManifest:
    ...
    provides_auxiliary_tasks: List[str] = field(default_factory=list)
    ...
    # lookups and by ``miho plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``miho honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``miho <subcommand>``
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``miho model → Configure auxiliary models`` picker
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to miho_cli/. Two layouts are
        ...
        # 3. Project plugins (./.miho/plugins/)
        if _env_enabled("MIHO_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".miho" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set MIHO_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with miho and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform Miho ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `miho plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _parse_manifest(
        ...
        try:
            ...
            return PluginManifest(
                ...
                provides_auxiliary_tasks=data.get("provides_auxiliary_tasks", []),
                source=source,
                path=str(plugin_dir),
                kind=kind,
                key=key,
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``miho_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``miho_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
    ...
    async def invoke_hook_async(self, hook_name: str, **kwargs: Any) -> List[Any]:
        """Async variant of :meth:`invoke_hook` for gateway decision hooks."""
        callbacks = self._hooks.get(hook_name, [])
        results: List[Any] = []
        for cb in callbacks:
            try:
...
async def invoke_hook_async(hook_name: str, **kwargs: Any) -> List[Any]:
    """Invoke plugin hooks and await coroutine callbacks."""
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="miho-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``miho tools`` TUI so plugin-provided toolsets appear
