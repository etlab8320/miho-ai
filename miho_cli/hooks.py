"""miho hooks — inspect and manage shell-script hooks.

Usage::

    miho hooks list
    miho hooks test <event> [--for-tool X] [--payload-file F]
    miho hooks revoke <command>
    miho hooks doctor

Consent records live under ``~/.miho/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.miho/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``miho hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: miho hooks {list|test|revoke|doctor}")
        print("Run 'miho hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.miho/config.yaml.")
        print("See `miho hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `miho hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `miho hooks test` and `miho hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Miho wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `miho hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `miho hooks doctor` would execute
