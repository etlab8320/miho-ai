...
- ``miho_cli.providers``        -- canonical provider identity + overlays
- ``miho_cli.model_normalize``  -- per-provider name formatting
...
from miho_cli.providers import (
...
_MIHO_MODEL_WARNING = (
    "Nous Research Miho 3 & 4 models are NOT agentic and are not designed "
    "for use with Miho Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Miho 3 / Miho 4 chat families.
# The previous substring check (`"miho" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``miho-brain:qwen3-14b-ctx16k`` that just
# happen to carry "miho" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Miho-3-Llama-3.1-70B, miho-4-405b, openrouter/miho3:70b
# Negative examples it must NOT match:
#   miho-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_MIHO_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])miho[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_miho_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Miho 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_MIHO_NON_AGENTIC_RE.search(model_name))


def _check_miho_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Miho 3/4 chat model."""
    if is_nous_miho_non_agentic(model_name):
        return _MIHO_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``miho config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from miho_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'miho model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'miho doctor' — config issues detected:"
    ...
    # Mirrors the same logic in miho_cli.runtime_provider.resolve_runtime_provider;
    ...
    miho_warn = _check_miho_model_warning(new_model)
    if miho_warn:
        warnings.append(miho_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from miho_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from miho_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by miho provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://miho-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Miho release. Falls back to the in-repo
    ...
    # --- 1. Check Miho-mapped providers ---
    for miho_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(miho_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (MIHO_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from miho_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(miho_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a Miho release.
        model_ids = curated.get(miho_id, [])
        if miho_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(miho_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Miho-only providers (nous, openai-codex, copilot, opencode-go) ---
    from miho_cli.providers import MIHO_OVERLAYS
    from miho_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Miho provider ID.
    # MIHO_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Miho IDs ("copilot").
    _mdev_to_miho = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in MIHO_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Miho slug — e.g. "github-copilot" → "copilot"
        miho_slug = _mdev_to_miho.get(pid, pid)
        if miho_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(miho_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, miho_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or miho_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(miho_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", miho_slug, exc)
        ...
        # configured.
        if not has_creds and miho_slug == "anthropic":
            try:
                ...
                if (miho_creds and miho_creds.get("accessToken")) or \
        ...
        if miho_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = provider_model_ids(miho_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(miho_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(miho_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by Miho slug, fall back to overlay key
            model_ids = curated.get(miho_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if miho_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(miho_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": miho_slug,
            "name": get_label(miho_slug),
            "is_current": miho_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "miho",
        ...
        seen_slugs.add(miho_slug.lower())
        _record_builtin_endpoint(miho_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or MIHO_OVERLAYS (keeps /model in sync
    # with `miho model`).
    try:
        from miho_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Miho's canonical write key (matches
            ...
            # Miho writes ``models:`` as a dict keyed by model id
            # (see miho_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Miho's own writer uses; a hyphen variant is accepted
            ...
            # active model. Miho's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, miho_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`miho_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from miho_cli.models import fetch_openrouter_models
