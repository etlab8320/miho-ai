"""``miho portal`` — small CLI surface for Nous Portal users.
...
already in ``miho auth`` or ``miho tools``. It's a discovery + status
...
DOCS_URL = "https://miho-agent.nousresearch.com/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if logged_in:
        ...
        print(f"  Login:   miho auth add nous --type oauth")
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `miho model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `miho auth add nous --type oauth`.", Colors.YELLOW))
        print()
...
def portal_command(args) -> int:
    """Top-level dispatch for `miho portal <subcommand>`."""
    ...
    print("Run `miho portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `miho portal` on the given argparse subparsers object."""
