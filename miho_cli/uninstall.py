"""
Miho Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.miho/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Miho PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing miho-agent or miho PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Miho Agent" comment and following line
                if '# Miho Agent' in line or '# miho-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('miho' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing miho
                if 'miho' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the miho wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "miho",
        Path("/usr/local/bin/miho"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains miho_cli reference)
                content = wrapper.read_text()
                if 'miho_cli' in content or 'miho-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``miho gateway run`` processes
    ...
    try:
        from miho_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from miho_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``MIHO_HOME`` and ``MIHO_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\miho\git\cmd``, ``%LOCALAPPDATA%\miho\git\bin``,
#      ``%LOCALAPPDATA%\miho\git\usr\bin``, ``%LOCALAPPDATA%\miho\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\miho\git\`` and Node to
#      ``%LOCALAPPDATA%\miho\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``miho dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\miho\gateway-service\`` and sometimes
...
def _miho_path_markers(miho_home: Path) -> list[str]:
    """Path-entry substrings that identify Miho-owned User-PATH entries."""
    root = str(miho_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare miho-agent install dir.
    markers = [root + "\\miho-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if MIHO_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "miho".  We don't want to catch
    # unrelated entries like "cmiho-foo" or "ephermeral", so we look for
    # backslash-miho as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(miho_home: Path) -> list[str]:
    """Strip Miho-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _miho_path_markers(miho_home)
            kept: list[str] = []
...
def remove_miho_env_vars_windows() -> list[str]:
    """Delete MIHO_HOME and MIHO_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("MIHO_HOME", "MIHO_GIT_BASH_PATH"):
                try:
...
def _is_default_miho_home(miho_home: Path) -> bool:
    """Return True when ``miho_home`` points at the default (non-profile) root."""
    try:
        from miho_constants import get_default_miho_root
        return miho_home.resolve() == get_default_miho_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its MIHO_HOME directory.

    We shell out to ``miho -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current MIHO_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m miho_cli.main` so we don't depend on a `miho`
    #    wrapper that may be half-removed mid-uninstall.
    miho_invocation = [_sys.executable, "-m", "miho_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                miho_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's MIHO_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.miho/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.miho/ for future reinstall
    """
    project_root = get_project_root()
    miho_home = get_miho_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie MIHO_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_miho_home(miho_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Miho Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {miho_home / 'config.yaml'}")
    print(f"  Secrets: {miho_home / '.env'}")
    print(f"  Data:    {miho_home / 'cron/'}, {miho_home / 'sessions/'}, {miho_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their MIHO_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Miho data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Miho code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in miho_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\miho\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(miho_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Miho-owned PATH entries in User environment")

        log_info("Removing MIHO_HOME / MIHO_GIT_BASH_PATH User env vars...")
        removed_env = remove_miho_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Miho-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing miho command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.miho/, just remove the miho-agent subdir
            if miho_home in project_root.parents or project_root.parent == miho_home:
    ...
    #     under MIHO_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(miho_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(miho_home)
    ...
    # 5. Optionally remove ~/.miho/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile MIHO_HOME dirs live under
        ...
        try:
            if miho_home.exists():
                shutil.rmtree(miho_home)
                log_success(f"Removed {miho_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {miho_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {miho_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/NousResearch/miho-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/miho-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Miho Agent! ⚕")
    print()
