"""
Profile management for multiple isolated Miho instances.

Each profile is a fully independent MIHO_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.miho/profiles/<name>/`` by default.

The "default" profile is ``~/.miho`` itself — backward compatible,
zero migration needed.

Usage::

    miho profile create coder          # fresh profile + bundled skills
    miho profile create coder --clone  # also copy config, .env, SOUL.md, skills
    miho profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    miho -p coder chat                 # or via flag
    miho profile use coder             # set as sticky default
    miho profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See miho_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Marker file written by `miho profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `miho
...
# `miho skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Miho infrastructure directories that only the default profile
         (``~/.miho``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_miho_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.miho) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "miho-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "miho", "default", "test", "tmp", "root", "sudo",
})

# Miho subcommands that cannot be used as profile names/aliases
_MIHO_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the miho root, NOT to the current MIHO_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where MIHO_HOME points outside
    ``~/.miho``, profiles live under ``MIHO_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_miho_home() / "profiles"


def _get_default_miho_home() -> Path:
    """Return the default (pre-profile) MIHO_HOME path.

    In standard deployments this is ``~/.miho``.
    In Docker/custom deployments where MIHO_HOME is outside ``~/.miho``
    (e.g. ``/opt/data``), returns MIHO_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_miho_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Miho installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its MIHO_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_miho_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, miho subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _MIHO_SUBCOMMANDS:
        return f"'{canon}' conflicts with a miho subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``miho profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Miho config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_miho_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``miho update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.miho)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.miho/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `miho update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`miho profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `miho update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `miho profile describe`
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches MIHO_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``miho profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "MIHO_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.miho).\n"
            "To remove everything, use: miho uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set MIHO_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("MIHO_HOME")
    try:
        os.environ["MIHO_HOME"] = str(profile_dir)
        from miho_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["MIHO_HOME"] = old_home
        elif "MIHO_HOME" in os.environ:
            del os.environ["MIHO_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.miho/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: miho profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from MIHO_HOME.

    Returns ``"default"`` if MIHO_HOME is not set or points to ``~/.miho``.
    Returns the profile name if MIHO_HOME points into ``~/.miho/profiles/<name>``.
    Returns ``"custom"`` if MIHO_HOME is set to an unrecognized path.
    """
    from miho_constants import get_miho_home
    miho_home = get_miho_home()
    resolved = miho_home.resolve()

    default_resolved = _get_default_miho_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.miho itself — its parent is ~/ and its
        # directory name is ".miho", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: miho profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.miho itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.miho). "
            "Specify a different name: miho profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="miho_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"miho.{old_name}"
    new_host = f"miho.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_miho_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a MIHO_HOME path string.

    Called early in the CLI entry point, before any miho modules
    are imported, to set the MIHO_HOME environment variable.
    """
