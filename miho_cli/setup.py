"""
Interactive setup wizard for Miho Agent.
...
from miho_cli.nous_subscription import get_nous_subscription_features
...
_DOCS_BASE = "https://miho-agent.nousresearch.com/docs"
...
# display_miho_home imported lazily at call sites (stale-module safety during miho update)
...
from miho_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Miho Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Miho using environment variables or config commands:")
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from miho_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from miho_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'miho setup')")


def _print_setup_summary(config: dict, miho_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'miho setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from miho_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `miho tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    try:
        from agent.video_gen_registry import list_providers as _list_video_providers
        from miho_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'miho setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'miho setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'miho setup terminal'"))
    ...
    # Spotify (OAuth via miho auth spotify — check auth.json, not env vars)
    try:
        from miho_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'miho setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {miho_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('miho setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('miho setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('miho setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('miho setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('miho setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('miho config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('miho config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('miho config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('miho', Colors.GREEN)}              Start chatting")
    print(f"   {color('miho gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('miho doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `miho tools` and `miho setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``miho model``)
    ...
    provider added to ``miho model`` is automatically available here.
    ...
    # Delegate to the shared miho model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from miho_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: miho model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            print_info(
                "Miho can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Miho can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'miho setup' or configure AUXILIARY_VISION_* settings")
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``miho model`` -> xAI Grok OAuth (SuperGrok Subscription).
    """
    try:
        from miho_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.miho/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Miho auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via miho setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'miho setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Miho runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("Requires the optional SDK: pip install 'miho-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'miho-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into MIHO_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("MIHO_MAX_ITERATIONS")
    ...
    print_info("  Run `miho setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # MIHO_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Miho delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `miho update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://miho-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under MIHO_HOME, and print
    ...
    """
    try:
        from miho_cli.slack_cli import _build_full_manifest
        from miho_constants import get_miho_home

        manifest = _build_full_manifest(
            bot_name="Miho",
            bot_description="Your Miho agent on Slack",
        )
        target = Path(get_miho_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `miho slack manifest --write` anytime to refresh after "
            "Miho adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Miho delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Miho delivers cron job results and notifications.")
    print_info("   To get a channel ID: click channel name → View Info → copy the ID")
    print_info("   You can also set this later by typing /set-home in a Mattermost channel.")
    home_channel = prompt("Home channel ID (leave empty to set later with /set-home)")
    if home_channel:
        save_env_value("MATTERMOST_HOME_CHANNEL", home_channel)
    print_info("   Open config in your editor:  miho config edit")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Miho to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from miho_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://miho-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://miho-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  miho config edit")
    print_info("   Open config in your editor:  miho config edit")
...
def setup_gateway(config: dict):
    ...
    from miho_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Miho from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'miho setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     miho config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_miho_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  miho gateway")
        else:
            from miho_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   miho gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   miho gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `miho setup tools` and `miho tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``miho_cli.auth`` — lists every supported
    ...
    """
    try:
        from miho_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from miho_cli.config
    ...
    if section_key == "model":
        ...
        from miho_cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_miho migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_miho", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Miho.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Miho to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Miho at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Miho at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Miho at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Miho at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Miho equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Miho config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Miho.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Miho's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(miho_home: Path) -> bool:
    ...
    print_info("Miho can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: miho claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: miho claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Miho configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Miho config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Miho (use miho claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``miho setup --portal``. Does NOT prompt for anything
    besides what the underlying OAuth + Tool Gateway prompts already need.
    Designed to be shareable as a single command (``miho setup --portal``)
    that gets a brand-new user from zero to a fully working Miho session
    ...
    from miho_cli.auth import get_nous_auth_status
    ...
    print(color("│     ⚕ Miho Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # runs `miho setup --portal` after a successful first run).
    ...
    if already_logged_in:
        ...
        # identical to `miho auth add nous --type oauth`. SimpleNamespace
        ...
        try:
            ...
            print_info("  You can retry later with `miho auth add nous --type oauth`.")
            return
        except (KeyboardInterrupt, EOFError):
            print()
    ...
    # from `miho model` after picking Nous.
    print()
    try:
    ...
    print_info("  Run `miho portal status` to inspect routing.")
    print_info("  Run `miho` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      miho setup           — full or quick (auto-detected)
      miho setup model     — just model/provider
      miho setup tts       — just text-to-speech
      miho setup terminal  — just terminal backend
      miho setup gateway   — just messaging platforms
      miho setup tools     — just tool configuration
      miho setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Miho Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Miho Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Miho Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, miho_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Miho configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'miho setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(miho_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up Miho?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, miho_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {miho_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'miho config edit'")
    ...
    _print_setup_summary(config, miho_home)


def _run_first_time_quick_setup(config: dict, miho_home, is_existing: bool):
    ...
    the user can customize later via ``miho setup <section>``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'miho setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    miho setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  miho setup gateway")
    print()

    _print_setup_summary(config, miho_home)


def _run_quick_setup(config: dict, miho_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'miho setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Miho to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'miho setup gateway'.")
