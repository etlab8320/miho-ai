...
from miho_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
def build_welcome_banner(console: Console, model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else MIHO_CADUCEUS
    except Exception:
        _bskin = None
        _hero = MIHO_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        from miho_cli.codex_runtime_switch import get_current_runtime
        from miho_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from miho_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built miho; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else MIHO_AGENT_LOGO
        console.print(_logo)
        console.print()
