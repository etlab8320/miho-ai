"""``miho logs`` — view and filter Miho log files.
...
Usage examples::
    ...
    miho logs                    # last 50 lines of agent.log
    miho logs -f                 # follow agent.log in real time
    miho logs errors             # last 50 lines of errors.log
    miho logs gateway -n 100    # last 100 lines of gateway.log
    miho logs --level WARNING    # only WARNING+ lines
    miho logs --session abc123   # filter by session ID substring
    miho logs --component tools  # only tool-related lines
    miho logs --since 1h         # lines from the last hour
    miho logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_miho_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Miho runs — try 'miho chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_miho_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_miho_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_miho_home()}/logs/")
        return

    print(f"Log files in {display_miho_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'miho chat' to generate logs)")
