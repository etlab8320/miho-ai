...
``<root>`` is the **shared Miho root** (the parent of any active
...
``miho -p <profile>`` joins the same board as the dispatcher that
...
the profile's Miho home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``MIHO_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``MIHO_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``miho kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.miho``. In Docker / custom
deployments where ``MIHO_HOME`` points outside ``~/.miho`` (e.g.
``/opt/miho``), ``<root>`` is ``MIHO_HOME``. Legacy env-var
overrides still work:

* ``MIHO_KANBAN_DB`` — pin the database file path directly.
* ``MIHO_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``MIHO_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``MIHO_KANBAN_DB``,
``MIHO_KANBAN_WORKSPACES_ROOT``, and ``MIHO_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/miho-kanban-v1-spec.pdf`` for the full
...
# ``MIHO_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``MIHO_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("MIHO_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
# enough that kebab-case names like ``atm10-server`` or ``miho-agent``
...
def kanban_home() -> Path:
    """Return the shared Miho root that anchors the kanban board.

    Resolution order:

    1. ``MIHO_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_miho_root()``, which already returns ``<root>``
       when ``MIHO_HOME`` is ``<root>/profiles/<name>``, and returns
       ``MIHO_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``MIHO_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("MIHO_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from miho_constants import get_default_miho_root
    return get_default_miho_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``miho kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``MIHO_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``miho kanban boards
    ...
    """
    env = os.environ.get("MIHO_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``miho kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``MIHO_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("MIHO_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``MIHO_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("MIHO_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``miho kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``MIHO_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    max_retries          INTEGER,
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``MIHO_SESSION_ID``. NULL
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Miho treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``MIHO_KANBAN_DB`` env → ``MIHO_KANBAN_BOARD`` env →
    ...
    try:
        ...
        with _INIT_LOCK:
            ...
            # See miho_state._WAL_INCOMPAT_MARKERS for detection logic.
            from miho_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``miho kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``MIHO_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from miho_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``miho --skills ...`` alongside the built-in
    ...
    # `miho --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``miho kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `miho kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``miho kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``miho kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``miho kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``miho kanban complete <id>``) works without requiring
...
#      via ``miho kanban show <id>`` and the dashboard.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Miho
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``miho kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Miho profile.
    ...
    if not rows:
        return False
    try:
        from miho_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real Miho profile.
        # `_default_spawn` invokes ``miho -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Miho
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `miho kanban tail` — without
...
def _module_miho_argv() -> list[str]:
    """Return the interpreter-bound Miho CLI invocation."""
    # ``miho_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``miho`` package — there is no
    # ``miho`` package to import.
    return [sys.executable, "-m", "miho_cli.main"]


def _absolute_miho_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Miho shim."""
...
def _miho_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Miho executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_miho_argv()
    return [_absolute_miho_path(path)]


def _resolve_miho_argv() -> list[str]:
    """Resolve the ``miho`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$MIHO_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("miho")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\miho.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m miho_cli.main`` — fallback for setups where
       Miho is launched from a venv and the ``miho`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_miho_bin`` for the same reason. Kept
    local (not imported from gateway) because ``miho_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("MIHO_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _miho_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _miho_path_argv(resolved_env_bin)
        return _module_miho_argv()

    miho_bin = _safe_which_no_cwd("miho") if _IS_WINDOWS else shutil.which("miho")
    if miho_bin:
        return _miho_path_argv(miho_bin)
    return _module_miho_argv()


def _kanban_worker_skill_available(miho_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``miho -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset MIHO_HOME means the worker falls back to the default root
    # home (``~/.miho``), which ships the bundled skill.
    base = _Path(miho_home) if miho_home else (_Path.home() / ".miho")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``miho -p <profile> chat -q ...`` subprocess.
    ...
    ``MIHO_KANBAN_DB`` / ``MIHO_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject MIHO_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `miho -p <name>` the
    # _apply_profile_override() runs *before* miho_constants is imported.
    # If MIHO_HOME is absent from the child's env, get_miho_home() falls
    # back to Path.home() / ".miho" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from miho_cli.profiles import resolve_profile_env
    try:
        env["MIHO_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via MIHO_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # MIHO_HOME never had profiles created.
        pass
    if task.tenant:
        env["MIHO_TENANT"] = task.tenant
    env["MIHO_KANBAN_TASK"] = task.id
    env["MIHO_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["MIHO_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["MIHO_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["MIHO_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    # that even when the worker activates a profile (`miho -p <name>`
    # rewrites MIHO_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_miho_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["MIHO_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["MIHO_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["MIHO_KANBAN_BOARD"] = resolved_board
    # MIHO_PROFILE is the author the kanban_comment tool defaults to.
    # `miho -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["MIHO_PROFILE"] = profile_arg

    cmd = [
        *_resolve_miho_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped MIHO_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("MIHO_HOME")):
    ...
    # `miho kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`miho` executable not found on PATH. "
            "Install Miho Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``miho kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled MIHO_PROFILE values like "miho-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Miho root exists

    Reads profile paths directly so this module has no import dependency on
    ``miho_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from miho_constants import get_default_miho_root
        default_root = get_default_miho_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``miho kanban assignees`` for the terminal.
