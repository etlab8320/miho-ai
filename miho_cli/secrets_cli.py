"""CLI handlers for ``miho secrets bitwarden ...``.
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    Called from ``miho_cli.main`` as part of building the top-level
    ``miho secrets`` parser.
    """
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Secrets will be pulled at the start of every Miho process."
    )
    console.print(
        "  Status:  [cyan]miho secrets bitwarden status[/cyan]\n"
        "  Refresh: [cyan]miho secrets bitwarden sync[/cyan]\n"
        "  Disable: [cyan]miho secrets bitwarden disable[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]miho secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set — Miho will skip BSM "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`miho secrets bitwarden setup` first.[/yellow]"
        )
        return 1
    ...
    if not args.apply:
        console.print(
            ...
            "next [cyan]miho[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
