"""Slash command definitions and autocomplete for the Miho CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Miho config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Miho works on across turns until achieved", "Session",
    ...
    CommandDef("verbose", "Set or cycle tool progress display",
               "Configuration", cli_only=True,
               args_hint="[clean|off|new|all|verbose]",
    ...
    CommandDef("busy", "Control what Enter does while Miho is working", "Configuration",
    ...
    CommandDef("evolution", "Auditable Miho evolution ledger, snapshots, and rollbacks",
               "Tools & Skills", args_hint="[status|list|show|snapshot|rollback|record|mine|cycle]",
               subcommands=("status", "list", "ls", "show", "snapshot", "rollback", "record", "mine", "propose", "validate", "promote", "cycle")),
    ...
    CommandDef("memory", "Discord workspace memory status, search, or rebuild",
               "Tools & Skills", gateway_only=True, args_hint="[status|search|rebuild] [query]",
               subcommands=("status", "search", "rebuild")),
    ...
    CommandDef("update", "Update Miho Agent to the latest version", "Info"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`miho_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/miho`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Miho
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``miho skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``miho skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``miho skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/miho <verb>`` subcommand.
    ...
    can still reach them via ``/miho <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/miho`` is always reserved as the first entry so the
    legacy ``/miho <subcommand>`` form keeps working for anything that
    ...
    # Reserve /miho as the catch-all top-level command.
    entries.append(("miho", "Talk to Miho or run a subcommand", "[subcommand] [args]"))
    seen.add("miho")
...
def slack_app_manifest(request_url: str = "https://miho-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /miho handler.

    Maps both canonical names and aliases so /miho bg do stuff works
    the same as /miho background do stuff.

    Plugin-registered slash commands are included so ``/miho <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from miho_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
