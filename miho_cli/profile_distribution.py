"""Profile distributions — shareable, packaged Miho profiles via git.

A distribution is a Miho profile published as a git repository (or
...
* ``miho profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``miho skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``miho profile``, not a parallel tree):

    miho profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "miho-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            miho_requires=str(data.get("miho_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.miho_requires:
            out["miho_requires"] = self.miho_requires
        if self.author:
...
def check_miho_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Miho {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``miho profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Miho profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Miho distribution."
        )

    # Version check up-front so we fail fast
    check_miho_requires(manifest.miho_requires, miho_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.miho).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="miho_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `miho profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`miho profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="miho_dist_update_") as tmp:
        plan = plan_install(
