...
on Japanese locales, etc.).  Miho's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``miho_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Miho is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``MIHO_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("MIHO_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Miho-managed Git install directories so
    ...
    # already-running shells don't see the change — which means miho
    ...
    # install without Miho-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Miho's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Miho's
    ``miho config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Miho (User env var in Windows Settings, or
    export in a PowerShell profile) and Miho picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Miho-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\miho\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``miho`` from right after install) retain their old PATH.

    Any subprocess Miho spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Miho-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "miho", "git", "cmd"),
        os.path.join(local_appdata, "miho", "git", "bin"),
        os.path.join(local_appdata, "miho", "git", "usr", "bin"),
        # Miho venv Scripts directory — host of the miho.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes miho, but harmless to include.
        os.path.join(local_appdata, "miho", "miho-agent", "venv", "Scripts"),
