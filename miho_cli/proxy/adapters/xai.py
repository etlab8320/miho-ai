...
# xAI's public API is OpenAI-compatible for the endpoints Miho commonly
# uses. The Responses endpoint is included because Miho' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Miho-managed OAuth credentials."""

    auth_hint = "miho auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`miho auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`miho auth reset xai-oauth` or re-authenticate with "
                    "`miho auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `miho auth add xai-oauth --type oauth`."
            )
