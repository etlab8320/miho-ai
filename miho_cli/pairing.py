...
Usage:
    miho pairing list              # Show all pending + approved users
    miho pairing approve <platform> <code>  # Approve a pairing code
    miho pairing revoke <platform> <user_id> # Revoke user access
    miho pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle miho pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: miho pairing {list|approve|revoke|clear-pending}")
        print("Run 'miho pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.miho/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'miho pairing list' to see pending codes.\n")
