"""`miho checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.miho/checkpoints/``.  Actions:

    miho checkpoints               # same as `status`
    miho checkpoints status        # total size, project count, breakdown
    miho checkpoints list          # per-project checkpoint counts + workdir
    miho checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    miho checkpoints clear [-f]    # nuke the entire base (asks first)
    miho checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    miho checkpoints
    miho checkpoints prune --retention-days 3 --max-size-mb 200
    miho checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: miho checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``miho checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `miho checkpoints` → status
