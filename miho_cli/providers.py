"""
Single source of truth for provider identity in Miho Agent.
...
2. **Miho overlays** — transport type, auth patterns, aggregator flags,
...
# -- Miho overlay ----------------------------------------------------------
# Miho-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class MihoOverlay:
    """Miho-specific provider metadata layered on top of models.dev."""
...
MIHO_OVERLAYS: Dict[str, MihoOverlay] = {
    "openrouter": MihoOverlay(
    ...
    "openai-codex": MihoOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "xai-oauth": MihoOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": MihoOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="MIHO_QWEN_BASE_URL",
    ),
    "google-gemini-cli": MihoOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": MihoOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": MihoOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": MihoOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": MihoOverlay(
    ...
    "minimax-oauth": MihoOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": MihoOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": MihoOverlay(
        transport="openai_chat",
    ...
    "alibaba-coding-plan": MihoOverlay(
        transport="openai_chat",
    ...
    "opencode": MihoOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": MihoOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "miho", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Miho overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Miho overlay
    ...
    overlay = MIHO_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + miho extra
    ...
    if overlay is not None:
        # Miho-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in MIHO_OVERLAYS
    if provider == "bedrock":
