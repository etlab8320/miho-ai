"""Miho CLI skin/theme engine.

A data-driven skin system that lets users customize the CLI's visual appearance.
Skins are defined as YAML files in ~/.miho/skins/ or as built-in presets.
...
.. code-block:: yaml
    ...
    branding:
      agent_name: "Miho Agent"          # Banner title, status display
      welcome: "Welcome message"          # Shown at CLI startup
      goodbye: "Goodbye! ⚕"              # Shown on exit
      response_label: " ⚕ Miho "       # Response box header label
...
.. code-block:: python
    ...
    from miho_cli.skin_engine import get_active_skin, list_skins, set_active_skin

    skin = get_active_skin()
    print(skin.colors["banner_title"])    # "#FFD700"
    print(skin.get_branding("agent_name"))  # "Miho Agent"

    set_active_skin("ares")               # Switch to built-in ares skin
    set_active_skin("mytheme")            # Switch to user skin from ~/.miho/skins/
...
Drop a YAML file in ``~/.miho/skins/<name>.yaml`` following the schema above.
...
class SkinConfig:
    ...
    banner_logo: str = ""    # Rich-markup ASCII art logo (replaces MIHO_AGENT_LOGO)
    banner_hero: str = ""    # Rich-markup hero art (replaces MIHO_CADUCEUS)
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        "name": "default",
        "description": "Classic Miho — gold and kawaii",
        "colors": {
        ...
        "branding": {
            "agent_name": "Miho Agent",
            "welcome": "Welcome to Miho Agent! Type your message or /help for commands.",
            "goodbye": "Goodbye! ⚕",
            "response_label": " ⚕ Miho ",
            "prompt_symbol": "❯",
...
def init_skin_from_config(config: dict) -> None:
    ...
    if not isinstance(display, dict):
        display = {}
    default_skin = os.environ.get("MIHO_DEFAULT_SKIN", "default").strip() or "default"
