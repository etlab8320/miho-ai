"""
Dump command for miho CLI.

Outputs a compact, plain-text summary of the user's Miho setup
...
from miho_cli.config import get_miho_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``miho dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from miho_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(miho_home: Path) -> int:
...
def _cron_summary(miho_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = miho_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"miho_home:      {display_miho_home()}")
    ...
    toolsets = config.get("toolsets", ["miho-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(miho_home)}")
    lines.append(f"  skills:             {_count_skills(miho_home)}")
