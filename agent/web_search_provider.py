...
``kind: backend``) or ``~/.miho/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``miho tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``miho tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by ``miho_cli/tools_config.py`` to inject this provider as a
