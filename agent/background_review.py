...
See the ``miho-agent-dev`` skill (``references/self-improvement-loop.md``)
...
from agent.background_review_prompts import (
...
def summarize_background_review_actions(
    ...
    for msg in review_messages or []:
        ...
        if data.get("id") and data.get("category") and data.get("title"):
            actions.append("Owner profile updated")
...
def build_memory_write_metadata(
    ...
    *,
    ...
    metadata: Dict[str, Any] = {
        ...
        "platform": agent.platform or os.environ.get("MIHO_SESSION_SOURCE", "cli"),
        "tool_name": "memory",
    }
    if task_id:
...
def _run_review_in_thread(
    ...
    try:
        with open(os.devnull, "w", encoding="utf-8") as _devnull, \
            ...
            # skill_manage). Those tools require Miho' own dispatch,
            # which the codex_app_server runtime bypasses entirely
            # (it runs the turn inside codex's subprocess). So when
            # the parent is on codex_app_server, downgrade the review
            # fork to codex_responses — same auth/credentials, but
            # talks to the OpenAI Responses API directly so Miho
            ...
            # scratch (fresh _miho_now() timestamp, fresh
            ...
            review_whitelist = {
                ...
                for t in get_tool_definitions(
                    enabled_toolsets=["memory", "owner_profile", "skills"],
                    quiet_mode=True,
                )
            }
            set_thread_tool_whitelist(
            ...
            try:
                review_agent.run_conversation(
                    user_message=(
                        prompt
                        + "\n\nYou can only call memory, owner_profile, "
