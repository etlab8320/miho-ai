...
Miho pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.miho/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<miho_home>/bin/bws`` on
  first use.  Miho pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.miho/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``miho`` invocations don't
  hammer the API.
* Failures NEVER block Miho startup.  Missing binary, no network,
...
# In-process cache so repeated load_miho_dotenv() calls (CLI startup,
...
def _miho_bin_dir() -> Path:
    """Where Miho stores its managed binaries.  Profile-aware."""
    from miho_constants import get_miho_home

    return get_miho_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<miho_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _miho_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``miho secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _miho_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="miho-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "miho-agent"})
    try:
...
# ---------------------------------------------------------------------------
# Public entry point — called from miho_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_miho_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `miho secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `miho secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `miho secrets bitwarden setup` to install."
        )
        return result
