...
from miho_constants import get_miho_home, get_skills_dir, is_wsl
...
_MIHO_MD_NAMES = (".miho.md", "MIHO.md")


def _find_miho_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.miho.md`` or ``MIHO.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Miho Agent, an intelligent AI assistant created by Nous Research. "
...
MIHO_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Miho Agent "
    "itself, load the `miho-agent` skill with skill_view(name='miho-agent') "
    "before answering. Docs: https://miho-agent.nousresearch.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.miho/kanban.db`. Your task id is in "
    "`$MIHO_KANBAN_TASK`; your workspace is `$MIHO_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $MIHO_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `miho kanban <verb>` for board operations. Use "
...
OPENAI_MODEL_EXECUTION_GUIDANCE = (
    ...
    "\n"
    "<tool_economy>\n"
    "- Tool efficiency must preserve accuracy. Never replace lookup, inspection, "
    "tests, logs, or live state checks with hardcoded assumptions.\n"
    "- Reduce only waste: duplicate reads in the same turn, cosmetic API calls, "
    "known-failing retries, and exploration that cannot affect the answer.\n"
    "- Reuse facts already verified in this turn instead of checking the same "
    "source again, but re-check if the state may have changed or the action is "
    "high-risk.\n"
    "- Prefer batching independent reads/lookups before analysis so you do fewer "
    "round trips without skipping evidence.\n"
    "</tool_economy>\n"
    "\n"
    "<mandatory_tool_use>\n"
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Miho WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Miho itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Miho runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Miho itself is running. The host OS, home, and cwd "
                f"of the Miho process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Miho "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_miho_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.miho/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("MIHO_PLATFORM")
        or get_session_env("MIHO_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Miho Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `miho-agent` skill "
            "first. It has the actual commands (e.g. `miho config set …`, `miho tools`, "
            "`miho setup`) so you don't have to guess or invent workarounds.\n"
            "If a skill has issues, fix it with skill_manage(action='patch').\n"
            "After difficult/iterative tasks, offer to save as a skill. "
            "If a skill you loaded was missing steps, had wrong commands, or needed "
            "pitfalls you discovered, update it before finishing. If the evidence is "
            "not strong enough for an immediate skill edit, record it with "
            "skill_curator as a failure_pattern, update_existing, or new_skill "
            "candidate so the daily curator can review it.\n"
            "\n"
            "<available_skills>\n"
            + "\n".join(index_lines) + "\n"
            "</available_skills>\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: miho setup, miho setup tools, miho setup terminal, miho status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from MIHO_HOME and return its content, or None.
    ...
    """
    try:
        from miho_cli.config import ensure_miho_home
        ensure_miho_home()
    except Exception as e:
        logger.debug("Could not ensure MIHO_HOME before loading SOUL.md: %s", e)

    soul_path = get_miho_home() / "SOUL.md"
...
def _load_miho_md(cwd_path: Path) -> str:
    """.miho.md / MIHO.md — walk to git root."""
    miho_md_path = _find_miho_md(cwd_path)
    if not miho_md_path:
        return ""
    try:
        content = miho_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = miho_md_path.name
        try:
            rel = str(miho_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".miho.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", miho_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .miho.md / MIHO.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from MIHO_HOME is independent and always included when present.
    ...
    # SOUL.md from MIHO_HOME only — skip when already loaded as identity
    if not skip_soul:
