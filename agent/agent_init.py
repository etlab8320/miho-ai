...
from miho_cli.timeouts import get_provider_request_timeout
...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.miho/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from miho_cli.config import load_config as _load_pc_cfg
    ...
    # both live under ~/.miho/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from miho_logging import setup_logging, setup_verbose_logging
    setup_logging(miho_home=_ra()._miho_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that miho_logging.setup_logging() does not
        ...
        # handler level inside miho_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from miho_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from miho_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `miho model` to "
                        "select a provider, or run `miho setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff MIHO_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["MIHO_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.miho/sessions/ alongside gateway sessions
    miho_home = get_miho_home()
    agent.logs_dir = miho_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.miho/sessions/session_{sid}.json)
    ...
    try:
        from miho_cli.config import load_config as _load_sess_cfg
    ...
    # When that happens, conversation_loop disables replay for this session.
    ...
    try:
        from miho_cli.config import load_config as _load_agent_config
        ...
        try:
            ...
            if agent._memory_enabled or agent._user_profile_enabled:
                ...
                agent._memory_store = MemoryStore(
                    ...
                    user_char_limit=mem_config.get("user_char_limit", 2200),
                )
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "miho"
                    except Exception:
                        pass
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from miho_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from miho_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Miho Agent.  Choose a model with at least "
