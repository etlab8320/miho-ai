...
upstream prefix cache warm.  See ``miho-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Miho never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want MIHO_HOME persona while keeping
    ...
    # Pointer to the miho-agent skill + docs for user questions about Miho itself.
    stable_parts.append(MIHO_AGENT_HELP_GUIDANCE)
    ...
    # MIHO_KANBAN_TASK env var). Normal chat sessions never see
    ...
    if agent.valid_tool_names:
        ...
        if _inject:
            ...
            # Execution discipline: tool persistence, prerequisite checks,
            # verification, anti-hallucination, and retry-on-empty. Family-
            # agnostic despite the OPENAI_ name — the same failure modes (claims
            # completion without tool calls, suggests workarounds instead of
            # using existing tools, replies with plans instead of executing,
            # accepts empty/failed results as final) hit EVERY non-Gemini model,
            # including the self-hosted Hermes/Nous/local builds this product is
            # sold to run on. Gemini/Gemma get their own guidance above.
            elif _model_lower:
    ...
    if not agent.skip_context_files:
        ...
        # mode).  The gateway process runs from the miho-agent install
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Miho never rebuilds or reinjects parts of it
