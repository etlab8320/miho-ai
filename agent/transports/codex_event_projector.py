"""Projects codex app-server events into Miho' messages list.

The translator that lets Miho' memory/skill review keep working under the
...
Miho' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Miho
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Miho' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Miho stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Miho already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Miho' messages store text only.
        text_parts: list[str] = []
