...
Owns one Codex thread per Miho session. Drives `turn/start`, consumes
...
# Miho' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_MIHO_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Miho/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Miho session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _MIHO_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("MIHO_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="miho",
            client_title="Miho Agent",
            client_version=_get_miho_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Miho' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Miho' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own miho-tools callback we
            # auto-accept — the user already approved Miho' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Miho approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Miho returns 'once', 'session', 'always', or 'deny'.
...
def _get_miho_version() -> str:
    """Best-effort Miho version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("miho-agent")
