...
def run_conversation(
    ...
    # ``miho logs --session <id>`` can filter a single conversation.
    from miho_logging import set_session_context
    ...
    # are reused.  The system prompt is Miho's territory; plugins
    ...
    # all run inside Codex). Default Miho path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Miho internals.
        #
        # Miho invariant: the system prompt is built ONCE per session
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if (
                    ...
                    from miho_constants import display_miho_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Miho-managed OAuth/setup tokens")
                ...
                # providers (DeepSeek, Kimi, MiMo, Miho).  A 429 can
                ...
                # which Miho' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Miho' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`miho",
                        force=True,
                    )
                    agent._vprint(
    ...
    if final_response is None and (
        ...
        _kanban_task = os.environ.get("MIHO_KANBAN_TASK")
        if _kanban_task:
            try:
