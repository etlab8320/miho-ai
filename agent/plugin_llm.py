...
Plugins built on Miho Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Miho
...
:class:`~miho_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Miho' main config
...
handles every provider, fallback chain, and per-task override Miho
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`miho_cli.plugins.PluginContext`
