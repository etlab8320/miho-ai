...
as ``kind: backend``) or ``~/.miho/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$MIHO_HOME/cache/images/``, creating parents as needed."""
    from miho_constants import get_miho_home

    path = get_miho_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$MIHO_HOME/cache/images/``.
