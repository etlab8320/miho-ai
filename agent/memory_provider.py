...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - miho_home (str): The active MIHO_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.miho``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "miho").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'miho memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], miho_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'miho memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``miho_home`` is the active MIHO_HOME directory path.
