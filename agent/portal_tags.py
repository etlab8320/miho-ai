...
Every Miho request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Miho Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=miho-agent",
        "client=miho-client-v<__version__>",
    ]

The version is sourced live from ``miho_cli.__version__`` so it auto-aligns
...
``miho_cli.__version__`` is the canonical source of truth.
"""
...
def _miho_version() -> str:
    """Return the current Miho release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``miho_cli`` cannot be imported (should
...
def miho_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=miho-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=miho-client-v{_miho_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=miho-agent", miho_client_tag()]
