...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Miho starts and finds ``~/.openclaw/``.

    Points users at ``miho claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``miho claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Miho, run "
        "`miho claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `miho claw cleanup` (renames it to ~/.openclaw.pre-migration — "
