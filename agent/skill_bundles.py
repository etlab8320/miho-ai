...
Storage
-------
Bundles live in ``~/.miho/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``MIHO_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``miho bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under MIHO_HOME.

    Honors ``MIHO_BUNDLES_DIR`` for tests; falls back to
    ``<MIHO_HOME>/skill-bundles``.
    """
    override = os.environ.get("MIHO_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_miho_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `miho bundles` CLI subcommand.
# ---------------------------------------------------------------------------
