"""Anthropic Messages API adapter for Miho Agent.

Translates between Miho's internal OpenAI-style message format and
...
# Miho effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Miho historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Miho)
    ...
    # 1. Miho-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Miho configs.
...
_MIHO_OAUTH_FILE = get_miho_home() / ".anthropic_oauth.json"
...
def run_miho_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Miho-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Miho with your Claude Pro/Max subscription.")
    print()
...
def read_miho_oauth_credentials() -> Optional[Dict[str, Any]]:
    ...
    if _MIHO_OAUTH_FILE.exists():
        try:
            ...
            logger.debug("Failed to read Miho OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See miho-agent#13848.
    #
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See miho-agent#13848 (Kimi) and
    miho-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Miho Agent", "Claude Code")
                text = text.replace("Miho agent", "Claude Code")
                text = text.replace("miho-agent", "claude-code")
    ...
    # silently hides reasoning text that Miho surfaces in its CLI. We
