...
as ``kind: backend``) or ``~/.miho/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``miho tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``miho tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``miho tools`` picker."""
        return {
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``miho tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$MIHO_HOME/cache/videos/``, creating parents as needed."""
    from miho_constants import get_miho_home

    path = get_miho_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$MIHO_HOME/cache/videos/``.
