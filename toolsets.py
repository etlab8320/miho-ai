...
_MIHO_CORE_TOOLS = [
    ...
    # Skills
    "skills_list", "skill_view", "skill_manage", "skill_curator",
    # Browser automation
    ...
    "send_message",
    # Governance delivery contract for generated artifacts
    "media_delivery_contract", "governance_os", "pixel_document_evidence",
    # Home Assistant smart home control (gated on HASS_TOKEN via check_fn)
    "ha_list_entities", "ha_get_state", "ha_list_services", "ha_call_service",
    # Kanban multi-agent coordination — only in schema when the agent is
    # spawned as a kanban worker (MIHO_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `miho tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "skills": {
        ...
        "tools": ["skills_list", "skill_view", "skill_manage", "skill_curator"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (MIHO_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Miho toolsets (CLI + messaging platforms)
    #
    ...
    "miho-cron": {
        # Mirrors miho-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `miho tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as miho-cli; gated by `miho tools`",
        "tools": _MIHO_CORE_TOOLS,
        "includes": []
    },
    ...
    "miho-email": {
        "description": "Email bot toolset - interact with Miho via email (IMAP/SMTP)",
        "tools": _MIHO_CORE_TOOLS,
        "includes": []
    },
    ...
    "miho-sms": {
        "description": "SMS bot toolset - interact with Miho via SMS (Twilio)",
        "tools": _MIHO_CORE_TOOLS,
        "includes": []
    },
    ...
    "miho-gateway": {
        ...
        "tools": [],
        "includes": ["miho-telegram", "miho-discord", "miho-whatsapp", "miho-slack", "miho-signal", "miho-bluebubbles", "miho-homeassistant", "miho-email", "miho-sms", "miho-mattermost", "miho-matrix", "miho-dingtalk", "miho-feishu", "miho-wecom", "miho-wecom-callback", "miho-weixin", "miho-qqbot", "miho-webhook", "miho-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (miho-<name>).
        # Gives them _MIHO_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("miho-"):
            platform_name = name[len("miho-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_MIHO_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
