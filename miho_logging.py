"""Centralized logging setup for Miho Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.miho/logs/`` (profile-aware via ``get_miho_home()``).
...
from miho_constants import get_config_path, get_miho_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_miho_session_injector", False):
    ...
    _session_record_factory._miho_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``miho logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "miho_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Miho logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    miho_home
        Override for the Miho home directory.  Falls back to
        ``get_miho_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_miho_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._miho_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the miho group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from miho_cli.config import is_managed
