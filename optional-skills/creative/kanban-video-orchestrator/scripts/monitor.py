#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `miho kanban list` and
...
def miho_available() -> bool:
    return shutil.which("miho") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed miho CLI."""
    try:
        out = subprocess.run(
            ["miho", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `miho kanban list`
    out = subprocess.run(
        ["miho", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def main():
    ...
    if not miho_available():
        print("ERROR: 'miho' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
