...
Maps each role archetype to the Miho skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public miho-agent
...
## Miho skills relevant to video production

### Visual / rendering skills (`miho-agent/skills/creative/`)
...
### Design / pre-production skills (`miho-agent/skills/creative/`)
...
### Audio / media skills (`miho-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`miho-agent/skills/devops/`)
...
These are **not** Miho skills but external CLIs / APIs that profiles invoke.
...
## Built-in Miho tools for media review

These are native Miho tools — not invoked via terminal but through their own
...
key is present in `~/.miho/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Miho profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.miho/.env` → environment variable.
...
When a new Miho-public video skill ships:
