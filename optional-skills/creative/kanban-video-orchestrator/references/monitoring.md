...
miho kanban watch --tenant <project-slug>

# Snapshot of the board
miho kanban list --tenant <project-slug>
miho kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
miho kanban stats --tenant <project-slug>

# Visual dashboard (browser)
miho dashboard

# Inspect a specific task (includes comments + events)
miho kanban show <task-id>

# Follow a single task's event stream
miho kanban tail <task-id>
```

Verify available subcommands with `miho kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `miho kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `miho kanban block <id>` then `miho kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `miho kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `miho kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `miho profile list` (profile exists?), `miho status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(miho kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
miho kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
miho kanban block <task-id>
miho kanban archive <task-id>

# Diagnose what happened
miho kanban show <task-id>      # task body, comments, recent events
miho kanban tail <task-id>      # follow the live event stream
miho kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `miho kanban create` for the director
...
while true; do
    clear
    miho kanban list --tenant <slug>
    echo "---"
    miho kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `miho kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `miho profile list` and start
