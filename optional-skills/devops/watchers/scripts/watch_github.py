...
Usage (via cron with --no-agent):
    ...
    miho cron create miho-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$MIHO_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name miho-issues --repo NousResearch/miho-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.miho/.env to avoid the 60 req/hr
