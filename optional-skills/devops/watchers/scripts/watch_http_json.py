...
Usage (via cron with --no-agent):
    ...
    miho cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$MIHO_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Miho-Watcher/1.0"})
