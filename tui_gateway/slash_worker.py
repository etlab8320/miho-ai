"""Persistent slash-command worker — one MihoCLI per TUI session.
...
def _run(cli: MihoCLI, command: str) -> str:
...
def main():
    ...
    os.environ["MIHO_SESSION_KEY"] = args.session_key
    os.environ["MIHO_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = MihoCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
