"""Windows UTF-8 bootstrap for Miho entry points.
...
should be imported at the very top of every Miho entry point
(``miho``, ``miho-agent``, ``miho-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import miho_bootstrap``
# (or ``from miho_bootstrap import apply_windows_utf8_bootstrap``) at
