# nix/nixosModules.nix — NixOS module for miho-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: miho runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $MIHO_HOME/.env
# and read by miho at startup — no container recreation needed for env changes.
#
# Tool resolution: the miho wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.miho-agent;
    effectivePackage =
    ...
    miho-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-miho-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "miho-config-attrs";
      description = "Miho YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "miho-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "miho-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'MIHO_DOC_EOF'\n${value}\nMIHO_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "miho-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/miho";
    ...
    # miho user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "miho-container-entrypoint" ''
      set -eu

      MIHO_UID="''${MIHO_UID:?MIHO_UID must be set}"
      MIHO_GID="''${MIHO_GID:?MIHO_GID must be set}"

      # ── Group: ensure a group with GID=$MIHO_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$MIHO_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="miho"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$MIHO_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$MIHO_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$MIHO_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$MIHO_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="miho"
        TARGET_HOME="/home/miho"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$MIHO_UID" -g "$MIHO_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$MIHO_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$MIHO_UID:$MIHO_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure MIHO_HOME is owned by the target user.
      ...
      if [ -n "''${MIHO_HOME:-}" ] && [ -d "$MIHO_HOME" ]; then
        find "$MIHO_HOME" \! -user "$MIHO_UID" -exec chown "$MIHO_UID:$MIHO_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/miho-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/miho-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/miho ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/miho
        chmod 0440 /etc/sudoers.d/miho
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$MIHO_UID" --regid="$MIHO_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $MIHO_HOME/.env.
    ...
    # Default: /var/lib/miho/workspace → /data/workspace.
  ...
  in {
    options.services.miho-agent = with lib; {
      enable = mkEnableOption "Miho Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = miho-agent;
        description = "The miho-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/miho";
        description = "State directory. Contains .miho/ subdir (MIHO_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Miho config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $MIHO_HOME/.env at activation time.
          Miho reads this file on every startup via load_miho_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $MIHO_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $MIHO_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `miho gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the miho user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the miho plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Miho discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          miho_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as miho (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in miho-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the miho CLI to environment.systemPackages and export
          MIHO_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.miho symlink to the service
            stateDir. These users are automatically added to the miho group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.miho-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the miho CLI to system PATH and export MIHO_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.miho/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.MIHO_HOME = "${cfg.stateDir}/.miho";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.miho-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the miho user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.miho-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed miho
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure miho is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.miho        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.miho/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.miho/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.miho/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.miho/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.miho/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."miho-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.miho
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.miho ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.miho ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.miho -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.miho/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.miho/$_subdir"
            chmod 2770 "${cfg.stateDir}/.miho/$_subdir"
            find "${cfg.stateDir}/.miho/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.miho/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.miho/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.miho/config.yaml
            chmod 0640 ${cfg.stateDir}/.miho/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.miho/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.miho/.managed
          chmod 0644 ${cfg.stateDir}/.miho/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.miho/.container-mode <<'MIHO_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    miho_bin=${containerDataDir}/current-package/bin/miho
    MIHO_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.miho/.container-mode
            chmod 0644 ${cfg.stateDir}/.miho/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.miho/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.miho";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.miho" ]; then
                  rm -f "${symlinkPath}"
                  echo "miho-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.miho -> stateDir/.miho for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.miho";
                target = "${cfg.stateDir}/.miho";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "miho-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.miho/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.miho/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.miho/auth.json
              fi
            ''}
          ''}
          ...
          # Miho reads $MIHO_HOME/.env at startup via load_miho_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.miho/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'MIHO_NIX_ENV_EOF'
    ${envFileContent}
    MIHO_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.miho/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.miho/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.miho/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.miho-agent = {
          description = "Miho Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $MIHO_HOME/.env by the activation script. load_miho_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/miho"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the miho group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.miho-agent = {
          description = "Miho Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              MIHO_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              MIHO_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env MIHO_UID="$MIHO_UID" \
                --env MIHO_GID="$MIHO_GID" \
                --env MIHO_HOME=${containerDataDir}/.miho \
                --env MIHO_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/miho gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
