...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      miho-agent = self'.packages.default;
      mihoVenv = miho-agent.mihoVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "miho-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${mihoVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "miho-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "miho-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${miho-agent}/bin/miho || (echo "FAIL: miho binary missing"; exit 1)
          test -x ${miho-agent}/bin/miho-agent || (echo "FAIL: miho-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${miho-agent}/bin/miho version 2>&1 | grep -qi "miho" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "miho-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in miho miho-agent miho-acp; do
            test -x ${miho-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "miho-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking miho --help ==="
          ${miho-agent}/bin/miho --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${miho-agent}/bin/miho --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "miho-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${miho-agent}/share/miho-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${miho-agent}/share/miho-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "MIHO_BUNDLED_SKILLS" ${miho-agent}/bin/miho || \
            (echo "FAIL: MIHO_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: MIHO_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "miho-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${miho-agent}/share/miho-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${miho-agent}/share/miho-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "MIHO_BUNDLED_PLUGINS" ${miho-agent}/bin/miho || \
            (echo "FAIL: MIHO_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: MIHO_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "miho-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${miho-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${miho-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "MIHO_TUI_DIR" ${miho-agent}/bin/miho || \
            (echo "FAIL: MIHO_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: MIHO_TUI_DIR set in wrapper"
        ...
        # Verify MIHO_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        miho-node = pkgs.runCommand "miho-node-version" { } ''
          set -e
          echo "=== Checking MIHO_NODE in wrapper ==="
          grep -q "MIHO_NODE" ${miho-agent}/bin/miho || \
            (echo "FAIL: MIHO_NODE not set in wrapper"; exit 1)
          echo "PASS: MIHO_NODE present in wrapper"

          MIHO_NODE=$(sed -n "s/^export MIHO_NODE='\(.*\)'/\1/p" ${miho-agent}/bin/miho)
          test -x "$MIHO_NODE" || (echo "FAIL: MIHO_NODE=$MIHO_NODE not executable"; exit 1)
          echo "PASS: MIHO_NODE executable at $MIHO_NODE"

          NODE_MAJOR=$("$MIHO_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All MIHO_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify MIHO_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "miho-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(MIHO_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking MIHO_MANAGED guards ==="
          check_blocked "config set" ${miho-agent}/bin/miho config set model foo
          check_blocked "config edit" ${miho-agent}/bin/miho config edit
        ...
        extra-python-packages = let
          ...
          mihoWithExtra = miho-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "miho-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${mihoWithExtra}/bin/miho || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${mihoWithExtra}/bin/miho || \
          ...
          if grep -q "PYTHONPATH" ${miho-agent}/bin/miho; then
        ...
        extra-dependency-groups = let
          mihoWithGroups = miho-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "miho-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${mihoWithGroups}"
          echo "venv: ${mihoWithGroups.mihoVenv}"
        ...
        in pkgs.runCommand "miho-config-roundtrip" {
          ...
          merge_and_load() {
            ...
            ${configMergeScript} ${nixSettings} "$miho_home/config.yaml"
            ${mihoVenv}/bin/python3 -c '
import json, sys
