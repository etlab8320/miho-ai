"""ACP auth helpers — detect and advertise Miho authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "miho-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Miho runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Miho can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Miho.
    ...
    not have Miho provider credentials configured yet, so Miho always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Miho using the currently configured "
