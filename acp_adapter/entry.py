"""CLI entry point for the miho-agent ACP adapter.

Loads environment variables from ``~/.miho/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from MIHO_HOME (default ``~/.miho``)."""
    from miho_cli.env_loader import load_miho_dotenv

    miho_home = get_miho_home()
    loaded = load_miho_dotenv(miho_home=miho_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", miho_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="miho-acp",
        description="Run Miho Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Miho version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _run_check() -> None:
    ...
    from acp_adapter.server import MihoACPAgent  # noqa: F401

    print("Miho ACP check OK")


def _run_setup() -> None:
    from miho_cli.main import main as miho_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "miho", "model"]
        miho_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``miho postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting miho-agent ACP adapter")
    ...
    import acp
    from .server import MihoACPAgent
