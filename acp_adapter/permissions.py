"""ACP permission bridging for Miho dangerous-command approvals."""
...
# Maps ACP permission option ids to Miho approval result strings.
...
def _build_permission_options(*, allow_permanent: bool) -> list[PermissionOption]:
    """Return ACP options that match Miho approval semantics."""
    options = [
        PermissionOption(option_id="allow_once", kind="allow_once", name="Allow once"),
        PermissionOption(
            option_id="allow_session",
            # ACP has no session-scoped kind, so use the closest persistent
            # hint while keeping Miho semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_miho(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into Miho approval strings."""
    ...
    if option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_MIHO.get(option_id, "deny")
...
def make_approval_callback(
    ...
    """
    Return a Miho-compatible approval callback that bridges to ACP.
