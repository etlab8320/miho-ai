...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.MIHO_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Miho on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via MIHO_TUI_TRUECOLOR=1.
