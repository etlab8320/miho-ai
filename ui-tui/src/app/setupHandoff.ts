import type { RunExternalProcess } from '@miho/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`miho ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching miho: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`miho ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
