import { forceRedraw, type MouseTrackingMode } from '@miho/ink'
...
export const coreCommands: SlashCommand[] = [
  ...
  {
    help: 'update Miho Agent to the latest version (exits TUI)',
    name: 'update',
    run: (_arg, ctx) => {
      ctx.transcript.sys('exiting TUI to run update...')
      // Exit code 42 signals the Python wrapper to exec `miho update`.
    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys(
            'clipboard copy failed — try MIHO_TUI_FORCE_OSC52=1 to force the escape sequence'
          )
        }
      }
...

  {
    ...
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `Miho #${i + 1}`
