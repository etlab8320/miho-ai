...
await build({
  ...
  // Skip the prebuilt @miho/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@miho/ink': resolve(root, 'packages/miho-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The miho_cli launcher
