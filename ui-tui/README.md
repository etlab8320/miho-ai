...
React + Ink terminal UI for Miho. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `MIHO_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.miho/.miho_history` or under `MIHO_HOME`.
...
```text
ui-tui/
  packages/miho-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      miho-ink.d.ts    type declarations for @miho/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent MihoCLI subprocess for slash commands
```
